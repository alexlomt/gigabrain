...
import { ensureProjectionStore, normalizeProjectionScope, resolveVisibleProjectionScopes } from './projection-store.js';
...
const ENTITY_NOISE_STOPWORDS = new Set([
  ...
  'warm', 'operator', 'creator', 'evidence', 'public', 'research',
...
const COMPOUND_ENTITY_CUE_PATTERNS = Object.freeze([
  /\b(?:[Pp]roject|[Rr]epo(?:sitory)?|[Pp]roduct|[Ff]eature|[Ll]aunch|[Rr]ollout|[Rr]oadmap|[Vv]ault|[Pp]lugin|[Rr]elease)\s+([A-ZÄÖÜ][A-Za-zÄÖÜäöüß0-9'’.-]*(?:\s+[A-ZÄÖÜ][A-Za-zÄÖÜäöüß0-9'’.-]*){0,4})\b/gu,
  /\b(?:[Cc]ompany|[Ss]tartup|[Ff]irm|[Oo]rganization|[Oo]rganisation|[Bb]usiness|[Bb]ank)\s+([A-ZÄÖÜ][A-Za-zÄÖÜäöüß0-9'’.-]*(?:\s+[A-ZÄÖÜ][A-Za-zÄÖÜäöüß0-9'’.-]*){0,4})\b/gu,
]);
const LEADING_ENTITY_CUE_TOKENS = new Set(['project', 'repo', 'repository', 'product', 'feature', 'launch', 'rollout', 'roadmap', 'vault', 'plugin', 'release', 'company', 'startup', 'firm', 'organization', 'organisation', 'business', 'bank']);
const LEADING_ROLE_TITLE_RE = /^\s*([A-ZÄÖÜ][A-Za-zÀ-ÖØ-öø-ÿ0-9&'’._/-]*(?:\s+[A-ZÄÖÜ][A-Za-zÀ-ÖØ-öø-ÿ0-9&'’._/-]*){1,7})(?=\s+(?:is\b|must\b|should\b|belongs\b|routes\b|reports\b|works\b|keeps\b|output\b|identity\b))/u;
...
const collectCompoundEntityCandidates = (text = '') => {
  const phrases = new Set();
  const blockedSingles = new Set();
  const pushPhrase = (value, options = {}) => {
    const phrase = String(value || '').trim();
    const normalized = normalizeContent(phrase);
    if (!normalized) return;
    let tokens = normalized.split(/\s+/).filter(Boolean);
    if (tokens[0] && LEADING_ENTITY_CUE_TOKENS.has(tokens[0]) && tokens.length >= 3) {
      tokens = tokens.slice(1);
    }
    if (tokens.length < 2) return;
    if (tokens.every((token) => ENTITY_NOISE_STOPWORDS.has(token))) return;
    const normalizedPhrase = tokens.join(' ');
    phrases.add(normalizedPhrase);
    const blockSinglesMode = options.blockSinglesMode === 'generic' ? 'generic' : 'all';
    for (const token of tokens) {
      if (token.length < 3) continue;
      if (blockSinglesMode === 'generic' && !ENTITY_NOISE_STOPWORDS.has(token)) continue;
      blockedSingles.add(token);
    }
  };
  for (const pattern of COMPOUND_ENTITY_CUE_PATTERNS) {
    for (const match of text.matchAll(pattern)) {
      pushPhrase(match?.[1] || '', { blockSinglesMode: 'all' });
    }
  }
  const leadingRoleTitle = String(text.match(LEADING_ROLE_TITLE_RE)?.[1] || '').trim();
  if (leadingRoleTitle) pushPhrase(leadingRoleTitle, { blockSinglesMode: 'generic' });
  return {
...
const splitNameCandidates = (value) => {
  ...
  const out = new Set();
  const compound = collectCompoundEntityCandidates(text);
  for (const phrase of compound.phrases) out.add(phrase);
  const proper = text.match(/\b[A-ZÄÖÜ][a-zäöüß]{2,}\b/g) || [];
  for (const item of proper) {
    const normalized = normalizeContent(item);
    if (compound.blockedSingles.has(normalized)) continue;
...
const buildEntityMentionScopeFilter = (scope = '', column = 'scope') => {
  ...
  const visibleScopes = resolveVisibleProjectionScopes(normalizedScope);
  if (visibleScopes.length === 0) {
    return { sql: '1=1', params: [] };
  }
  return {
    sql: `${column} IN (${visibleScopes.map(() => '?').join(',')})`,
    params: visibleScopes,
  };
};
...
const resolveEntityKeysForQuery = (db, query, options = {}) => {
  ...
  if (options?.fallbackTokens === true || (options?.fallbackTokens !== false && out.size === 0)) {
    for (const token of tokens) {
