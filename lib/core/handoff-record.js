...
import { expandMemorySourceLinks, getSyncStatus, hasSecretRisk, listMemorySources, redactHandoffText } from './host-memory-sync.js';
...
const redactedPreview = (value = '', maxLength = 180) => {
  const redacted = redactHandoffText(value).replace(/\s+/g, ' ').trim();
...
const buildHandoffBrief = ({ heading, label, rows = [], scope = '', limit = 25 } = {}) => {
  ...
  if (selected.length === 0) {
    lines.push('_No active memories matched this Handoff Record scope._');
  } else {
...
const buildPassportMarkdown = (passport) => {
  ...
  const lines = [
    ...
    '## What this is',
    '',
    'Gigabrain is a local-first, read-only layer over the memory stores your coding agents already keep. This Memory Audit inventories those host memories and flags trust risks (duplicates, contradictions, stale rows, provenance gaps, secret leaks). It also produces safe Handoff Records: redacted context passes you can paste into another agent. Cross-store arbitration (deciding who is right, with a receipt) is the roadmap.',
    '',
    '## Summary',
    '',
...
<head>
  ...
  <title>Gigabrain Memory Audit</title>
  <style>
...
<body>
  <main>
    <h1>Gigabrain Memory Audit</h1>
    <div class="meta">Generated ${escapeHtml(passport.generated_at)} · Scope ${escapeHtml(passport.scope || 'all scopes')}</div>
    <p>Local-first, read-only audit of the memory stores your coding agents already keep. Source inventory, audit findings, and safe Handoff Records are generated from the local Gigabrain registry.</p>
...
const buildMemoryPassport = ({
  ...
  const passport = {
    ...
    product: {
      name: 'Gigabrain Memory Audit',
      positioning: 'Local-first, read-only audit + Handoff Records for the memory stores AI agents already keep',
...
const writeMemoryPassport = (passport, { outputDir, formats = ['all'], includeHandoffs = true } = {}) => {
  ...
  if (includeHandoffs && selected.has('handoffs')) {
    const handoffDir = path.join(outputDir, 'handoff-records');
