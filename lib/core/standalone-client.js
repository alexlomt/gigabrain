...
const readJson = (filePath, fallback = {}, options = {}) => {
  const state = readFileIfExistsSync(filePath, 'utf8');
...
const ensureGitIgnoreEntry = (projectRoot, entry = '.gigabrain/') => {
  ...
  const existing = readFileIfExistsSync(gitignorePath, 'utf8').data;
  ...
  const next = existing.trim().length > 0
    ? `${existing.trimEnd()}\n${entry}\n`
    : `${entry}\n`;
  atomicWriteFileSync(gitignorePath, next, { mode: 0o644 });
  return { changed: true, path: gitignorePath };
};

const writeExecutableFile = (filePath, content) => {
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  atomicWriteFileSync(filePath, content, { mode: 0o755 });
};
...
const upsertMcpServerEntry = ({
  ...
  const existing = readFileIfExistsSync(mcpPath, 'utf8').data;
  ...
  if (nextText !== existing) atomicWriteFileSync(mcpPath, nextText, { mode: 0o600 });
  return {
