...
import { getCurrentMemory, recordVerdict, withProjectionMutationBatch } from './projection-store.js';
import {
...
const consolidateBeliefRows = (beliefRows = [], options = {}) => {
  ...
  if (db && verdictIntents.length > 0) {
    const intentFingerprint = createHash('sha256').update(JSON.stringify(
      verdictIntents.map((intent) => ({
        loserIds: [...intent.loserIds].sort(),
        winnerId: intent.winnerId,
      })).sort((left, right) => left.winnerId.localeCompare(right.winnerId)),
    )).digest('hex').slice(0, 24);
    const operationId = String(options.operationId || `belief-arbitration-${intentFingerprint}`);
    const applyVerdicts = (tx) => {
    ...
    if (options.tx) applyVerdicts(options.tx);
    else withProjectionMutationBatch({ db, operationId }, applyVerdicts);
  }
...
const runBeliefArbitration = ({
  ...
  const { rows, conflictGroups, verdicts } = consolidateBeliefRows(beliefRows, {
    db,
    faultInjector,
    operationId,
    settings,
