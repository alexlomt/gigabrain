...
const classifyHostTier = (host) => {
  ...
  // Manual/cloud identities are exact typed hosts. Attacker-controlled names
  // containing "manual" never inherit the manual-import tier.
  if (CLOUD.has(h)) return 'manual_import';
  if (WORKSPACE.has(h)) return 'workspace';
  if (OWN_AGENT.has(h)) return 'own_agent';
  // Exact membership is deliberate. Prefix/lookalike names are attacker-
  // controlled free text and remain at the unknown floor.
  return 'unknown';
};
...
// membership only: prefix/lookalike variants never inherit host or agent trust.
