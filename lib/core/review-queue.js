...
const applyQueueRetentionUnlocked = (queuePath, retentionConfig = {}, options = {}) => {
  ...
  let queueState = readFileIfExistsSync(queuePath, 'utf8');
  if (!queueState.exists && createFileExclusiveSync(queuePath, '', { mode: 0o600 })) {
    ensureQueueDir(queuePath, allowedRoot);
    return {
      applied: true,
      before_rows: 0,
  ...
  if (!queueState.exists) queueState = readFileIfExistsSync(queuePath, 'utf8');

  const raw = queueState.data;
  ...
  if (!dryRun && nextRaw !== raw) {
    ensureQueueDir(queuePath, allowedRoot);
    atomicWriteFileSync(queuePath, nextRaw, { mode: 0o600 });
  }
...
const appendQueueRow = (queuePath, row, options = {}) => {
  ...
  return withQueueLock(queuePath, options?.lock || {}, () => {
    ...
    // Queue rows hold parked memory content (and are the only copy for
    // parked remember-intents) — keep them out of world-readable reach.
    try { fs.chmodSync(queuePath, 0o600); } catch { /* best-effort */ }
...
// U16 read surface (agent-native review F3): list/filter queue entries.
// READ-ONLY by design this unit — listing never rewrites, resolves, or
// retention-compacts the file; resolution/act-on is future scope. Filters:
// status (default 'pending'; 'all' disables the filter) and reason code
// (matches either the row's reason_code or its legacy reason field — both
// are written by producers). Entries return newest-first.
const listQueueEntries = (queuePath, {
  status = 'pending',
  reasonCode = '',
  limit = 100,
  allowedRoot = '',
  ...
  const safeLimit = clampInt(limit, 1, 10000, 100);
  if (!queuePath || !fs.existsSync(queuePath)) {
    return { entries: [], total: 0, malformed: 0 };
  }
  const parsed = parseRows(fs.readFileSync(queuePath, 'utf8'));
  const wantStatus = String(status || '').trim().toLowerCase();
  const wantReason = String(reasonCode || '').trim().toLowerCase();
  let matched = parsed.rows;
  if (wantStatus && wantStatus !== 'all') {
    matched = matched.filter((row) => String(row?.status || '').trim().toLowerCase() === wantStatus);
  }
  if (wantReason) {
    matched = matched.filter((row) => (
      String(row?.reason_code || '').trim().toLowerCase() === wantReason
      || String(row?.reason || '').trim().toLowerCase() === wantReason
    ));
  }
  const total = matched.length;
  const entries = matched
    .map((row, index) => ({ row, index, timestampMs: rowTimestampMs(row, 0) }))
    .sort((a, b) => Number(b.timestampMs || 0) - Number(a.timestampMs || 0) || Number(b.index || 0) - Number(a.index || 0))
    .slice(0, safeLimit)
    .map((entry) => entry.row);
  return { entries, total, malformed: parsed.malformed };
};
