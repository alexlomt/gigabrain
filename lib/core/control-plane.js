...
const controlPlaneError = (code, message = '') => {
...
const ensureControlPlaneStore = (db) => {
  ...
  const duplicate = db.prepare(`
    ...
    FROM (
      ...
      GROUP BY scope, session_id
      HAVING COUNT(*) > 1
    )
  `).get();
  if (Number(duplicate?.group_count || 0) > 0) {
    throw controlPlaneError(
      'CHECKPOINT_SCOPE_SESSION_DUPLICATES',
      `${Number(duplicate.group_count)} group(s), ${Number(duplicate.row_count)} row(s)`,
    );
  }
  db.exec(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_memory_checkpoints_scope_session_unique
      ON memory_checkpoints(scope, session_id);
  `);
  const index = db.prepare('PRAGMA index_list(memory_checkpoints)').all()
    .find((row) => String(row.name || '') === 'idx_memory_checkpoints_scope_session_unique');
  const columns = db.prepare('PRAGMA index_info(idx_memory_checkpoints_scope_session_unique)').all()
    .map((row) => String(row.name || ''));
  if (Number(index?.unique || 0) !== 1 || JSON.stringify(columns) !== JSON.stringify(['scope', 'session_id'])) {
    throw controlPlaneError('CHECKPOINT_SCOPE_SESSION_INDEX_INVALID');
  }
...
let checkpointSavepointCounter = 0;
const beginCheckpointMutation = (db) => {
  if (db.isTransaction) {
    checkpointSavepointCounter += 1;
    const name = `checkpoint_${process.pid}_${checkpointSavepointCounter}`;
    db.exec(`SAVEPOINT ${quoteSqlIdentifier(name)}`);
    return { name, nested: true };
  }
  db.exec('BEGIN IMMEDIATE');
  return { name: '', nested: false };
};

const commitCheckpointMutation = (db, transaction) => {
  if (transaction.nested) db.exec(`RELEASE SAVEPOINT ${quoteSqlIdentifier(transaction.name)}`);
  else db.exec('COMMIT');
};

const rollbackCheckpointMutation = (db, transaction) => {
  if (transaction.nested) {
    db.exec(`ROLLBACK TO SAVEPOINT ${quoteSqlIdentifier(transaction.name)}`);
    db.exec(`RELEASE SAVEPOINT ${quoteSqlIdentifier(transaction.name)}`);
  } else {
    db.exec('ROLLBACK');
  }
};

const isCheckpointScopeSessionConflict = (error) => (
  (
    String(error?.code || '') === 'SQLITE_CONSTRAINT_UNIQUE'
    || (String(error?.code || '') === 'ERR_SQLITE_ERROR' && Number(error?.errcode) === 2067)
  )
  && (
    /UNIQUE constraint failed:\s*memory_checkpoints\.scope,\s*memory_checkpoints\.session_id/i.test(String(error?.message || ''))
    || String(error?.message || '').includes('idx_memory_checkpoints_scope_session_unique')
  )
);
...
const appendCheckpointEpisode = (db, input = {}) => {
  ...
  const injectFault = (stage) => {
    if (typeof input.faultInjector === 'function') input.faultInjector(stage);
  };
  const resolveDeduplicated = (existingRow) => {
    injectFault('before_checkpoint_hydration');
    const existing = hydrateCheckpointRow(db, existingRow, input);
    injectFault('after_checkpoint_hydration');
    const receipt = insertReceipt(db, {
      createdAt,
      ...
      sessionId,
      inputRefs: [checkpointId],
      outputRefs: [existing.checkpoint_id, ...existing.proposal_ids],
      evidenceRefs: evidence,
      summary: 'Resolved a concurrent checkpoint append to the existing scope/session winner.',
      payload: {
        requested_checkpoint_id: checkpointId,
        winner_checkpoint_id: existing.checkpoint_id,
        schema_version: CHECKPOINT_SCHEMA_VERSION,
      },
    });
    return {
  ...
  const transaction = beginCheckpointMutation(db);
  try {
    ...
    `).get(scope, sessionId);
    if (existingRow) {
      const result = resolveDeduplicated(existingRow);
      commitCheckpointMutation(db, transaction);
      return result;
    }

    if (typeof input.onCreate === 'function') {
      const effect = input.onCreate({
        checkpointId,
        createdAt,
        scope,
        sessionId,
      ...
      if (effect && typeof effect === 'object') {
        if (effect.sourcePath !== undefined || effect.source_path !== undefined) {
          row.source_path = normalizeText(effect.sourcePath ?? effect.source_path);
        }
        if (effect.sourceLine !== undefined || effect.source_line !== undefined) {
          const sourceLine = effect.sourceLine ?? effect.source_line;
          row.source_line = Number.isFinite(Number(sourceLine)) ? Number(sourceLine) : null;
        }
        if (effect.sourceKind !== undefined || effect.source_kind !== undefined) {
          row.source_kind = normalizeText(effect.sourceKind ?? effect.source_kind) || 'daily_note';
        }
        if (effect.payload && typeof effect.payload === 'object') {
          row.payload = { ...row.payload, ...effect.payload };
        }
      }
    }
    row.record_hash = hashRecord(row);
    ...
    for (const [kind, values] of itemGroups) {
      ...
      for (let position = 0; position < values.length; position += 1) {
        ...
        injectFault('after_checkpoint_item');
        if (kind === 'durable_candidate' && createProposals) {
          ...
          injectFault('after_checkpoint_proposal');
        }
      }
    }
    ...
    injectFault('after_checkpoint_receipt');
    injectFault('before_checkpoint_hydration');
    const checkpoint = getCheckpointEpisode(db, checkpointId);
    injectFault('after_checkpoint_hydration');
    injectFault('before_checkpoint_commit');
    commitCheckpointMutation(db, transaction);
    return {
      checkpoint,
      deduplicated: false,
    ...
    try { rollbackCheckpointMutation(db, transaction); } catch { /* preserve original */ }
    if (isCheckpointScopeSessionConflict(error)) {
      const recovery = beginCheckpointMutation(db);
      try {
        const existingRow = db.prepare(`
          SELECT * FROM memory_checkpoints
          WHERE scope = ? AND session_id = ?
        ...
        const result = resolveDeduplicated(existingRow);
        commitCheckpointMutation(db, recovery);
        return result;
      } catch (recoveryError) {
        try { rollbackCheckpointMutation(db, recovery); } catch { /* preserve original */ }
        throw recoveryError;
      }
    }
    throw error;
