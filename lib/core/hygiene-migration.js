...
const TEST_ARTIFACT_RE = /\b(?:MEMTEST[_-]?|MEM_TEST[_-]?|TEMPORARY MEMORY[-\s]SYSTEM TEST|DIRECT NATIVE[-\s]WRITE TEST|SEMANTIC DUPLICATE TEST|NATIVE[-\s]PROMOTION TEST|CONTROL (?:REMEMBER|REPLACE) TEST|PROTECT[-\s]MAINTAIN TEST|TYPED DAILY[-\s]PROMOTION TEST)\b/i;
const TEST_SOURCE_SESSION_RE = /^memtest:/i;
...
const isTestArtifactMemory = (row = {}) => {
  const content = String(row.content || '');
  const sourceSession = String(row.source_session || '');
  return TEST_ARTIFACT_RE.test(content) || TEST_SOURCE_SESSION_RE.test(sourceSession);
};

const findActiveTestArtifactRows = (db) => {
  ensureProjectionStore(db);
  const rows = db.prepare(`
    SELECT memory_id, type, scope, status, content, source_session, tags
    FROM memory_current
    WHERE status = 'active'
      AND (
        ...
        OR upper(content) LIKE '%PROTECT-MAINTAIN TEST%'
        OR upper(content) LIKE '%PROTECT MAINTAIN TEST%'
        OR lower(coalesce(source_session, '')) LIKE 'memtest:%'
      )
    ORDER BY updated_at DESC, memory_id ASC
  `).all();
  return rows.filter((row) => isTestArtifactMemory(row));
};
