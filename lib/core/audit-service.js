...
const runAudit = async ({
  ...
  try {
    ...
    const resolvedOperationId = stableProjectionOperationId(operationId || resolvedRunId);
    ...
    if (normalizedMode === 'apply' && rows.length === 0) {
      const completedRows = db.prepare(`
        ...
        JOIN memory_events e ON e.memory_id = q.memory_id
          AND json_extract(e.payload, '$.operation_id') = ?
          AND json_extract(e.payload, '$.projection_event_kind') = 'row'
        WHERE q.review_version = ? AND COALESCE(q.config_fingerprint, '') = ?
        ORDER BY q.reviewed_at, q.memory_id
      `).all(resolvedOperationId, resolvedReviewVersion, reviewConfigFingerprint);
