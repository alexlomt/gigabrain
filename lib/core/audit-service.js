...
const stableProjectionOperationId = (value) => String(value || 'audit')
...
const runAudit = async ({
  ...
  try {
    ...
    const auditReceiptId = stableProjectionOperationId(`audit-review-${resolvedReviewVersion}-v1`);
    const resolvedOperationId = stableProjectionOperationId(operationId || auditReceiptId);
    const completedMemoryIds = new Set(db.prepare(`
      SELECT DISTINCT memory_id FROM memory_events
      WHERE component='review'
        AND json_extract(payload, '$.audit_receipt_id') = ?
        AND json_extract(payload, '$.projection_event_kind') = 'row'
    `).all(auditReceiptId).map((row) => String(row.memory_id || '')));
    ...
    const rows = [];
    const llmReviewActive = typeof reviewer === 'function'
      || (llmCfg.enabled && String(llmCfg.provider || 'none').trim().toLowerCase() !== 'none');
    const llmStats = {
    ...
    for (const memory of activeRows) {
      if (completedMemoryIds.has(String(memory.memory_id))) continue;
      ...
      if (canLlmReview) {
        const llmResult = await (reviewer || reviewWithLlm)({
          provider: llmCfg.provider,
            ...
            updateCurrentStatus(db, row.memory_id, afterStatus, {
              ...
              event: {
                ...
                payload: {
                  ...
                  committed_action: row.action,
                  committed_review_version: resolvedReviewVersion,
                },
              },
            });
          }
    ...
    if (normalizedMode === 'apply' && completedMemoryIds.size > 0) {
      const newlyReviewedRows = [...rows];
      const receiptRows = [];
      const completedRows = db.prepare(`
        SELECT q.*, c.type, c.scope, c.content, e.similarity, e.matched_memory_id,
               e.payload AS event_payload
        FROM memory_quality_reviews q
        JOIN memory_current c ON c.memory_id = q.memory_id
        JOIN memory_events e ON e.memory_id = q.memory_id
          AND json_extract(e.payload, '$.audit_receipt_id') = ?
          AND json_extract(e.payload, '$.projection_event_kind') = 'row'
        WHERE q.review_version = ?
        ORDER BY q.reviewed_at, q.memory_id
      `).all(auditReceiptId, resolvedReviewVersion);
      for (const row of completedRows) {
        if (!completedMemoryIds.has(String(row.memory_id || ''))) continue;
        const payload = parseJsonSafe(row.event_payload, {});
        receiptRows.push({
          ...
          value_label: String(payload.value_label || labelForAction(row.action)),
          ...
          canonical_hint: String(payload.canonical_hint || ''),
        });
      }
      const byMemoryId = new Map();
      const stableSort = (left, right) => (
        String(left.reviewed_at || '').localeCompare(String(right.reviewed_at || ''), 'en')
        || String(left.memory_id || '').localeCompare(String(right.memory_id || ''), 'en')
      );
      for (const row of receiptRows.sort(stableSort)) byMemoryId.set(row.memory_id, row);
      for (const row of newlyReviewedRows.sort(stableSort)) byMemoryId.set(row.memory_id, row);
      rows.splice(0, rows.length, ...byMemoryId.values());
    }

    const outputPaths = buildOutputPaths({ out, summary, samples }, config);
    if (typeof completionFaultInjector === 'function') completionFaultInjector('before_audit_output');
