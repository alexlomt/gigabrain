...
import { ensureProjectionStore, getCurrentMemory, listAdjudications, listBeliefsAsOf, materializeProjectionFromMemories } from './projection-store.js';
...
const createMemoryHttpHandler = ({
  ...
  const openReadDb = () => openDatabase(dbPath, { readOnly: true });
  ...
  return async (req, res) => {
    try {
      ...
      if (pathname === '/gb/bench/recall' && method === 'POST') {
        ...
        const db = openReadDb();
        try {
          const result = orchestrateRecall({ db, config, query, scope, scopeVisibility: recallScopeVisibility(scope) });
          sendJson(res, 200, {
            ok: true,
      ...
      if (pathname === '/gb/recall' && method === 'POST') {
        ...
        const db = openReadDb();
        try {
          const recall = orchestrateRecall({ db, config, query, scope, scopeVisibility: recallScopeVisibility(scope) });
