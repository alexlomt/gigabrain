...
import { normalizeMemoryType, parseGigabrainMetadata } from './native-metadata.js';
import { containsEntity } from './person-service.js';
import { normalizeContent } from './policy.js';
import { normalizeProjectionScope, resolveVisibleProjectionScopes } from './projection-store.js';
...
const parseChunksFromText = ({
  ...
  const pushChunk = ({
    ...
    chunks.push({
      ...
      scope: scope ? normalizeNativeQueryScope(scope) : null,
      memory_type: normalizeMemoryType(memoryType) || null,
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    const scoped = parseGigabrainMetadata(bullet[1] || '');
...
const syncNativeMemory = ({
  ...
  const includeFiles = Array.isArray(config?.native?.includeFiles)
    ? config.native.includeFiles.map((item) => path.resolve(String(item)))
    : [];
  const requestedSourcePaths = Array.isArray(sourcePaths)
    ? Array.from(new Set(sourcePaths
      .map((item) => String(item || '').trim())
      .filter(Boolean)
      .map((item) => path.resolve(item))))
    : [];
  const allCandidatePaths = resolveNativeSourcePaths(config);
  const candidatePaths = requestedSourcePaths.length > 0
    ? allCandidatePaths.filter((sourcePath) => requestedSourcePaths.includes(path.resolve(sourcePath)))
    : allCandidatePaths;
  const targeted = requestedSourcePaths.length > 0;
...
const queryNativeChunks = ({
  ...
  if (visibleScopes.length > 0) {
    where.push(`(${effectiveScopeSql} IN (${visibleScopes.map(() => '?').join(',')})${normalizedScope !== 'shared' ? ` OR ${effectiveScopeSql} = ''` : ''})`);
