    ...
    return requestedStores
      .map((storeTarget) => {
        ...
        return {
          storeTarget,
          config,
          scope: resolveScopeForTarget(config, storeTarget, options.scope),
          exactScope: false,
        };
      })
      .filter(Boolean);
...
const runRecall = async (options = {}) => {
  ...
  for (const { storeTarget, config, scope, exactScope } of readQueries) {
    const prepared = openPreparedDb({
      config,
      syncNative: !exactScope,
      rebuildWorldOnNativeChange: !exactScope,
  ...
  if (options.recordReceipt === true || options.record_receipt === true) {
    ...
    if (receiptConfig) {
      ...
      const prepared = openPreparedDb({
        ...
        syncNative: false,
        rebuildWorldOnNativeChange: false,
        allowMaintenance: receiptQuery?.exactScope !== true,
      });
      try {
