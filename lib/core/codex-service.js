...
import { ensureProjectionStore, getCurrentMemory, listAdjudications, listBeliefsAsOf, listCurrentMemories, materializeProjectionFromMemories, searchCurrentMemories, tableStats } from './projection-store.js';
...
// U15 cross-store merge ordering fix (plan/review minor): the old merge
// concatenated batches in store order and cut at topK, so the project batch
// ALWAYS outranked user/remote rows regardless of score — a weak project hit
// could evict a strong user-store answer. Merged ordering now respects scores:
// dedupe by content (keeping the higher-scoring duplicate), stable-sort by
// score descending (ties keep store order: project, then user, then remote),
// then cut at topK.
const mergeAnnotatedResults = (batches = [], topK = 8) => {
  ...
  for (const batch of batches) {
    for (const row of Array.isArray(batch?.results) ? batch.results : []) {
      ...
      if (Number(row.score || 0) > Number(prev.score || 0)) byKey.set(key, row);
    }
  }
  return order
...
// ---------------------------------------------------------------------------
// Relevance floor (MCP boundary only — the benchmark adapter and internal
// orchestration are untouched). Negative probes lack informative-token
// coverage while true hits match at least one token, so "at least one
// informative query token in the content, OR dense cosine above threshold"
// separates cleanly. Do NOT floor on the fused _score / hybrid_strength
// channels; they are pool-relative ordering signals, not confidence.
// ---------------------------------------------------------------------------
...
const resolveRelevanceFloor = (config = {}) => {
  ...
  return {
    ...
    minMatchedTokens: Math.max(0, Number(floor.minMatchedTokens ?? 1) || 1),
    denseCosine: Math.max(0, Math.min(1, Number(floor.denseCosine ?? 0.6))),
  };
};
...
const applyRelevanceFloor = (query, rows, floor) => {
  ...
  const evaluated = rows.map((row) => {
    ...
    const accepted = !lexicalApplicable
      || matched >= floor.minMatchedTokens
  ...
  return {
    ...
    diagnostics: {
      applied: true,
      min_matched_tokens: floor.minMatchedTokens,
...
const runRecall = async (options = {}) => {
  ...
  const merged = mergeAnnotatedResults(batches, topK);
