...
const runDoctor = async (options = {}) => {
  ...
  const provenance = tryLoadReleaseProvenance(context.projectConfig?.releaseRoot);
...
const runClaimDecide = (options = {}) => {
  ...
  const reopened = openProjectControlPlane({ ...options, scope: proposal.scope }, { write: true });
  try {
