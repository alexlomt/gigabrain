...
import { ensureProjectionStore, upsertCurrentMemory } from './projection-store.js';
...
const harvestTranscripts = (options = {}) => {
  ...
  for (const { filePath, host } of files) {
    ...
    if (newText) {
      ...
      for (let i = 0; i < lines.length; i += 1) {
        ...
        if (turnBudget <= 0) break; // budget exhausted: stop BEFORE consuming this line

        const turn = parseTurn(rawLine);
        if (turn) {
          summary.turns_segmented += 1;
          fileTurns += 1;
          const salience = scoreTurnSalience(turn);
          if (!salience.keep) {
            summary.turns_dropped += 1;
          } else {
            summary.turns_salient += 1;
            turnBudget -= 1;
            ...
            if (hasSecretRisk(turn.text)) {
              ...
              for (const fact of facts) {
                ...
                if (stored.inserted) summary.inserted_count += 1;
                summary.linked_count += 1;
                if (options.dryRun !== true) touched.add(stored.memoryId);
              }
            }
          }
        }
    ...
    // Advance the cursor over exactly the lines we consumed this run (CDC). On a
    // turn-budget cutoff, consumedOffset < endOffset so the rest is re-read next
    // run. dryRun never writes the cursor (so a dry run is repeatable).
  ...
  // Ingest-time arbitration (idea #1 mirrors host_sync Feature #3): re-arbitrate
  // JUST the slots this harvest touched, so a freshly-mined transcript fact that
  // contradicts a standing belief is judged in-band. The transcript trust FLOOR
  // guarantees the transcript fact loses to any deliberate memory of the slot.
  const arbitrate = options.arbitrate !== false && options.dryRun !== true;
  if (arbitrate && touched.size > 0) {
    ...
    if (arb.arbitration_busy_retries) summary.arbitration_busy_retries = arb.arbitration_busy_retries;
    if (arb.arbitration_error) summary.arbitration_error = arb.arbitration_error;
    if (arb.arbitration_busy_exhausted) summary.arbitration_busy_exhausted = true;
  }
...
const storeFact = ({ db, config, scope, host, filePath, sourceLine, content, type, dryRun }) => {
