...
const normalizeProjectionScope = (value = '', options = {}) => {
  ...
  if (lowered === 'shared') return 'shared';
  if (lowered === 'main') return 'profile:main';
  const segments = raw.split(':');
  if (segments.length === 2 && segments[0].toLowerCase() === 'profile' && segments[1].toLowerCase() === 'main') {
    return 'profile:main';
  }
...
const resolveVisibleProjectionScopes = (scope = '') => {
  const normalized = normalizeProjectionScope(scope || '', { allowEmpty: true });
  if (!normalized) return [];
  if (normalized === 'shared') return ['shared'];
  const scopes = [normalized];
  if (normalized === 'profile:main') scopes.push('main');
  scopes.push('shared');
...
const listCurrentMemories = (db, options = {}) => {
  ...
  const visibleScopes = resolveVisibleProjectionScopes(scope);
  if (visibleScopes.length > 0) {
    where.push(`memory_current.scope IN (${visibleScopes.map(() => '?').join(',')})`);
  ...
  for (const [memoryId, bonus] of ftsBoost) {
    ...
    try {
      ...
      if (ftsRow) {
        ...
        const ftsScope = normalizeProjectionScope(ftsRow.scope || '', { allowEmpty: true }) || String(ftsRow.scope || '').trim();
        if (visibleScopes.length > 0 && !visibleScopes.includes(ftsScope)) continue;
...
export {
  ...
  resolveVisibleProjectionScopes,
  upsertCurrentMemory,
  updateCurrentStatus,
  getCurrentMemory,
  listCurrentMemories,
