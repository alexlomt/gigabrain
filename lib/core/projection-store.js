...
const upsertCurrentMemory = (db, memory = {}, options = {}) => executeProjectionMutation({
  db,
  // `updated_at` is caller data, not an ingest-clock authority. Only an
  // explicit trusted override (tests/migrations) may replace wall time.
  now: options.now || '',
...
const recordVerdict = (db, { winnerId, loserIds = [], signals = {}, agentId = null, reason } = {}, options = {}) => {
  ...
  const run = (tx) => {
    ...
    const verdictEvent = appendProjectionOperationSummary({
      tx,
      faultInjector: options.faultInjector,
      event: {
        component: 'arbiter',
    ...
    if (String(winnerRow.status || '').toLowerCase() === 'superseded') {
      ...
      mutateCurrentMemoryWithLegacyProjection({
        ...
        event: {
          component: 'arbiter',
          action: 'arbiter:winner_temporal',
          reason_codes: reasonCodes,
          memory_id: winner,
          agent_id: agent,
          timestamp: nowIso,
      ...
      winnerEvent = tx.lastMutationEvent;
    }
    ...
    return { verdictEvent, winnerEvent, reinstateEvent, supersedeEvents };
  };
