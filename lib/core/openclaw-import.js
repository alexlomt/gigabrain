...
const loadLegacyRows = (sourceDb) => {
  ...
  return sourceDb.prepare(`
    SELECT
      ...
      ${col(columns, 'last_injected_at')},
      ${col(columns, 'last_confirmed_at')},
      ${col(columns, 'ttl_days')},
...
const existingCanonical = (db, { memoryId, normalizedHash, normalized, scope } = {}) => {
  ...
  return matched?.memory_id ? getCurrentMemory(db, matched.memory_id) : null;
};
...
const importOpenClawRegistry = ({
  ...
  try {
    ...
    const sourceFingerprint = hashNormalized(JSON.stringify({
    ...
    const runId = String(operationId || `openclaw-import-${normalizedSourceHost}-${sourceFingerprint}`);
    const apply = (tx = null) => {
      for (const row of rows) {
        ...
        if (dryRun !== true && !existing) {
          upsertCurrentMemory(db, {
            ...
            event: {
              action: 'openclaw_import_upsert',
              component: 'openclaw_import',
              payload: {
                source_host: normalizedSourceHost,
                source_label: String(sourceLabel || ''),
              ...
              reason_codes: ['openclaw_registry_import'],
              run_id: runId,
            },
            faultInjector,
            metadata: {
        ...
        } else if (dryRun !== true && existing?.memory_id) {
          upsertCurrentMemory(db, {
        ...
        if (dryRun !== true) {
          ...
          snippets.forEach((item, index) => {
            ...
            if (typeof faultInjector === 'function') faultInjector('after_evidence');
    ...
    if (dryRun === true) {
      ...
      withProjectionMutationBatch({ db, operationId: runId }, (tx) => {
        apply(tx);
        recordSyncRun(db, {
          run_id: runId,
          source_host: normalizedSourceHost,
        ...
        if (typeof faultInjector === 'function') faultInjector('after_sync_run');
      });
    }
