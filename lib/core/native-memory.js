...
const appendCheckpointSections = ({
  ...
  for (const [sectionHeading, entries] of sections) {
    ...
    for (const entry of entries || []) {
      ...
      seen.add(key);
      bullets.push(`- ${appendScopeComment(text, scope)}`);
    }
    if (bullets.length > 0) pending.push([sectionHeading, bullets]);
  }
  if (pending.length === 0) {
    return {
      written: false,
...
const writeNativeSessionCheckpoint = ({
  ...
  const sections = [
    ...
    [sectionTitles.decisions, normalizeCheckpointItems(decisions).map((item) => prefixCheckpointItem('Decision', item))],
    [sectionTitles.open_loops, normalizeCheckpointItems(openLoops).map((item) => prefixCheckpointItem('Open loop', item))],
    [sectionTitles.touched_files, normalizeCheckpointItems(touchedFiles).map((item) => prefixCheckpointItem('Touched file', item))],
    [sectionTitles.durable_candidates, normalizeCheckpointItems(durableCandidates)],
  ];
