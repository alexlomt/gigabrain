...
import { appendGigabrainMetadata, parseGigabrainMetadata } from './native-metadata.js';
...
const resolveNativeMemoryLockDir = (config = {}) => {
  const explicit = String(config?.native?.lockDir || process.env.GIGABRAIN_NATIVE_MEMORY_LOCK_DIR || '').trim();
  if (explicit) return path.resolve(explicit);
  const outputDir = String(config?.runtime?.paths?.outputDir || '').trim();
  if (outputDir) return path.join(path.dirname(path.resolve(outputDir)), 'native-memory.lockdir');
  const workspaceRoot = String(config?.runtime?.paths?.workspaceRoot || '').trim();
  if (workspaceRoot) return path.join(path.resolve(workspaceRoot), '.gigabrain-native-memory.lockdir');
  return '';
};

const acquireNativeMemoryLock = (lockDir, {
  timeoutMs = DEFAULT_LOCK_TIMEOUT_MS,
  staleMs = DEFAULT_LOCK_STALE_MS,
} = {}) => {
  const resolved = String(lockDir || '').trim();
  if (!resolved) {
    return () => {};
  }
    ...
    try {
      ...
      return () => {
        try {
          ...
          // Best-effort cleanup; stale locks are handled by the next acquirer.
        }
      };
    } catch (err) {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`timed out waiting for native memory lock: ${resolved}`);
      }
      sleepSync(100);
    }
  }
...
const withNativeMemoryLock = (lockDir, fn) => {
  const release = acquireNativeMemoryLock(lockDir);
  try {
    return fn();
  } finally {
    release();
...
const normalizeBulletContent = (line) => {
  ...
  const { scope, text } = parseGigabrainMetadata(match?.[1] || '');
...
const normalizeSectionHeading = (value = '') => String(value || '').trim().toLowerCase();

const findTrailingSectionHeading = (lines = []) => {
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    const text = String(lines[index] || '').trim();
    if (!text) continue;
    const match = text.match(SECTION_HEADING_RE);
    if (match?.[1]) {
      return {
...
const appendSectionAndBullet = ({ filePath, title, sectionHeading, bulletLine, content, scope = '', lockDir = '' }, allowedRoot = '') => {
  ...
  return withNativeMemoryLock(lockDir, () => {
    ...
    if (lines.length === 0) {
      ...
      const trailingSection = findTrailingSectionHeading(lines);
      if (normalizeSectionHeading(trailingSection?.heading) !== normalizeSectionHeading(sectionHeading)) {
...
const writeCheckpointSection = ({
  ...
  for (const entry of entries) {
    ...
    results.push(appendSectionAndBullet({
      ...
      title,
      sectionHeading,
      bulletLine: `- ${appendGigabrainMetadata(text, { scope })}`,
      content: text,
      scope,
      lockDir,
    }, allowedRoot));
...
const writeNativeSessionCheckpoint = ({
  ...
  const lockDir = resolveNativeMemoryLockDir(config);
...
const writeNativeMemoryEntry = ({
  ...
  const result = appendSectionAndBullet({
    ...
    title,
    sectionHeading,
    bulletLine: `- ${appendGigabrainMetadata(bulletContent, { scope, type })}`,
    content: note,
    scope,
    lockDir,
  }, allowedRoot);
