...
const appendSectionAndBullet = ({ filePath, title, sectionHeading, bulletLine, content, scope = '' }, allowedRoot = '') => {
  ...
  // This rewrites the ENTIRE source-of-truth native
  // memory file — a crash mid-write truncated everything. Atomic temp+rename
  // makes truncation impossible (concurrent last-writer-wins on a single
  // bullet remains possible and is the accepted tradeoff for now).
  const tempPath = `${filePath}.tmp-${process.pid}-${Date.now()}`;
  fs.writeFileSync(tempPath, `${merged.endsWith('\n') ? merged : `${merged}\n`}`, 'utf8');
