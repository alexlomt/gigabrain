...
import { listCurrentMemories, updateCurrentStatus } from './projection-store.js';
...
const isPendingRow = (row = {}) => String(row.status || 'pending').trim().toLowerCase() === 'pending';
...
const reviewQueueRowWithLlm = async ({ row, config, winner, loser }) => {
  ...
  if (provider === 'none') return { ok: false, error: 'llm_disabled' };
  ...
  let raw = '';
  if (provider === 'openclaw' || provider === 'openai_compatible') {
  ...
  for (const index of targetIndexes) {
    ...
    if (!reviewed?.ok || !reviewed?.parsed) {
      ...
      summary.items.push({
        reasonCode,
        status: 'error',
        error: reviewed?.error || 'review_failed',
      });
      continue;
    }
    summary.reviewed += 1;
