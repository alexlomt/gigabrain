...
// (export-only) Memory Audit report. (Internal wire identifiers below keep the
// historical "passport" name for export/import back-compat.)
//
// Design:
...
const importPassportBundle = ({
  ...
  const resolvedOperationId = String(
    operationId || runId || `handoff-import-${recomputed.slice(0, 24)}`,
  );
  withProjectionMutationBatch({ db, operationId: resolvedOperationId }, (tx) => {
    for (const memory of bundle.memories) {
      ...
      if (tx.lastMutationEvent) result.imported_events += 1;
    }
