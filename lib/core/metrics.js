...
const countBy = (db, field, options = {}) => {
...
const captureSnapshotMetrics = (db, dbPath, options = {}) => {
  if (options.ensure !== false) ensureProjectionStore(db);
  const status = countBy(db, 'status', options);
  const scope = countBy(db, 'scope', options);
  const type = countBy(db, 'type', options);
