...
import { captureFromEvent, detectMemoryFlushTurn } from './capture-service.js';
...
const AUTO_CAPTURE_PROCESSING_STALE_MS = 3 * 60 * 1000;
...
const TOOLISH_TEXT_PATTERNS = [
  ...
  /\bOpenClaw runtime context for this turn\b/i,
  /\bCurrent user request:\b/i,
  /\bDelivery: to send a message\b/i,
  /\bNORMAL\s*\{/i,
  /\bTOOL_FILTER\s*\{/i,
...
const AUTO_CAPTURE_SKIP_PATTERNS = [
  ...
  /\b(?:smoke tests?|live tests?|unit tests?|targeted tests?|restart(?:ing)?|gateway|RAM|CPU|ps aux|free -h|journalctl|systemctl)\b/i,
  /\b(?:tool|synthetic|test fixture|mock|debug|log tail|trace|stack|execution summary|progress update|status update|diagnostics?|live worker check)\b/i,
];
...
const shouldConsiderAutoCaptureEvent = ({ event = {}, config = {} } = {}) => {
  ...
  if (detectMemoryFlushTurn(event)) {
    return { ok: false, reason: 'memory_flush_turn', syntheticWakeDetected: false };
  }
...
const TERMINAL_AUTO_CAPTURE_STATUSES = new Set(['completed', 'failed_terminal', 'resolved_historical']);

const summarizeAutoCapturePacket = (packet = {}) => ({
  scope: String(packet?.scope || '').trim(),
  agent_id: String(packet?.agent_id || '').trim(),
  session_key: String(packet?.session_key || '').trim(),
  conversation_turns: Array.isArray(packet?.conversation) ? packet.conversation.length : 0,
  existing_memories_count: Array.isArray(packet?.existing_memories) ? packet.existing_memories.length : 0,
  packet_scrubbed: true,
});

const scrubTerminalAutoCaptureRow = (row = {}) => {
  const status = String(row?.status || '');
  if (!TERMINAL_AUTO_CAPTURE_STATUSES.has(status) || !row?.packet || typeof row.packet !== 'object') {
    return row;
  }
  const { packet, ...rest } = row;
  return {
    ...rest,
    packet_summary: row.packet_summary || summarizeAutoCapturePacket(packet),
  };
};

const retainedAutoCaptureRows = (rows = [], now = Date.now()) => rows
  .filter((row) => row?.status !== 'completed' || Date.parse(String(row?.processed_at || '')) > now - 24 * 60 * 60 * 1000)
  .map((row) => scrubTerminalAutoCaptureRow(row))
  .slice(-AUTO_CAPTURE_QUEUE_MAX_ROWS);

const applyAutoCaptureQueueRetention = ({ queuePath, now = Date.now() } = {}) => {
  const rows = readAutoCaptureQueue(queuePath);
  const retained = retainedAutoCaptureRows(rows, now);
  writeAutoCaptureQueue(queuePath, retained);
  return {
    rows_before: rows.length,
    rows_after: retained.length,
    packets_scrubbed: rows.filter((row) => (
      TERMINAL_AUTO_CAPTURE_STATUSES.has(String(row?.status || ''))
      && row?.packet
      && typeof row.packet === 'object'
    )).length,
  };
};

const resolveAutoCaptureProcessingStaleMs = (config = {}) => {
  const autoConfig = config?.capture?.autoCapture || {};
  return clampInt(
    autoConfig.processingStaleMs ?? config?.runtime?.autoCaptureProcessingStaleMs,
    60 * 1000,
    60 * 60 * 1000,
    AUTO_CAPTURE_PROCESSING_STALE_MS,
  );
};

const recoverStaleAutoCaptureProcessingRows = ({ rows = [], config = {}, nowMs = Date.now() } = {}) => {
  const staleMs = resolveAutoCaptureProcessingStaleMs(config);
  const nowIso = new Date(nowMs).toISOString();
  const recovered = [];
  for (const row of rows) {
    if (String(row?.status || '') !== 'processing') continue;
    const updatedAtMs = Date.parse(String(row?.processing_started_at || row?.updated_at || row?.enqueued_at || ''));
    if (Number.isFinite(updatedAtMs) && nowMs - updatedAtMs < staleMs) continue;
    const attempts = Number(row?.attempts || 0);
    const retryable = attempts < AUTO_CAPTURE_QUEUE_MAX_ATTEMPTS;
    row.status = retryable ? 'failed_retryable' : 'failed_terminal';
    row.updated_at = nowIso;
    row.error_class = 'timeout_or_aborted';
    row.error_message = 'stale_processing_recovered';
    row.error = 'stale_processing_recovered';
    row.next_attempt_at = retryable ? nextRetryAt(attempts, nowMs) : '';
    row.processing_started_at = '';
    recovered.push(String(row?.id || row?.hash || ''));
  }
  return {
    count: recovered.length,
    ids: recovered.filter(Boolean),
  };
};
...
const processAutoCaptureQueue = async ({
  ...
  const nowIso = new Date(nowMs).toISOString();
  const recovery = recoverStaleAutoCaptureProcessingRows({ rows, config, nowMs });
  if (recovery.count > 0) {
    writeAutoCaptureQueue(resolvedQueuePath, retainedAutoCaptureRows(rows, nowMs));
  }
  ...
  let queuedReview = 0;
  const results = [];
  const persistQueue = () => writeAutoCaptureQueue(resolvedQueuePath, retainedAutoCaptureRows(rows));
  ...
  if (circuit.open) {
    return {
      ...
      autoSaved,
      queuedReview,
      processingRecovered: recovery.count,
      circuit_open: true,
  ...
  for (const row of rows) {
    ...
    if (!row?.packet || typeof row.packet !== 'object') {
      ...
      failed += 1;
      results.push({ id: row.id, status: row.status, error: row.error_message });
      persistQueue();
      continue;
    }
    processed += 1;
    ...
    row.updated_at = attemptStartedAt;
    row.processing_started_at = attemptStartedAt;
