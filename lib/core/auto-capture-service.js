...
const shouldConsiderAutoCaptureEvent = ({ event = {}, config = {} } = {}) => {
  ...
  const turns = buildTurns(event, { ...autoConfig, maxTurns: Math.min(Number(autoConfig.maxTurns || 16), 8) });
  if (turns.length === 0) return { ok: false, reason: 'no_user_visible_turns' };
  const text = turns.map((turn) => `${turn.role}: ${turn.content}`).join('\n').trim();
  if (text.length < Math.max(60, Number(autoConfig.minTriggerChars || 80))) return { ok: false, reason: 'too_short' };
  if (containsSecretLikeValue(text)) return { ok: false, reason: 'secret_like' };
  if (isToolLikeText(text)) return { ok: false, reason: 'tool_like' };
  const lastUser = [...turns].reverse().find((turn) => turn.role === 'user')?.content || '';
  if (!lastUser) return { ok: false, reason: 'no_user_message' };
  if (shouldSkipAutoCaptureText(lastUser) || shouldSkipAutoCaptureText(text)) return { ok: false, reason: 'skip_pattern' };
  if (!hasAutoCaptureSignal(lastUser) && !hasAutoCaptureSignal(text)) return { ok: false, reason: 'no_memory_signal' };
  return { ok: true, reason: 'memory_signal' };
};

const buildTurns = (event = {}, options = {}) => {
  ...
  for (const message of messages) {
    ...
    const rawText = valueToText(message?.content ?? message?.text ?? message?.output);
...
const enqueueAutoCaptureEvent = ({
  ...
  if (!gate.ok) {
    ...
    summary.decisions.push({ route: 'skip', reason: gate.reason });
    return summary;
  }
  try {
    const scope = safeScope(event.scope || event.agentId, 'shared');
...
const buildAutoCapturePrompt = (packet) => [
  ...
  '{"candidates":[{"action":"auto_save|queue_review|reject","type":"PREFERENCE|DECISION|USER_FACT|ENTITY|EPISODE|AGENT_IDENTITY|CONTEXT","content":"single atomic memory, written as a durable fact","scope":"profile:main|shared|other provided scope","confidence":0.0,"importance":0.0,"sensitivity":"low|medium|high","reason":"short reason"}]}',
  '',
  'Decision rules:',
  ...
  '- Do not save credentials, secret values, raw auth headers, private keys, cookies, passwords, or sensitive security posture.',
...
const classifyCandidate = (candidate, config = {}, fallbackScope = 'shared') => {
  ...
  if (!content || content.length < Math.max(10, Number(autoConfig.minContentChars || 20))) {
    return { route: 'reject', candidate: { ...candidate, scope }, reason: 'too_short' };
  }
  if (containsSecretLikeValue(content) || candidate.sensitivity === 'high') {
    return { route: 'reject', candidate: { ...candidate, scope }, reason: 'sensitive' };
  }
  if (candidate.action === 'auto_save'
    ...
    return { route: 'auto_save', candidate: { ...candidate, scope }, reason: 'passed_thresholds' };
  }
  if ((candidate.action === 'queue_review' || candidate.action === 'auto_save')
    ...
    return { route: 'queue_review', candidate: { ...candidate, scope }, reason: 'review_thresholds' };
  }
  return { route: 'reject', candidate: { ...candidate, scope }, reason: 'below_thresholds' };
};
...
const processAutoCapturePacket = async ({
  ...
  try {
    ...
    const candidates = parseAutoCaptureDecision(raw).slice(0, maxCandidates);
    summary.candidates = candidates.length;

    const saveNotes = [];
    for (const candidate of candidates) {
      const classified = classifyCandidate(candidate, config, scope);
