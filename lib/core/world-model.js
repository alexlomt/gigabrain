...
const inferEntityKind = (entityKey, memoryRows = [], mentionRows = [], config = {}) => {
  ...
  const hasAliasProfileCue = memoryRows.some((row) => {
    const content = String(row.content || '');
    return hasAliasCueWithinWindow(content, entityKey, new Set(['founder', 'ceo', 'role', 'job', 'title', 'based', 'lives', 'prefers', 'likes', 'london']), 6)
...
const evaluateEntityCandidate = ({
  ...
  if (kind === 'person') {
    ...
    const hasAliasProfileCue = relatedMemoryRows.some((row) => filteredAliases.some((alias) => (
      hasAliasCueWithinWindow(String(row.content || ''), alias, new Set(['founder', 'ceo', 'role', 'job', 'title', 'based', 'lives', 'prefers', 'likes', 'london']), 6)
...
const rebuildWorldModel = ({
  ...
  if (briefingEnabled) {
    // Scope-partitioned session brief (governance): both belief sources — the
    // recent session-relevant leaders AND the entity fillers — are restricted to
    // brief-eligible (shared / profile:*) scopes via isBriefEligibleScope, so a
    // project-scoped fact never rides the always-on brief into another workspace.
    // Composition (recent beliefs first, then one-person/one-project fillers) is
    // otherwise unchanged.
    const briefEligibleBeliefsForEntity = (entityId) => (
      (beliefsByEntity.get(entityId) || []).filter(isBriefEligibleScope)
    );
    synthesisRows.push({
      ...
      subject_id: 'global',
      content: (() => {
        // Recent session-relevant beliefs lead the brief: fresh decisions and
        // runbooks are worth more at session start than static entity summaries
        // (the old 2-people + 2-projects reservation surfaced the same
        // relationship/pronoun trivia every session while recent work got 2 slots).
        ...
        const sessionBeliefs = selectDistinctItems(
          recentBeliefs.filter((belief) => isSessionRelevantBelief(belief) && isBriefEligibleScope(belief)),
          (belief) => belief.payload?.claim_slot || belief.content,
          4,
        );
        const lines = [];
        ...
        for (const entity of [...sessionPeople, ...sessionProjects]) {
          const summaryBelief = pickSurfaceSummaryBelief(entity, briefEligibleBeliefsForEntity(entity.entity_id));
      ...
      input_hash: hashNormalized(JSON.stringify([
        ...
        ...recentBeliefs.filter(isBriefEligibleScope).slice(0, 5).map((row) => row.belief_id),
      ])),
