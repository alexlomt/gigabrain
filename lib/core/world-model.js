...
const ENTITY_ALIAS_STOPWORDS = new Set([
  ...
  'warm', 'wichtige', 'wrong', 'vienna', 'wien', 'brigittaplatz', 'operator', 'creator', 'evidence', 'public', 'research',
...
const NON_PERSON_ALIAS_HINTS = new Set(['paperclip', 'telegram', 'discord', 'gigabrain', 'hindsight', 'openclaw', 'gateway', 'scrapling', 'higgsfield']);

const isLikelyPersonAlias = (value = '') => {
  const normalized = normalizeContent(value);
  if (!normalized || isGenericAlias(normalized)) return false;
  if (NON_PERSON_ALIAS_HINTS.has(normalized)) return false;
...
const extractRoleValue = (content = '') => {
  const text = String(content || '').trim();
  const direct = extractTrailingValue(
    text,
    /\b(?:works? as|arbeitet als|role|job title|title|serves as|ist ein|ist eine|is an?|was an?)\s+([A-Za-zÀ-ÖØ-öø-ÿ][A-Za-zÀ-ÖØ-öø-ÿ0-9'’._ -]{1,60})/i,
  );
  if (direct) return direct;
  const explicit = text.match(/\b(?:is|was|serves as)\s+(founder|ceo|cto|cfo|coo|advisor|berater(?:in)?|operator|researcher|designer|engineer)\b/i);
  return normalizeContent(String(explicit?.[1] || '').trim());
};
...
const queryEntityMentions = (db) => {
  ...
  return db.prepare(`
    SELECT id, memory_id, entity_key, entity_display, role, confidence, source, scope, source_path, linked_memory_id
...
const inferEntityKind = (entityKey, memoryRows = [], mentionRows = [], config = {}) => {
  ...
  const workspaceRoot = String(config?.runtime?.paths?.workspaceRoot || '').trim();
  const mentionRoles = new Set(mentionRows.map((row) => String(row.role || '').toLowerCase()));
  const hasRelationshipMention = mentionRoles.has('relationship');
  const hasPublicProfileMention = mentionRoles.has('public_profile');
  const hasOperationalMention = mentionRoles.has('ops_noise');
  ...
  const hasAliasProfileCue = memoryRows.some((row) => {
    ...
    return hasAliasCueWithinWindow(content, entityKey, new Set(['partner', 'wife', 'husband', 'girlfriend', 'boyfriend', 'friend', 'founder', 'ceo', 'role', 'job', 'title', 'based', 'lives', 'prefers', 'likes', 'community', 'birthday', 'london', 'vienna', 'wien']), 6)
  ...
  const hasPersonalProfileEvidence = hasPersonalScopeEvidence
    && hasPersonalMemoryType
    && !hasOrganizationCue
    && !hasProjectCue
    && !hasOperationalMention
    && !hasStrongProjectEvidence;
  const hasStrongPersonalIdentityEvidence = hasRelationshipMention
    || hasPublicProfileMention
    || hasAliasProfileCue
    || hasPlaceCue
    || (isDeclaredWorkspaceIdentity && hasPersonalScopeEvidence && hasPersonalMemoryType)
    || hasPersonalProfileEvidence;
  if (isLikelyPersonAlias(entityKey) && isEntityKindEnabled(config, 'person')) {
    ...
    if (hasAgentIdentityEvidence && hasStrongPersonalIdentityEvidence) return 'person';
    if (hasAliasProfileCue && !hasOrganizationCue && !hasProjectCue && !hasOperationalMention) return 'person';
    if (isDeclaredWorkspaceIdentity && hasPersonalScopeEvidence && hasPersonalMemoryType) return 'person';
    if (hasPersonalProfileEvidence) return 'person';
  }
...
const evaluateEntityCandidate = ({
  ...
  if (kind === 'person') {
    ...
    const hasAliasProfileCue = relatedMemoryRows.some((row) => filteredAliases.some((alias) => (
      hasAliasCueWithinWindow(String(row.content || ''), alias, new Set(['partner', 'wife', 'husband', 'girlfriend', 'boyfriend', 'friend', 'founder', 'ceo', 'role', 'job', 'title', 'based', 'lives', 'prefers', 'likes', 'community', 'birthday', 'london', 'vienna', 'wien']), 6)
      || hasPlacePatternForAlias(String(row.content || ''), alias)
    )));
    const hasOrganizationCue = relatedMemoryRows.some((row) => filteredAliases.some((alias) => hasAliasCueWithinWindow(row.content, alias, ORG_CUE_TOKENS, 5)));
    const hasProjectCue = relatedMemoryRows.some((row) => filteredAliases.some((alias) => hasAliasCueWithinWindow(row.content, alias, PROJECT_CUE_TOKENS, 5)));
    const hasOperationalMention = mentionRows.some((row) => String(row.role || '').toLowerCase() === 'ops_noise');
    const hasPersonalScopeEvidence = relatedMemoryRows.some((row) => {
      const scope = normalizeScope(row.scope);
      return scope === 'main' || scope.startsWith('profile:');
    });
    const hasPersonalMemoryType = relatedMemoryRows.some((row) => ['AGENT_IDENTITY', 'USER_FACT', 'PREFERENCE', 'DECISION'].includes(String(row.type || '').trim().toUpperCase()));
    const hasPersonalProfileEvidence = hasPersonalScopeEvidence && hasPersonalMemoryType && !hasOrganizationCue && !hasProjectCue && !hasOperationalMention;
    ...
    if ((hasOrganizationCue || hasProjectCue || hasOperationalMention)
      && !declaredWorkspaceIdentity
      && !strongMention
      && !hasAliasProfileCue) {
      return { accepted: false, kind, aliases: filteredAliases, reason: 'non_person_alias_bias' };
    }
    if (!declaredWorkspaceIdentity && !strongMention && !hasAliasProfileCue && !(hasAgentIdentityEvidence && hasAliasProfileCue) && !hasPersonalProfileEvidence) {
...
const inferBeliefType = (row = {}) => {
  ...
  if (extractRoleValue(content)) return 'role';
...
const rebuildWorldModel = ({
    ...
    const entityRow = {
      ...
      payload: {
        ...
        scopes: Array.from(new Set([
          ...
          ...entityMentions.map((row) => row.scope),
        ].map((value) => normalizeScope(value)).filter(Boolean))),
  ...
  for (const group of consolidatedBeliefs.conflictGroups) {
    ...
    const entityKind = String(entityId.split(':')[0] || '').trim();
    if ((group.topic === 'role' || group.subtopic === 'primary_role') && entityKind !== 'person') {
      continue;
    }
  ...
  synthesisRows.push({
    ...
    subject_id: 'global',
    content: (() => {
      const sessionPeople = importantPeople.filter((entity) => shouldExposeEntitySyntheses(entity.payload, 'shared')).slice(0, 2);
      const sessionProjects = importantProjects.filter((entity) => shouldExposeEntitySyntheses(entity.payload, 'shared')).slice(0, 2);
      const usedSummaryKeys = new Set();
      const usedSummarySlots = new Set();
      const sessionBeliefs = selectDistinctItems(
        recentBeliefs.filter((belief) => isSessionRelevantBelief(belief) && normalizeScope(belief?.payload?.scope) === 'shared'),
        (belief) => belief.payload?.claim_slot || belief.content,
        2,
      );
      const lines = [];
      for (const entity of sessionPeople) {
        const summaryBelief = pickSurfaceSummaryBelief(
          entity,
          (beliefsByEntity.get(entity.entity_id) || []).filter((belief) => normalizeScope(belief?.payload?.scope) === 'shared'),
        );
