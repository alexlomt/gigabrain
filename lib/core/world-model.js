...
// U17: the arbiter (position clustering + trust>corroboration>recency rule +
// recordVerdict orchestration) lives in belief-arbitration.js. This module
// keeps belief PROJECTION (memory rows → belief rows with claim slots) and the
// world-model surfaces; the rebuild is one arbitration caller among others.
import {
  beliefPriorityScore,
...
// Token set mirroring RELATIONSHIP_RE for the windowed person-kind check.
const RELATIONSHIP_CUE_TOKENS = new Set(['partner', 'partnerin', 'wife', 'husband', 'girlfriend', 'boyfriend', 'friend', 'freund', 'freundin', 'relationship', 'beziehung', 'dating']);
...
const PLACE_RE = /\b(?:city|country|town|village|office|home|berlin|london|paris)\b/i;
...
const SURFACE_BELIEF_NOISE_RE = /\b(?:access_token|cookies?|remote-debugging-port|headless|base64|skill\.md|\.json|chat_id|numeric chat id|send login code|verify code|chrome cdp|api calls needed|tool ignores voiceid|unread emails?|heartbeat|book a call)\b/i;
...
const SESSION_BRIEF_NOISE_RE = /\b(?:tts|voice preset|voice reference|default engine|profile image|saved to avatars|chat id|username|api calls needed|tool ignores|heartbeat|unread emails?|book a call|send login code|verify code|cookies?|remote debugging|chrome cdp|visual identity|voice identity|memory-?notes?)\b/i;
const SURFACE_PERSON_PREFERRED_RE = /\b(?:partner|partnerin|relationship|beziehung|birthday|geburtstag|prefers|bevorzugt|works? (?:in|as)|arbeitet als|focus(?:es)? on|community)\b/i;
const SURFACE_PROJECT_PREFERRED_RE = /\b(?:investor|investment|neobank|feature|integration|analysis|analyz|privacy|tts|voice|recall|respond|stores and logs|default engine)\b/i;
const PERSON_SURFACE_CUE_TOKENS = new Set(['partner', 'partnerin', 'relationship', 'beziehung', 'works', 'arbeitet', 'coach', 'community', 'lives', 'lebt', 'birthday', 'geburtstag', 'prefers', 'focuses', 'active', 'dates']);
...
// U19: the ops-runbook detector keeps only GENERIC security/credential/transport
// vocabulary. Deployment-specific runbook phrasing (a particular host's gateway,
// launch-agent, messaging RPC, browser-debug or login-code wording) is NOT baked
// in — that overfits the engine to one operator's stack. Operators supply those
// via config.worldModel.customSlotRules (slot: "ops…"); see
// docs/world-model-custom-slots.md for a copy-paste example. The
// OWNER_DEPLOYMENT_OPS_TOKENS list below MUST stay empty in the shipped default;
// the private engineering release suite rejects operator-specific literals in
// this file before a public mirror can be built.
const OPS_RUNBOOK_RE = /\b(?:access[_ ]?token|cookies?|remote-debugging-port|headless|base64|api calls needed|heartbeat|token refresh|refresh token|webhook|oauth)\b/i;
const OWNER_DEPLOYMENT_OPS_TOKENS = Object.freeze([]);
...
const PERSONAL_MEMORY_RE = /\b(?:partner(?:in)?|wife|husband|girlfriend|boyfriend|birthday|geburtstag|prefers?|bevorzugt|likes?|loves?|dislikes?|hates?|favorite author|communication style|tone|works? as|arbeitet als|who is|wer ist)\b/i;
...
const PROJECT_REFERENCE_RE = /\b(?:provider|model|weights?|openrouter|api|endpoint|docs?|documentation|research|search source|semantic search|vector|oauth|chat id|username|telegram|bot|gateway|restart|chrome cdp|remote debugging|cookies?|heartbeat|voice reference|voice preset|default engine|tts)\b/i;
// Two-cue gate: a fact is a telegram-contact claim only when it BOTH mentions
// telegram AND carries an identifier cue. The old single-regex version matched
// any @-token (emails, npm scopes, git handles) or the bare word "username",
// which could slot deployment runbooks under contact.telegram.primary and
// manufacture fake contradictions. Low-confidence content must abstain from
// slotting rather than guess.
const CONTACT_TELEGRAM_CONTEXT_RE = /\btelegram\b/i;
const CONTACT_TELEGRAM_ID_CUE_RE = /(?:telegram:\s*\d+|chat[_ ]?id|@[\w_]+|\busername\b)/i;
const CONTACT_INFO_RE = {
  test: (content) => CONTACT_TELEGRAM_CONTEXT_RE.test(content) && CONTACT_TELEGRAM_ID_CUE_RE.test(content),
};
// U19: HEALTH_MEMORY_RE is GENERIC category vocabulary (a weight/fitness goal in
// any language) — it carries no operator identity, so it stays in the engine and
// keeps slotting durable_personal/health on a fresh install. Operator-specific
// health phrasing (a named program, a specific target like "target: NNkg", a
// personal club/coach name) must NOT be hardcoded; it goes through
// config.worldModel.customSlotRules. OWNER_HEALTH_TOKENS MUST stay empty in the
// default; the private engineering release suite asserts that invariant before
// a public mirror can be built.
const HEALTH_MEMORY_RE = /\b(?:weight loss journey|goal weight|target weight|lose weight|abnehmen|healthier goal weight|improve fitness)\b/i;
const OWNER_HEALTH_TOKENS = Object.freeze([]);
const emptyConfiguredOperatorRules = () => ({
  entity: {
    rejectTerms: new Set(),
    nonPersonTerms: new Set(),
    rejectPatterns: [],
    nonPersonPatterns: [],
  },
  memoryTier: {
  ...
  surface: {
    ...
    personCueTerms: new Set(),
    projectCueTerms: new Set(),
  },
  sessionBrief: { excludePatterns: [] },
});
let CONFIGURED_OPERATOR_RULES = emptyConfiguredOperatorRules();

const compileProtectedPatterns = (rules = []) => (Array.isArray(rules) ? rules : [])
  .map((rule) => {
    const pattern = String(rule?.pattern || '').trim();
    if (!pattern) return null;
    try {
      return new RegExp(pattern, String(rule?.flags ?? 'i'));
    } catch {
      return null;
    }
  })
...
const normalizeProtectedTerms = (values = []) => new Set(
  (Array.isArray(values) ? values : []).map((value) => normalizeContent(value)).filter(Boolean),
);

const configureOperatorRules = (config = {}) => {
  const raw = config?.operatorRules || {};
  CONFIGURED_OPERATOR_RULES = {
    entity: {
      rejectTerms: normalizeProtectedTerms(raw?.entity?.rejectTerms),
      nonPersonTerms: normalizeProtectedTerms(raw?.entity?.nonPersonTerms),
      rejectPatterns: compileProtectedPatterns(raw?.entity?.rejectPatterns),
      nonPersonPatterns: compileProtectedPatterns(raw?.entity?.nonPersonPatterns),
    },
    memoryTier: {
      opsPatterns: compileProtectedPatterns(raw?.memoryTier?.opsPatterns),
      workingReferencePatterns: compileProtectedPatterns(raw?.memoryTier?.workingReferencePatterns),
      personalMemoryPatterns: compileProtectedPatterns(raw?.memoryTier?.personalMemoryPatterns),
      projectMemoryPatterns: compileProtectedPatterns(raw?.memoryTier?.projectMemoryPatterns),
      projectEpisodePatterns: compileProtectedPatterns(raw?.memoryTier?.projectEpisodePatterns),
      projectIdentityPatterns: compileProtectedPatterns(raw?.memoryTier?.projectIdentityPatterns),
      personalGoalPatterns: compileProtectedPatterns(raw?.memoryTier?.personalGoalPatterns),
      projectReferencePatterns: compileProtectedPatterns(raw?.memoryTier?.projectReferencePatterns),
      contactInfoPatterns: compileProtectedPatterns(raw?.memoryTier?.contactInfoPatterns),
      healthMemoryPatterns: compileProtectedPatterns(raw?.memoryTier?.healthMemoryPatterns),
    },
    surface: {
      beliefNoisePatterns: compileProtectedPatterns(raw?.surface?.beliefNoisePatterns),
      beliefMetaPatterns: compileProtectedPatterns(raw?.surface?.beliefMetaPatterns),
      summaryWeakPatterns: compileProtectedPatterns(raw?.surface?.summaryWeakPatterns),
      personPreferredPatterns: compileProtectedPatterns(raw?.surface?.personPreferredPatterns),
      projectPreferredPatterns: compileProtectedPatterns(raw?.surface?.projectPreferredPatterns),
      personCueTerms: normalizeProtectedTerms(raw?.surface?.personCueTerms),
      projectCueTerms: normalizeProtectedTerms(raw?.surface?.projectCueTerms),
    },
    sessionBrief: {
      excludePatterns: compileProtectedPatterns(raw?.sessionBrief?.excludePatterns),
    },
  };
};

const matchesProtectedPatterns = (family, name, value = '') => {
  const text = String(value || '');
  const rules = CONFIGURED_OPERATOR_RULES?.[family]?.[name] || [];
...
const protectedTermSet = (family, name, defaults = new Set()) => new Set([
  ...defaults,
  ...(CONFIGURED_OPERATOR_RULES?.[family]?.[name] || []),
]);
...
const isGenericAlias = (value = '') => {
  ...
  if (CONFIGURED_OPERATOR_RULES.entity.rejectTerms.has(normalized)) return true;
  if (matchesProtectedPatterns('entity', 'rejectPatterns', value)) return true;
  if (/^\d+$/.test(normalized)) return true;
  return false;
};

const isScopeVisibleForRequested = (rowScope = '', requestedScope = '', options = {}) => {
  ...
  const sharedVisible = options.includeShared !== false && actual === 'shared';
  if (!requested.startsWith('project:') && !requested.startsWith('profile:')) {
    return actual === requested || sharedVisible || (requested === 'main' && actual === 'profile:main');
  }
  return actual === requested || sharedVisible;
};
...
const isEntityVisibleInScope = (payload = {}, scope = '', options = {}) => {
  const requested = normalizeRequestedScope(scope);
  if (!requested) return true;
  const scopes = normalizeScopeList(payload?.scopes);
  if (scopes.length === 0) return false;
  return scopes.some((rowScope) => isScopeVisibleForRequested(rowScope, requested, options));
};

const shouldExposeEntitySyntheses = (payload = {}, scope = '', options = {}) => {
  const requested = normalizeRequestedScope(scope);
  if (!requested) return true;
  const scopes = normalizeScopeList(payload?.scopes);
  if (scopes.length === 0) return false;
  if (scopes.length > 1) return false;
  return isScopeVisibleForRequested(scopes[0], requested, options);
};
const PERSONISH_ALIAS_RE = /^[a-zà-öø-ÿ][a-zà-öø-ÿ'’.-]{1,31}(?:\s+[a-zà-öø-ÿ][a-zà-öø-ÿ'’.-]{1,31}){0,2}$/iu;
...
const isLikelyPersonAlias = (value = '') => {
  ...
  if (CONFIGURED_OPERATOR_RULES.entity.nonPersonTerms.has(normalized)) return false;
  if (matchesProtectedPatterns('entity', 'nonPersonPatterns', value)) return false;
...
const resolveMemoryTier = ({ row = {}, claimSignal = null, entityKeys = [] } = {}) => {
  ...
  const projectReferenceLike = PROJECT_REFERENCE_RE.test(content)
    || matchesProtectedPatterns('memoryTier', 'projectReferencePatterns', content);
  const projectMemoryLike = PROJECT_MEMORY_RE.test(content)
    || matchesProtectedPatterns('memoryTier', 'projectMemoryPatterns', content);
  const projectEpisodeLike = PROJECT_EPISODE_RE.test(content)
    || matchesProtectedPatterns('memoryTier', 'projectEpisodePatterns', content);
  const personalGoalLike = PERSONAL_GOAL_RE.test(content)
    || matchesProtectedPatterns('memoryTier', 'personalGoalPatterns', content);
  const healthMemoryLike = HEALTH_MEMORY_RE.test(content)
    || matchesProtectedPatterns('memoryTier', 'healthMemoryPatterns', content);
  const personalMemoryLike = PERSONAL_MEMORY_RE.test(content)
    || matchesProtectedPatterns('memoryTier', 'personalMemoryPatterns', content);

  if (!content) return 'working_reference';
  if (OPS_RUNBOOK_RE.test(content) || matchesProtectedPatterns('memoryTier', 'opsPatterns', content)) return 'ops_runbook';
  if (claimTopic === 'ops') return 'ops_runbook';
  if (claimTopic === 'contact' || matchesProtectedPatterns('memoryTier', 'contactInfoPatterns', content)) return 'working_reference';
  ...
  if (PROJECT_IDENTITY_RE.test(content) || matchesProtectedPatterns('memoryTier', 'projectIdentityPatterns', content)) return 'durable_project';
  ...
  if (memoryType === 'PREFERENCE') {
    ...
    if (claimTopic === 'project' || (projectMemoryLike && !projectReferenceLike)) return 'durable_project';
    return 'durable_personal';
  }
  if (memoryType === 'USER_FACT' && (personalGoalLike || healthMemoryLike)) return 'durable_personal';
  ...
  if (memoryType === 'CONTEXT' && projectEpisodeLike && !projectReferenceLike) return 'durable_project';
  if (memoryType === 'EPISODE' && (projectMemoryLike || projectEpisodeLike) && !projectReferenceLike) return 'durable_project';
  if (projectMemoryLike || entityHints.some((hint) => PROJECT_MEMORY_RE.test(hint))) {
  ...
  if (fromDurableNative) {
    ...
    if (claimTopic === 'relationship' || claimTopic === 'health' || personalMemoryLike) return 'durable_personal';
    return 'working_reference';
  }
  if (WORKING_REFERENCE_RE.test(content) || matchesProtectedPatterns('memoryTier', 'workingReferencePatterns', content)) return 'working_reference';
...
const isOperationalSurfaceBelief = (belief = {}) => {
  const content = String(belief.content || '');
  return SURFACE_BELIEF_NOISE_RE.test(content)
    || matchesProtectedPatterns('surface', 'beliefNoisePatterns', content);
};
const isMetaSurfaceBelief = (belief = {}) => {
  const content = String(belief.content || '').trim();
  return SURFACE_BELIEF_META_RE.test(content)
    || matchesProtectedPatterns('surface', 'beliefMetaPatterns', content);
};
...
const isDisplaySurfaceEpisode = (episode = {}) => {
  ...
  return !SURFACE_BELIEF_NOISE_RE.test(text)
    && !SURFACE_BELIEF_META_RE.test(text)
    && !matchesProtectedPatterns('surface', 'beliefNoisePatterns', text)
    && !matchesProtectedPatterns('surface', 'beliefMetaPatterns', text);
};
...
const isDisplaySurfaceBelief = (belief = {}, entity = {}) => {
  ...
  if (['project', 'organization'].includes(String(entity.kind || '')) && /\b(?:@username|message tool)\b/i.test(content)) return false;
  return true;
};
...
const surfaceBeliefDirectnessScore = (belief = {}, entity = {}) => {
  ...
  const cueTokens = String(entity.kind || '') === 'person'
    ? protectedTermSet('surface', 'personCueTerms', PERSON_SURFACE_CUE_TOKENS)
    : protectedTermSet('surface', 'projectCueTerms', PROJECT_SURFACE_CUE_TOKENS);
  let best = 0;
  for (const alias of aliases) {
...
const isWeakSurfaceSummaryBelief = (belief = {}, entity = {}) => {
  ...
  if (matchesProtectedPatterns('surface', 'summaryWeakPatterns', content)) return true;
...
const isSessionRelevantBelief = (belief = {}) => {
  ...
  if (matchesProtectedPatterns('sessionBrief', 'excludePatterns', content)) return false;
...
// The session brief is injected at the START of every
// workspace/session, so it may carry only CROSS-workspace facts. A project:*
// belief is scoped to one project — injecting it into every other workspace's
// session start is a cross-scope leak, not a ranking nicety. Bare-agent scopes
// (e.g. 'example-agent') are likewise workspace-local. Only 'shared' and personal
// 'profile:*' beliefs are eligible for the always-on brief. The full recall
// path keeps its own (project-visible) scope rules; this gate is the session
// brief's alone.
const isBriefEligibleScope = (belief = {}) => {
  const scope = String(belief?.payload?.scope || '').trim();
  return scope === 'shared' || scope.startsWith('profile:');
};
...
const surfaceBeliefDisplayScore = (belief = {}, entity = {}) => {
  ...
  if (String(entity.kind || '') === 'person') {
    ...
    if (SURFACE_PERSON_PREFERRED_RE.test(content)
      || matchesProtectedPatterns('surface', 'personPreferredPatterns', content)) score += 0.8;
    ...
    if (SURFACE_PROJECT_PREFERRED_RE.test(content)
      || matchesProtectedPatterns('surface', 'projectPreferredPatterns', content)) score += 0.6;
...
// --- U13b: entity-less user-fact belief projection -----------------------------
// Rows that mention no recognized entity never became beliefs, so implicit
// rivals about the USER ("drives a car to the office" vs "sold the car, rides
// the light-rail") were invisible to the arbiter (the U7 0/30 root cause).
// They are projected onto a synthetic, arbitration-only entity. Slot precision
// is the safety property: the generic value-typed detectors run first; failing
// those, a NARROW mode-level topic lexicon assigns a life-domain slot. No slot
// → no belief (prior behavior). The synthetic entity is never written to
// memory_entities, so entity surfaces and briefs are untouched; verdicts and
// supersession ride the existing consolidation + recordVerdict path (U12
// stamps included).
const USER_SELF_ENTITY_ID = 'user:self';
// Topic-lexicon fallback only applies to explicitly user-typed facts; noisy
// CONTEXT/ops rows must not acquire life-domain slots.
const USER_FACT_TOPIC_TYPES = new Set(['USER_FACT', 'PREFERENCE', 'DECISION']);
// Mode-level lexicons on purpose: topic words like "commute"/"morning"/"walk"
// would sweep complementary co-facts into one slot and supersede them. Only
// words naming the EXCLUSIVE state of the domain belong here.
// Coverage expansion (arbitration coverage, post-U13c): the canonical
// personal-fact domains of a cross-agent memory product, derived from the
// product question "what durable, exclusive-state facts does a personal
// memory store hold about its user?" — NOT from any eval fixture's wording.
// Same discipline as the original four rules: every alternative names a
// DOMAIN NOUN (employer, landlord, prescription, mortgage …), never a bare
// verb, so unrelated facts cannot acquire a life-domain slot. ORDER IS THE
// SECOND SAFETY PROPERTY (first match wins): noun-demanding domains run
// BEFORE the legacy bare-verb rules — exercise's `runs?/running` otherwise
// steals "manages a team and RUNS their one-on-ones" (employment) and a
// "Pixel phone RUNNING Android" (device) — and hobby, the co-fact-heaviest
// domain, runs last. Financial obligations get per-obligation slots
// (mortgage / auto loan / student loan / banking) because each is its own
// exclusive state: one broad "finance" domain would group a car-loan payoff
// against an unrelated mortgage co-fact and supersede the wrong rival.
const USER_FACT_TOPIC_RULES = [
  {
    slot: 'user.topic.transport_mode',
    subtopic: 'transport_mode',
    // Lookbehinds: "Google Drive"/"iCloud Drive" are product names, not
    // commuting; host stores commonly contain both.
    re: /\b(?:(?<!google )(?<!icloud )drives?|driving|drove|(?:own|their|the|a) car|light[- ]rail|commuter (?:rail|train)|takes? the (?:bus|train|subway|metro|tram)|(?:bus|rail|transit) pass|bik(?:es?|ing) to work|carpool(?:s|ing)?)\b/i,
  },
  {
    slot: 'user.topic.work_location_mode',
    subtopic: 'work_location_mode',
    re: /\b(?:fully remote|works? remote(?:ly)?|home office|work(?:s|ing)? from home|on[- ]site|in[- ]office|hybrid schedule|headquarters|badg(?:e|ing) in(?:to)?|return to office)\b/i,
  },
  {
    slot: 'user.topic.residence',
    subtopic: 'residence',
    // Dwelling type / tenure only. Deliberately NO "lives in"/"based in":
    // city-valued facts must keep flowing to the value-typed
    // location.current_city slot (full arbitration), not a cue-gated domain.
    // Prospective shopping language ("house-hunting", "toured listings") is
    // excluded on purpose — searching is not a dwelling state.
    re: /\b(?:apartment|condo|townhouse|duplex|landlord|roommates?|housemates?|(?:signed|renewed) (?:a|the) lease|lease (?:on|renewal)|rent(?:s|ed|ing) (?:a |an |the )?(?:house|home|room|place|flat|studio)|(?:bought|buying|own(?:s|ed)?) (?:a |an |the |their )?(?:house|home|flat)|homeowner|closed on (?:a|an|the)|mov(?:ed|ing) (?:back )?in with|new place)\b/i,
  },
  {
    slot: 'user.topic.employment',
    subtopic: 'employment',
    // After residence: "accepted a relocation package from their EMPLOYER and
    // settled into an APARTMENT" is a residence change that mentions a job.
    re: /\b(?:employer|new job|job (?:at|with|as|offer|title)|day job|works? (?:at|for|as)|working (?:at|for|as)|hired (?:at|by)|got hired|promot(?:ed|ion) to|self[- ]employed|freelanc(?:es?|ing|er)|salaried|on (?:the )?(?:company )?payroll|full[- ]time (?:at|job|role)|part[- ]time (?:at|job|role)|new role|position (?:at|as)|offer letter|laid off|resign(?:ed|ing)|unemployed|between jobs|manages (?:a|the) team|individual[- ]contributor|lateral move)\b/i,
  },
  {
    slot: 'user.topic.device_platform',
    subtopic: 'device_platform',
    re: /\b(?:iphone|ipad|android (?:phone|tablet)|pixel (?:phone|\d)|galaxy (?:s|note)\d+|smartphone|macbook|imac|thinkpad|chromebook|(?:my|the user'?s|a new|new) (?:phone|laptop|computer|tablet)|windows (?:pc|laptop|machine)|linux (?:laptop|machine|desktop)|gaming pc)\b/i,
  },
  {
    slot: 'user.topic.subscription_service',
    subtopic: 'subscription_service',
    // Tier-adjective + plan/tier is the universal subscription naming scheme
    // ("Premium 4K plan", "Basic single-stream plan", "annual plan"). Bare
    // "streaming service" is deliberately absent: news ABOUT a service ("the
    // streaming service raised prices") is not the user's subscription state.
    re: /\b(?:subscri(?:bes?|bed|ption)|(?:basic|standard|premium|pro|plus|family|free|paid|monthly|annual|yearly)(?:[- ][a-z0-9]+)* (?:plan|tier)|(?:cloud )?storage plan|free tier|paid tier)\b/i,
  },
  {
    slot: 'user.topic.health_regimen',
    subtopic: 'health_regimen',
    // Treatment/medication regimen — disjoint from the value-typed
    // health.weight_goal slot (weight-loss phrasing stays value-typed).
    // Dose phrasing demands medication context ("10 mg of", "50 mg once
    // daily") — a bare \d+mg would sweep nutrient quantities ("2,000 mg of
    // sodium") into the medication group.
    re: /\b(?:medicat(?:ion|ions)|prescri(?:bed|ption)|dosage|\d+ ?mg (?:of|once|twice|daily|per|every)|takes? \d+ ?mg|physical therapy|physiotherapy|supplements?|insulin|inhaler|allerg(?:y|ies) (?:shots?|meds?|medication)|blood pressure (?:medication|pills|meds))\b/i,
  },
  {
    slot: 'user.topic.mortgage',
    subtopic: 'mortgage',
    re: /\bmortgage\b/i,
  },
  {
    slot: 'user.topic.auto_loan',
    subtopic: 'auto_loan',
    re: /\b(?:(?:car|auto) loan|car payments?)\b/i,
  },
  {
    slot: 'user.topic.student_loan',
    subtopic: 'student_loan',
    re: /\bstudent loans?\b/i,
  },
  {
    slot: 'user.topic.banking',
    subtopic: 'banking',
    re: /\b(?:(?:bank|savings|checking|brokerage) account|banks? with|credit card)\b/i,
  },
  {
    slot: 'user.topic.relationship_status',
    subtopic: 'relationship_status',
    // USER_FACT/PREFERENCE/DECISION types only (the shared type gate), so
    // relationship-typed rows keep the value-typed relationship.primary_partner.
    re: /\b(?:married|marriage|engaged to|got engaged|divorc(?:e|ed|ing)|separat(?:ed|ion) from|spouse|wife|husband|fianc[ée]+|girlfriend|boyfriend|broke up with|dating)\b/i,
  },
  {
    slot: 'user.topic.education_program',
    subtopic: 'education_program',
    re: /\b(?:enrolled (?:in|at)|bootcamp|grad school|master'?s (?:degree|program)|bachelor'?s(?: degree)?|phd|doctorate|night (?:classes|school)|certification (?:course|program|exam)|degree (?:in|at|from)|studying (?:for|at)|coursework|semester)\b/i,
  },
  {
    slot: 'user.topic.pet_companion',
    subtopic: 'pet_companion',
    re: /\b(?:(?:my|our|the user'?s) (?:dog|cat|puppy|kitten|pets?)|adopt(?:ed|ing) (?:a|an|the|two) (?:dog|cat|puppy|kitten|rescue)|pet (?:dog|cat|sitter)|rehom(?:ed|ing)|vet (?:visit|appointment)s?)\b/i,
  },
  {
    slot: 'user.topic.exercise_routine',
    subtopic: 'exercise_routine',
    // Late on purpose: `runs?/running` is the broadest verb in the lexicon.
    re: /\b(?:runs?|running|jog(?:s|ging)?|gym|workout(?:s)?|work(?:s|ed)? out|yoga|swim(?:s|ming)?|crossfit|impact activity|weight[- ]bearing|marathon training)\b/i,
  },
  {
    slot: 'user.topic.diet_pattern',
    subtopic: 'diet_pattern',
    re: /\b(?:vegetarian|vegan|pescatarian|keto|gluten[- ]free|dairy[- ]free|eats? meat|intermittent fasting|quit (?:coffee|alcohol|sugar)|caffeine|decaf|espresso|celiac)\b/i,
  },
  {
    slot: 'user.topic.hobby_activity',
    subtopic: 'hobby_activity',
    // The co-fact-heaviest domain: taking up a second hobby is NEVER a rival
    // to the first. Safe only because the change-cue gate refuses to
    // arbitrate the group without explicit change language ("quit", "gave
    // up", "no longer") — verified against the no-conflict controls.
    re: /\b(?:hobby|hobbies|photography|hik(?:es?|ing)|knit(?:s|ting)|garden(?:s|ing|er)?|book club|chess (?:club|set|tournament|night)|(?:plays?|playing|learn(?:s|ing)) (?:the )?(?:guitar|piano|violin|drums)|collect(?:s|ing|ion) (?:of )?(?:vinyl|records?|stamps|coins)|vinyl (?:records?|collection)|woodworking|pottery|birdwatch(?:ing|er)|rock climbing|rubik'?s cube|painting class(?:es)?|watercolors?|camping trips?|fishing trips?)\b/i,
  },
];
// A user-self belief must be a fact ABOUT THE USER. Without this anchor, a
// third-party fact ("Morgan lives in Lisbon") would be misattributed to
// user:self — and worse, arbitrated there during world-model builds that run
// before entity mentions exist (the standalone bootstrap path), starving the
// real entity of evidence. English + German first-person/user anchors.
// Bare "user" is NOT an anchor: in heterogeneous host stores it is commonly a
// unix account or ops noun ("runs as user `example-user`", "user session"),
// which can let infrastructure notes acquire life-domain slots.
const USER_ANCHOR_RE = /\b(?:the user|this user|user's|ich|mein(?:e|er|em)?|mir|mich)\b/i;
const FIRST_PERSON_RE = /\b(?:I|I'm|I've|my|me)\b/;
const isUserAnchoredContent = (content = '') => {
  const text = String(content || '');
  return USER_ANCHOR_RE.test(text) || FIRST_PERSON_RE.test(text);
};
// A user.topic.* slot holds an assertion about the user's LIFE STATE. Host-
// synced agent notes are mostly META: reported requests ("the user asked …"),
// agent guidance ("-> future runs should treat …"), diagnosis notes
// ("Symptom: … cause: …"), or memory-index link lines. They pass the user
// anchor (they mention the user) and trip topic vocabulary incidentally
// ("restarting my laptop", "future RUNS"). Meta content stays fully recallable;
// it just never becomes an exclusive-state belief.
const USER_TOPIC_META_RE = new RegExp([
  String.raw`->\s`,
  String.raw`\bfuture (?:runs|sessions|agents)\b`,
  String.raw`\bhow to apply\b`,
  String.raw`\bsymptom:`,
  String.raw`\bcause:`,
  // Reported speech is meta only when it QUOTES the user (the transcript-
  // note shape: the user said, "pls uninstall codex bar"). Unquoted reported
  // speech can carry a genuine state change ("the user told the project they
  // are cutting out caffeine") and must keep projecting.
  String.raw`\bthe user(?:['’]s)? (?:\w+ )?(?:said|says|asked|asks|wanted|wants|requested|requests|framed|told|typed|wrote|complained|reported|confirmed|clarified|insisted)\b[^.!?\n]{0,80}["'“”„«»]`,
  // Notes ABOUT the user's query/prompt are analysis, not user state.
  String.raw`\bthe user['’]?s? (?:[\w-]+ )*(?:query|prompt)\b`,
  String.raw`^\s*(?:[-*]\s*)?\[[^\]]+\]\([^)]+\)`,
  // Code formatting = ops/technical note, never a life-state assertion:
  // backticks, markdown bold, SCREAMING_SNAKE constants.
  '\\u0060',
  String.raw`\*\*`,
  String.raw`\b[A-Z]{2,}_[A-Z_]{2,}\b`,
].join('|'), 'im');
const matchUserFactTopicRule = (row = {}) => {
  if (!USER_FACT_TOPIC_TYPES.has(String(row.type || '').trim().toUpperCase())) return null;
  const content = String(row.content || '');
  if (!isUserAnchoredContent(content)) return null;
  if (USER_TOPIC_META_RE.test(content)) return null;
  return USER_FACT_TOPIC_RULES.find((rule) => rule.re.test(content)) || null;
};
// One user-self belief row, shaped exactly like the entity-branch belief rows
// so consolidation, persistence, and U12 stamps treat it identically.
const buildUserSelfBeliefRow = (row, memoryClaim, { claimSignal = null, topicRule = null } = {}) => {
  const beliefType = inferBeliefType(row);
  const beliefStatus = inferBeliefStatus(row);
  return {
    belief_id: `belief:${row.memory_id}:user-self`,
    entity_id: USER_SELF_ENTITY_ID,
    type: beliefType,
    content: row.content,
    status: beliefStatus,
    ...
    supersedes_belief_id: row.superseded_by ? `belief:${row.superseded_by}:user-self` : null,
    ...
    payload: {
      ...
      topic: claimSignal?.topic || topicRule?.subtopic || inferBeliefTopic(row),
      source_layer: row.source_layer,
      memory_tier: memoryClaim.memory_tier,
      source_strength: memoryClaim.source_strength,
      surface_candidate: false,
      claim_slot: claimSignal?.slot || topicRule?.slot || '',
      claim_value: claimSignal?.normalizedValue || summarizeContent(row.content, 120),
      claim_topic: claimSignal?.topic || 'user',
      claim_subtopic: claimSignal?.subtopic || topicRule?.subtopic || '',
      consolidation_operation: claimSignal?.operation || (beliefStatus === 'stale' ? 'forget' : 'remember'),
    },
  };
};
...
const normalizeClaimSlotFromBelief = (belief = {}) => {
  ...
  if (/\b(?:visual identity|profile image|avatar)\b/i.test(content)) {
    return {
  ...
  if (/\b(?:tts default engine|voice preset|voice preference)\b/i.test(content)) {
    return {
  ...
  // A birthday BELIEF states WHEN (a month or a numeric date). Rows that only
  // mention birthdays as events ("sent a birthday story", "birthday gifts")
  // carry no value to arbitrate and would otherwise pile into
  // identity.birthday, producing false contradiction loops.
  if (/\b(?:birthday|geburtstag)\b/i.test(content)
    && (/\b(?:jan(?:uary|uar)?|feb(?:ruary|ruar)?|march|märz|apr(?:il)?|may|mai|june?|juni|july?|juli|aug(?:ust)?|sep(?:t|tember)?|o[ck]t(?:ober)?|nov(?:ember)?|de[cz](?:ember)?)\b/i.test(content)
...
const readProtectedLegacyEntities = (db) => {
  if (!hasTable(db, 'memory_entities')) {
    return { protectedByName: new Map(), protectedEntities: [] };
  }
  const identityEntityIds = hasTable(db, 'memory_beliefs') ? new Set(db.prepare(`
    SELECT DISTINCT entity_id
    FROM memory_beliefs
    WHERE lower(type) = 'identity'
      AND status IN ('current', 'uncertain')
  `).all().map((row) => String(row.entity_id || '')).filter(Boolean)) : new Set();
  const aliasesByEntity = new Map();
  if (hasTable(db, 'memory_entity_aliases')) {
    for (const row of db.prepare(`
      SELECT entity_id, alias
      FROM memory_entity_aliases
      ORDER BY entity_id, confidence DESC, alias
    `).all()) {
      const entityId = String(row.entity_id || '');
      const aliases = aliasesByEntity.get(entityId) || [];
      aliases.push(String(row.alias || '').trim());
      aliasesByEntity.set(entityId, aliases.filter(Boolean));
    }
  }
  const protectedByName = new Map();
  const protectedEntities = [];
  for (const row of db.prepare(`
    SELECT entity_id, kind, display_name, normalized_name, status, confidence,
  ...
  `).all()) {
    ...
    if (!entityId || !normalizedName) continue;
    const payload = parseJsonSafe(row.payload, {});
    const protectedEntity = identityEntityIds.has(entityId)
      || payload.rebuild_protected === true
      || payload.surface_curated === true
      || payload.surface_visible === true;
    ...
    const sameName = protectedByName.get(normalizedName) || [];
    sameName.push(preserved);
    protectedByName.set(normalizedName, sameName);
    protectedEntities.push(preserved);
  }
  return { protectedByName, protectedEntities };
};
...
const queryNativeWorldRows = (db, now = new Date().toISOString()) => {
  ensureNativeStore(db);
  if (!hasTable(db, 'memory_native_chunks')) return [];
  return db.prepare(`
    SELECT
      'native:' || chunk.chunk_id AS memory_id,
      COALESCE(chunk.memory_type, 'CONTEXT') AS type,
      chunk.content,
      chunk.normalized,
      COALESCE(linked.scope, chunk.scope, CASE
        WHEN chunk.source_kind = 'curated' THEN 'shared'
        WHEN chunk.source_kind = 'memory_md' THEN 'profile:main'
        ELSE ''
      END) AS scope,
      COALESCE(chunk.last_seen_at, ?) AS updated_at,
      COALESCE(chunk.first_seen_at, chunk.last_seen_at, ?) AS created_at,
      'native' AS source_layer,
      chunk.source_path,
    ...
    WHERE chunk.status = 'active'
      AND lower(trim(COALESCE(chunk.source_kind, ''))) != 'vault'
    ORDER BY chunk.last_seen_at DESC
    LIMIT 10000
  `).all(now, now);
};

const buildVisibleEntityAliasesForScope = (db, entityKey = '', scope = '', options = {}) => {
  const requested = normalizeRequestedScope(scope);
  ...
  ensurePersonStore(db);
  const scopeFilter = buildEntityMentionScopeFilter(requested, 'scope', options);
...
const buildVisibleEntityKeysForScope = (db, scope = '', options = {}) => {
  const requested = normalizeRequestedScope(scope);
...
const buildSourceScopeLookup = (db, sourceIds = []) => {
  ...
  const query = db.prepare(`
    ...
    LIMIT 1
  `);
  const nativeQuery = hasTable(db, 'memory_native_chunks') ? db.prepare(`
    SELECT
      'native:' || chunk.chunk_id AS memory_id,
      COALESCE(linked.scope, chunk.scope, CASE
        WHEN chunk.source_kind = 'curated' THEN 'shared'
    ...
    LEFT JOIN memory_current AS linked
      ON linked.memory_id = chunk.linked_memory_id
      AND linked.status = 'active'
    WHERE ('native:' || chunk.chunk_id) = ?
    LIMIT 1
  `) : null;
  for (const sourceId of ids) {
    const row = query.get(sourceId) || nativeQuery?.get(sourceId);
    if (!row) continue;
...
const inferEntityKind = (entityKey, memoryRows = [], mentionRows = [], config = {}) => {
  ...
  const personProfileCueTokens = protectedTermSet(
    'surface',
    'personCueTerms',
    new Set(['founder', 'ceo', 'role', 'job', 'title', 'based', 'lives', 'prefers', 'likes', 'london']),
  );
  const hasAliasProfileCue = memoryRows.some((row) => {
    const content = String(row.content || '');
    return hasAliasCueWithinWindow(content, entityKey, personProfileCueTokens, 6)
  ...
  if (isLikelyPersonAlias(entityKey) && isEntityKindEnabled(config, 'person')) {
    ...
    // WINDOWED, not whole-text: a relationship cue anywhere in the memory
    // used to make every co-mentioned capitalized token a person. German
    // capitalizes all nouns, so relationship-heavy content can turn common
    // nouns into false person entities. The cue must sit within the same
    // 6-token window as the alias, exactly like hasAliasProfileCue.
    if (memoryRows.some((row) => hasAliasCueWithinWindow(row.content, entityKey, RELATIONSHIP_CUE_TOKENS, 6))) return 'person';
...
const evaluateEntityCandidate = ({
  ...
  if (kind === 'person') {
    ...
    const hasAliasProfileCue = relatedMemoryRows.some((row) => filteredAliases.some((alias) => (
      hasAliasCueWithinWindow(String(row.content || ''), alias, personProfileCueTokens, 6)
      || hasPlacePatternForAlias(String(row.content || ''), alias)
    )));
    if ((!strongName && !strongMention) || !isLikelyPersonAlias(entityKey)
      || (!enoughEvidence && !strongMention && !hasCuratedEvidence && !hasAgentIdentityEvidence)) {
...
const ensureWorldModelReady = ({
  ...
  const activeCount = Number(db.prepare(`
    ...
    WHERE status = 'active'
  `).get()?.c || 0);
  const eligibleNativeState = db.prepare(`
    SELECT
      COUNT(*) AS count,
      COALESCE(MAX(last_seen_at), '') AS last_seen_at
    FROM memory_native_chunks
    WHERE status = 'active'
      AND lower(trim(COALESCE(source_kind, ''))) != 'vault'
  `).get();
  const eligibleNativeCount = Number(eligibleNativeState?.count || 0);
  const latestMemoryUpdatedAt = String(db.prepare(`
    SELECT COALESCE(MAX(updated_at), '') AS updated_at
    FROM memory_current
    WHERE status IN ('active', 'superseded')
  `).get()?.updated_at || '').trim();
  const latestNativeLastSeenAt = String(eligibleNativeState?.last_seen_at || '').trim();
  const latestSourceUpdatedAt = [latestMemoryUpdatedAt, latestNativeLastSeenAt]
    .filter(Boolean)
    .sort((left, right) => (Date.parse(right) || 0) - (Date.parse(left) || 0))[0] || '';
  const eligibleSourceCount = activeCount + eligibleNativeCount;
  ...
  const needsRefresh = Boolean(
    ...
    && (!latestSynthesisGeneratedAt || Date.parse(latestSourceUpdatedAt) > Date.parse(latestSynthesisGeneratedAt)),
  );
...
// U17: the projection half of the world-model rebuild — registry/native rows →
// entity evaluation → belief rows with claim slots (plus the episode and
// open-loop rows the surfaces need). Shared by `rebuildWorldModel` (which
// consolidates + persists the world-model tables) and
// `projectArbitrationBeliefRows` (which feeds the SAME belief rows to
// belief-arbitration.js when the world model is OFF). Writes no world-model
// table rows; the ensure* calls are schema-only.
const buildWorldModelProjection = ({
  db,
  config,
  ...
  ensurePersonStore(db);
  const {
    protectedByName: protectedLegacyByName,
    protectedEntities: protectedLegacyEntities,
  } = readProtectedLegacyEntities(db);

  const rows = [
    ...listCurrentMemories(db, {
      statuses: ['active', 'superseded'],
      limit: 10000,
    }),
    ...queryNativeWorldRows(db, now),
  ].map((row) => ({
    ...row,
    memory_id: String(row.memory_id || ''),
    type: String(row.type || 'CONTEXT').trim().toUpperCase() || 'CONTEXT',
    ...
    origin_kind: row.origin_kind ? String(row.origin_kind) : null,
  }));
  if (rows.length === 0) {
    return {
      empty: true,
  ...
  const compoundCoverageByToken = new Map();
  for (const [key, rowsForKey] of mentionsByKey) {
    const tokens = key.split(/\s+/).filter(Boolean);
    if (tokens.length < 2) continue;
    const memoryIds = new Set(rowsForKey.map((row) => String(row.memory_id || '')).filter(Boolean));
    for (const token of tokens) {
      const candidates = compoundCoverageByToken.get(token) || [];
      candidates.push({ key, memoryIds });
      compoundCoverageByToken.set(token, candidates);
    }
  }
  const isCompoundShadowedFragment = (key, rowsForKey) => {
    if (!key || key.includes(' ')) return false;
    if (protectedLegacyByName.has(key)) return false;
    const memoryIds = new Set(rowsForKey.map((row) => String(row.memory_id || '')).filter(Boolean));
    if (memoryIds.size === 0) return false;
    return (compoundCoverageByToken.get(key) || []).some((compound) => (
      compound.key !== key && [...memoryIds].every((memoryId) => compound.memoryIds.has(memoryId))
    ));
  };
  ...
  for (const [entityKey, entityMentions] of mentionsByKey.entries()) {
    if (isCompoundShadowedFragment(entityKey, entityMentions)) continue;
    const protectedLegacy = protectedLegacyByName.get(entityKey)?.[0] || null;
    ...
    const kind = protectedLegacy?.kind || inferEntityKind(entityKey, relatedMemoryRows, entityMentions, config);
    if (isGenericAlias(entityKey) && !protectedLegacy) continue;
    if (!kind) continue;
    const entityId = protectedLegacy?.entity_id || `${kind}:${slugify(entityKey)}`;
    const display = protectedLegacy?.display_name
      || String(entityMentions[0]?.entity_display || displayNameFromKey(entityKey));
    const aliases = Array.from(new Set([
      display,
      displayNameFromKey(entityKey),
      ...(protectedLegacy?.aliases || []),
    ...
    let evaluation = evaluateEntityCandidate({
      entityKey,
      kind,
      aliases,
      relatedMemoryRows,
    ...
    if (protectedLegacy) {
      evaluation = {
        ...
        recallAllowed: protectedLegacy.payload?.recall_allowed !== false,
        surfaceVisible: protectedLegacy.payload?.surface_visible !== false,
      };
    }
    ...
    const entityRow = {
      ...
      status: 'active',
      confidence: Math.max(confidence, Number(protectedLegacy?.confidence || 0)),
      aliases: evaluation.aliases,
      created_at: protectedLegacy?.created_at || now,
      updated_at: new Date(updatedAt).toISOString(),
      payload: {
        ...(protectedLegacy?.payload || {}),
        evidence_count: relatedMemoryRows.length,
        scopes: Array.from(new Set([
          ...(Array.isArray(protectedLegacy?.payload?.scopes) ? protectedLegacy.payload.scopes : []),
          ...relatedMemoryRows.map((row) => row.scope),
        ])),
  ...
  const preservedEntityIds = new Set(entityRows.map((row) => String(row.entity_id || '')).filter(Boolean));
  for (const protectedLegacy of protectedLegacyEntities) {
    if (preservedEntityIds.has(protectedLegacy.entity_id)) continue;
    const aliases = Array.from(new Set([
      protectedLegacy.display_name,
      protectedLegacy.normalized_name,
      ...(protectedLegacy.aliases || []),
    ].map((value) => String(value || '').trim()).filter(Boolean)));
    entityRows.push({
      entity_id: protectedLegacy.entity_id,
      kind: protectedLegacy.kind,
      display_name: protectedLegacy.display_name,
      normalized_name: protectedLegacy.normalized_name,
      status: 'active',
      confidence: protectedLegacy.confidence,
      aliases,
      created_at: protectedLegacy.created_at || now,
      updated_at: protectedLegacy.updated_at || now,
      payload: {
        ...(protectedLegacy.payload || {}),
        recall_allowed: protectedLegacy.payload?.recall_allowed !== false,
        surface_visible: protectedLegacy.payload?.surface_visible !== false,
      },
    });
    preservedEntityIds.add(protectedLegacy.entity_id);
    const seenAliases = new Set();
    for (const alias of aliases) {
      const normalizedAlias = normalizeContent(alias);
      if (!normalizedAlias || seenAliases.has(normalizedAlias)) continue;
      seenAliases.add(normalizedAlias);
      aliasRows.push({
        alias_id: `${protectedLegacy.entity_id}:${slugify(normalizedAlias)}`,
        entity_id: protectedLegacy.entity_id,
        alias,
        normalized_alias: normalizedAlias,
        confidence: protectedLegacy.confidence,
  ...
  for (const row of rows) {
    ...
    // U13b: a user-typed row matching the topic lexicon states the USER's
    // current mode in some life domain — project a user-self belief even when
    // the row also mentions an entity (e.g. "accepted a new role … badging
    // into headquarters" names an org but changes the user's work-location
    // state; without this it can never meet its entity-less rival). The
    // lexicon match itself is the noise filter, so the ops-tier gate does not
    // apply ("non-weight-bearing BOOT" false-positives the ops regex).
    const userTopicRule = matchUserFactTopicRule(row);
    if (userTopicRule) {
      beliefRows.push(buildUserSelfBeliefRow(row, memoryClaim, { topicRule: userTopicRule }));
    }
    if (entityKeys.length === 0) {
      // U13b: entity-less rows previously never became beliefs at all. With
      // no topic-lexicon match, fall back to the generic value-typed
      // detectors; the lexicon takes precedence because the generic cues
      // ("from …") were tuned for entity-bearing content and mislabel free
      // text. The user anchor is required here too — an unanchored row is
      // about somebody/something else even when no entity was recognized.
      // No slot or ops-tier content → no belief (prior behavior).
      if (!userTopicRule && isUserAnchoredContent(row.content)) {
        const claimSignal = normalizeClaimSlotFromBelief({
          ...row,
          type: inferBeliefType(row),
          entityKey: '',
          entityKind: '',
        });
        if (claimSignal && normalizeMemoryTier(memoryClaim.memory_tier) !== 'ops_runbook') {
          beliefRows.push(buildUserSelfBeliefRow(row, memoryClaim, { claimSignal }));
        }
      }
      continue;
    }
...
// U17: belief projection for arbitration WITHOUT the world model — the
// projector handed to belief-arbitration.js#runBeliefArbitration by every
// caller that must keep verdicts flowing when `worldModel.enabled === false`
// (capture, maintenance, gigabrain_arbitrate, watch --arbitrate, startup).
// Same machinery, same belief rows, zero world-model table writes — the
// toggle gates the SURFACES, never the verdicts.
const projectArbitrationBeliefRows = ({ db, config, now = new Date().toISOString() } = {}) => (
  buildWorldModelProjection({ db, config, now }).beliefRows
);
...
const rebuildWorldModel = ({
  ...
  assertConfiguredCandidateOperation({ operation: 'world.rebuild', config, db });
  const projection = buildWorldModelProjection({ db, config, now });
  if (projection.empty) {
    clearWorldModelStore(db);
    return {
      ok: true,
  ...
  const consolidatedBeliefs = consolidateBeliefRows(beliefRows, { db, settings: arbiterSettings });
  ...
  const currentBeliefs = selectDistinctItems(
    [...resolvedBeliefRows]
      ...
      .sort((a, b) => beliefPriorityScore(b, arbiterSettings) - beliefPriorityScore(a, arbiterSettings)),
    (belief) => belief.payload?.claim_slot || belief.content,
    8,
  );
  const recentBeliefs = selectDistinctItems(
    [...resolvedBeliefRows]
      ...
      .sort((a, b) => {
        ...
        return bTs - aTs || beliefPriorityScore(b, arbiterSettings) - beliefPriorityScore(a, arbiterSettings);
      }),
    (belief) => belief.payload?.claim_slot || belief.content,
    12,
  );
  ...
  // Configuration discipline: synthesis.briefing.enabled has always
  // existed in schema/defaults but was never consumed. It now gates whether the
  // always-on session_brief synthesis is produced at all (default ON). The other
  // briefing surfaces (daily_memory_briefing, current_state) are untouched.
  const briefingEnabled = config?.synthesis?.briefing?.enabled !== false;
  if (briefingEnabled) {
    const briefScopes = Array.from(new Set([
      'shared',
      ...resolvedBeliefRows.map((belief) => normalizeScope(belief?.payload?.scope || 'shared')),
    ])).sort((left, right) => left.localeCompare(right, 'en'));
    for (const scope of briefScopes) {
      const includeShared = scope !== 'shared' && !scope.startsWith('project:');
      const visible = (belief) => isScopeVisibleForRequested(
        belief?.payload?.scope || 'shared',
        scope,
        { includeShared },
      );
      const visibleBeliefsForEntity = (entityId) => (
        (beliefsByEntity.get(entityId) || []).filter(visible)
      );
      const sessionPeople = importantPeople.slice(0, 1);
      const sessionProjects = importantProjects.slice(0, 1);
      const usedSummaryKeys = new Set();
      const usedSummarySlots = new Set();
      const sessionBeliefs = selectDistinctItems(
        recentBeliefs.filter((belief) => visible(belief) && isSessionRelevantBelief(belief)),
        (belief) => belief.payload?.claim_slot || belief.content,
        4,
      );
      const lines = [];
      ...
      for (const entity of [...sessionPeople, ...sessionProjects]) {
        const summaryBelief = pickSurfaceSummaryBelief(entity, visibleBeliefsForEntity(entity.entity_id));
        if (!summaryBelief) continue;
        const key = normalizeContent(summaryBelief.content);
        const slot = String(summaryBelief?.payload?.claim_slot || '').trim();
        if (usedSummaryKeys.has(key)) continue;
        if (slot && usedSummarySlots.has(slot)) continue;
        const overlapsBelief = Array.from(usedSummaryKeys).some((used) => (
          used.startsWith(key) || key.startsWith(used)
        ));
        if (overlapsBelief) continue;
        usedSummaryKeys.add(key);
        if (slot) usedSummarySlots.add(slot);
        lines.push(`- ${summaryBelief.content}`);
      }
      const visibleScopes = includeShared ? [scope, 'shared'] : [scope];
      synthesisRows.push({
        synthesis_id: `briefing:session:${scope}`,
        kind: 'session_brief',
        subject_type: 'scope',
        subject_id: scope,
        content: buildGlobalSummaryContent({
        ...
        input_hash: hashNormalized(JSON.stringify([
          scope,
          ...sessionBeliefs.map((belief) => belief.belief_id),
          ...lines.map((line) => normalizeContent(line)),
        ])),
        payload: {
          count: sessionBeliefs.length,
          curated: true,
          scope,
  ...
  try {
    ...
    const insertBelief = db.prepare(`
      INSERT OR REPLACE INTO memory_beliefs (
    ...
    const insertSynthesis = db.prepare(`
      INSERT OR REPLACE INTO memory_syntheses (
...
const listEntities = (db, options = {}) => {
  if (options.ensure !== false) ensureWorldModelStore(db);
...
const listEntityAliases = (db, entityId, options = {}) => {
  if (options.ensure !== false) ensureWorldModelStore(db);
  return db.prepare(`
...
const findEntityMatches = (db, query, options = {}) => {
  ...
  const scopeVisibility = { includeShared: options.includeShared !== false };
  const visibleEntityKeys = buildVisibleEntityKeysForScope(db, requestedScope, scopeVisibility);
  ...
  const scored = aliases.map((row) => {
    ...
    if (requestedScope && !isEntityVisibleInScope(payload, requestedScope, scopeVisibility)) return null;
...
const getEntityDetail = (db, entityId, options = {}) => {
  ...
  if (requestedScope && !isEntityVisibleInScope(entity.payload, requestedScope, scopeVisibility)) return null;
  const beliefs = listBeliefs(db, { ensure: options.ensure, entityId: entity.entity_id, limit: 200 });
  const episodes = listEpisodes(db, { ensure: options.ensure, entityId: entity.entity_id, limit: 100 });
  const openLoops = listOpenLoops(db, { ensure: options.ensure, entityId: entity.entity_id, limit: 100 });
  ...
  const scopedBeliefs = requestedScope
    ? beliefs.filter((belief) => {
      ...
      return isScopeVisibleForRequested(sourceScopeLookup.get(sourceId), requestedScope, scopeVisibility);
    })
    : beliefs;
  const scopedEpisodes = requestedScope
    ? episodes.filter((episode) => episode.source_memory_ids.some((sourceId) => {
      ...
      return isScopeVisibleForRequested(sourceScopeLookup.get(normalizedSourceId), requestedScope, scopeVisibility);
    }))
    : episodes;
  ...
  const scopedAliases = requestedScope
    ? buildVisibleEntityAliasesForScope(db, entity.normalized_name || entity.display_name, requestedScope, scopeVisibility)
    : listEntityAliases(db, entity.entity_id, { ensure: options.ensure }).map((aliasRow) => aliasRow.alias);
  return {
    ...entity,
    ...
    syntheses: shouldExposeEntitySyntheses(entity.payload, requestedScope, scopeVisibility)
      ? listSyntheses(db, { ensure: options.ensure, subjectType: 'entity', subjectId: entity.entity_id, limit: 20 })
      : [],
  };
};
...
export {
  ...
  // Exported for security regression coverage of the session-brief scope partition.
  isBriefEligibleScope,
};
