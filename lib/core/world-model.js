...
const readProtectedLegacyEntities = (db) => {
  if (!hasTable(db, 'memory_entities')) return new Map();
  ...
  `).all()) {
    ...
    if (!protectedEntity || protectedByName.has(normalizedName)) continue;
...
const buildWorldModelProjection = ({
  ...
  ensureProjectionStore(db);
  ensurePersonStore(db);
  const protectedLegacyByName = readProtectedLegacyEntities(db);
  ...
  for (const [entityKey, entityMentions] of mentionsByKey.entries()) {
    ...
    const protectedLegacy = protectedLegacyByName.get(entityKey) || null;
