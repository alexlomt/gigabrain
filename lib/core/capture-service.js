...
import { listCurrentMemories, normalizeProjectionScope, upsertCurrentMemory } from './projection-store.js';
...
const valueToText = (value, depth = 0) => {
  ...
  if (typeof value === 'object') {
    ...
    if (blockType) {
      if (['thinking', 'reasoning', 'toolcall', 'tool_call', 'toolresult', 'tool_result', 'image', 'media', 'attachment', 'file'].includes(blockType)) {
        return '';
      }
      if (['text', 'output_text', 'input_text', 'summary_text'].includes(blockType)) {
        const direct = valueToText(record.text ?? record.content ?? record.output_text ?? record.message ?? record.output ?? record.result ?? record.final, depth + 1);
...
const extractCandidateText = (event = {}) => {
  ...
  if (parts.length === 0 && Array.isArray(event?.messages)) {
...
const inferSourceKindFromPath = (sourcePath = '') => {
  const normalized = String(sourcePath || '').trim().replace(/\\/g, '/');
  if (!normalized) return '';
  if (/(^|\/)MEMORY\.md$/i.test(normalized)) return 'memory_md';
  if (/(^|\/)memory\/\d{4}-\d{2}-\d{2}.*\.md$/i.test(normalized)) return 'daily_note';
  if (/\.md$/i.test(normalized)) return 'curated';
  return '';
};
...
const captureFromEvent = ({
  ...
  const scope = canonicalScope(event.scope || event.agentId || 'shared');
  const sourceText = extractCandidateText(event);
  ...
  const writeNative = ({ memoryId = '', type, content, durable, scope: noteScope = '' }) => {
    try {
      ...
      if (result?.written) {
        ...
        if (result.source_path) nativeSourcePaths.add(String(result.source_path));
      }
      return result;
    } catch (err) {
  ...
  const finalizeSideEffects = () => {
    summary.native_source_paths = Array.from(nativeSourcePaths);
    if (summary.native_source_paths.length > 0 && config?.native?.enabled !== false) {
      try {
        summary.native_sync = syncNativeMemory({
          db,
          config,
          sourcePaths: summary.native_source_paths,
        });
        nativeIndexDirty = Number(summary.native_sync?.changed_files || 0) > 0 || Number(summary.native_sync?.inserted_chunks || 0) > 0;
      } catch (err) {
        logger?.warn?.(`[gigabrain] post-capture native sync failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    }

    if ((worldModelDirty || nativeIndexDirty) && config?.worldModel?.enabled !== false) {
      try {
  ...
  if (Array.isArray(actionSummary.native_source_paths)) {
    for (const sourcePath of actionSummary.native_source_paths) nativeSourcePaths.add(String(sourcePath));
  }
  ...
  for (const note of allNotes) {
    ...
    const noteScope = note.scope ? canonicalScope(note.scope) : scope;
    ...
    if (
      ...
      appendQueueRow(queuePath, {
        ...
        payload: {
          type,
          content,
          excerpt: buildQueueExcerpt(event, content),
          scope: noteScope,
    ...
    if (exact) {
      ...
      summary.write_records.push({
        ...
        written_registry: false,
        source_path: String(exact.source_path || ''),
        source_line: Number.isFinite(Number(exact.source_line)) ? Number(exact.source_line) : null,
        source_kind: inferSourceKindFromPath(exact.source_path || ''),
      });
      continue;
    }
    ...
    if (config?.dedupe?.semanticEnabled !== false) {
      ...
      if (semantic && semantic.similarity >= Number(semanticThresholds.auto)) {
        ...
        summary.write_records.push({
          ...
          written_native: false,
          written_registry: false,
          source_path: String(semantic.row.source_path || ''),
          source_line: Number.isFinite(Number(semantic.row.source_line)) ? Number(semantic.row.source_line) : null,
          source_kind: inferSourceKindFromPath(semantic.row.source_path || ''),
        });
        continue;
      }
