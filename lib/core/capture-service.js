...
const withCaptureMutation = ({ db, tx, operationId, now }, fn) => {
  if (tx) {
...
const enactCaptureContradict = ({
  ...
  withCaptureMutation({
    db,
    tx,
    operationId: operationId || runId || `capture-contradict-${winnerId}`,
    now: nowIso,
  }, (batchTx) => {
    winnerRow = upsertCurrentMemory(db, {
      memory_id: winnerId,
      type,
      content,
      normalized,
      ...
      event: {
        timestamp: nowIso,
        component: 'capture',
        action: 'capture_llm_contradict_inserted',
        reason_codes: ['capture_contradiction'],
        memory_id: winnerId,
        cleanup_version: cleanupVersion,
        run_id: runId || '',
        review_version: reviewVersion || '',
        payload: { scope: noteScope, target_id: target.id },
      },
    });
    recordVerdict(db, {
      winnerId,
    ...
    }, { timestamp: nowIso, tx: batchTx, faultInjector });
  });
...
const captureFromEventInBatch = ({
  ...
  for (const note of allNotes) {
    ...
    if (captureLlm.enabled && typeof captureLlm.decide === 'function' && !llmTransportFailed) {
      ...
      if (op === 'UPDATE' && target) {
        ...
        const revisedRow = upsertCurrentMemory(db, {
          ...
          tx,
          faultInjector,
          operationId: operationId || runId || `capture-update-${target.id}`,
          event: {
            timestamp: nowIso,
            component: 'capture',
    ...
    const row = upsertCurrentMemory(db, {
      ...
      tx,
      faultInjector,
      operationId: operationId || runId || `capture-insert-${memoryId}`,
      event: {
        timestamp: nowIso,
        component: 'capture',
  ...
  if (worldModelDirty && refreshDerived !== false) {
    // Projection-writing arbitration belongs to the capture transaction. A
    // verdict/current/legacy/FTS/event failure aborts the whole capture.
    runBeliefArbitration({ db, config, projectBeliefRows });
    summary.derived_refresh_required = true;
  }
...
const captureFromEvent = (options = {}) => {
  assertWriteAllowed({
    mode: resolveWriteMode(options.config),
    operation: 'capture.capture_from_event',
  });
  const db = options.db;
  if (!db) throw new Error('captureFromEvent requires db');
  const execute = () => {
    if (options.tx) return captureFromEventInBatch(options);
    const operationId = String(
      options.operationId
        || options.runId
        || options.event?.sessionKey
        || `capture-${randomUUID()}`,
    );
    const summary = withProjectionMutationBatch({ db, operationId }, (tx) => captureFromEventInBatch({
      ...options,
      operationId,
      tx,
    })).result;

    if (summary.derived_refresh_required && options.refreshDerived !== false) {
      // Derived tables refresh only after the authoritative capture commits.
      // These are mutating maintenance operations, so failures propagate rather
      // than being mislabeled as observational warnings.
      rebuildEntityMentions(db);
      if (options.config?.worldModel?.enabled !== false) {
        rebuildWorldModel({ db, config: options.config });
      }
    }
    return summary;
  };
  const hasNativeWritingAction = parseMemoryActions(extractCandidateText(options.event))
    .some((action) => ['replace', 'update'].includes(String(action?.action || '').toLowerCase()));
  const needsNativeLock = options.config?.capture?.rememberIntent?.writeNative !== false
    || hasNativeWritingAction;
  if (!needsNativeLock) return execute();
  if ((options.tx || db.isTransaction) && !isNativeMemoryLockHeld(options.config)) {
    throw new Error('GIGABRAIN_NATIVE_LOCK_ORDER: capture transaction requires the outer native-memory lock');
  }
  return withNativeMemoryLockSync(options.config, execute);
};
