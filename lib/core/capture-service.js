...
import { getEmbeddingSync, cosineSimilarity, blobToVec } from './embedding-service.js';
...
const rankNeighborsByEmbedding = (db, eligible, content, config = {}) => {
  ...
  try {
    ...
    for (let index = 0; index < eligibleIds.length; index += chunkSize) {
      ...
      stored.push(...db.prepare(
        `SELECT rowid, memory_id, embedding FROM memory_embeddings WHERE memory_id IN (${placeholders})`,
      ).all(...chunk));
    }
  } catch {
    return null;
  ...
  const queryVec = getEmbeddingSync(content, {
    baseUrl: recall.ollamaUrl,
    model: recall.embeddingModel,
