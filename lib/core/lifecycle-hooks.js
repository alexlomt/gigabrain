...
// session hook fires on the HOST's own lifecycle — Claude Code's SessionEnd and
// PreCompact events. It is the safety net for the lost-facts hole: capture
// otherwise only fires at the single agent_end flush turn + the MEMORY_FLUSH
// magic-phrase heuristic, so a session that dies (crash, /clear, OOM) with no
// flush turn loses its untagged facts. The SessionEnd hook auto-invokes a
// Gigabrain checkpoint so the session is flushed regardless of how it ends.
//
...
// the session auto-flush hook is Claude-Code-only until Codex ships such a hook.
...
// node + checkpoint script. Stays advisory from the host's view — a failed
// checkpoint must never surface an error during session teardown — but audit
// Hook failures leave a durable marker instead of exiting
// 0 in total silence; for a continuity product that failure silently loses
// the whole session's checkpoint.
...
export const installSessionHook = ({ settingsPath, configPath = '', nodeBin, checkpointScript } = {}) => {
  ...
  return {
    ...
    note: 'Claude Code SessionEnd + PreCompact auto-flush hook installed (best-effort, never blocks the host).',
  };
};
