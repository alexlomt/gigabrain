...
// U10 anchor cleanup (R8a): owner-overfit literals (weight/loss/kg/poly/
// newsletter/telegram) were DELETED — only genuinely generic life-event and
// relationship tokens remain. Deployment-specific anchors belong in config
// (quality.plausibility.semanticAnchorsAppend); U19's denylist enforces this.
...
const createSemanticFingerprint = (value) => {
  const text = String(value || '');
  const normalized = normalizeContent(text);
  return {
    normalized,
    words: new Set(normalized ? normalized.split(/\s+/).filter(Boolean) : []),
    trigrams: new Set(charNgrams(normalized, 3)),
    numbers: new Set(extractNumericTokens(normalized)),
    anchors: new Set(DEFAULT_SEMANTIC_ANCHORS_BASE.filter(
      (item) => new RegExp(`\\b${item}\\b`, 'i').test(text),
    )),
  };
};

const overlapPrepared = (setA, setB) => {
  if (!(setA instanceof Set) || !(setB instanceof Set) || setA.size === 0 || setB.size === 0) return null;
  let intersection = 0;
...
const jaccardSimilarityFromFingerprints = (left, right) => {
  ...
  pushWeighted(overlapPrepared(left.words, right.words) ?? 0, 0.35);
  pushWeighted(overlapPrepared(left.trigrams, right.trigrams) ?? 0, 0.25);
  const numericScore = overlapPrepared(left.numbers, right.numbers);
  if (numericScore != null) pushWeighted(numericScore, 0.2);
  const anchorScore = overlapPrepared(left.anchors, right.anchors);
...
const jaccardSimilarity = (a, b) => jaccardSimilarityFromFingerprints(
  createSemanticFingerprint(a),
  createSemanticFingerprint(b),
);
