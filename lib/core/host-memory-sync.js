  ...
  for (const { vendor, source_host: sourceHost } of CLOUD_INBOX_VENDORS) {
    ...
    for (const filePath of cloudExportFilesForVendor(vendorDir)) {
      ...
      if (incremental && sourceUnchanged(readCursor(db, filePath), stat, fileHash)) {
        ...
        if (options.dryRun !== true) {
          writeCloudInboxState(db, {
            ...
            factCount: Number(prevState?.fact_count || 0), nowIso,
          });
        }
        summary.sources.push({ source_host: sourceHost, vendor, source_path: filePath, status: 'unchanged', indexed_count: 0, last_export_mtime_ms: mtimeMs });
        continue;
      }
      ...
      for (const item of items) {
        ...
        if (!normalized || normalized.length < 4) { summary.skipped_count += 1; continue; }
        ...
        if (options.dryRun !== true && !existing) {
          upsertCurrentMemory(db, {
            ...
            // manual_import trust FLOOR (~0.5) via the SAME ingestConfidence path.
      ...
      if (options.dryRun !== true) {
        writeCloudInboxState(db, { sourceHost, sourcePath: filePath, vendor, mtimeMs, factCount: indexed, nowIso });
        // Reuse the host_sync cursor so a subsequent unchanged scan is a no-op.
        writeCursor(db, { sourcePath: filePath, sourceHost, stat, fileHash, nowIso });
      }
      summary.sources.push({ source_host: sourceHost, vendor, source_path: filePath, status: 'scanned', indexed_count: indexed, last_export_mtime_ms: mtimeMs });
    }
  }
...
const syncHostMemories = (options = {}) => {
  ...
  for (const source of sources) {
    ...
    let fileHash = '';
    if (incremental) {
      stat = statSourceFile(source.source_path);
      if (stat) {
        let raw = '';
        try { raw = fs.readFileSync(source.source_path, 'utf8'); } catch { raw = ''; }
        fileHash = sha256(raw);
        if (sourceUnchanged(readCursor(db, source.source_path), stat, fileHash)) {
      ...
      for (const item of items) {
        ...
        if (options.dryRun !== true) {
          ...
          if (existing && (existing.status === 'superseded' || existing.status === 'rejected')) {
            // Once per (memory, content_hash): re-sync runs are frequent and the
            // skip is steady-state — re-appending every cycle grows the ledger
            // without bound and buries real events. A changed source line (new
            // hash) is new information and gets a fresh event.
        ...
        // Track every touched memory (fresh insert OR re-linked existing) so
        // ingest-time arbitration (Feature #3) is scoped to JUST the slots this
        // sync moved — never a full-store re-arbitration.
        if (options.dryRun !== true) touchedMemoryIds.add(memoryId);
        run.indexed_count += 1;
        run.linked_count += 1;
      }
      // Record the incremental cursor only after a clean parse of this source,
      // so a mid-source failure re-parses next run instead of silently skipping.
