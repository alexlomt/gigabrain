...
const getWikiReconcileReceipt = (db, operationId) => {
 ...
 * @returns {object} summary
 */
const reconcileWiki = ({ db, config, dryRun = false, faultInjector, completionFaultInjector } = {}) => {
  ...
  const completeExternalState = (result, statePayload) => {
    ...
    writeStateFile(dir, statePayload);
    return result;
  };
  const priorReceipt = getWikiReconcileReceipt(db, operationId);
  if (priorReceipt) {
    const storedSummary = JSON.parse(String(priorReceipt.summary_json || '{}'));
    const storedState = JSON.parse(String(priorReceipt.state_json || '{}'));
    const resumed = { ...storedSummary, operation_id: operationId, resumed: true };
    return dryRun ? { ...resumed, completion_deferred: true } : completeExternalState(resumed, storedState);
  }
