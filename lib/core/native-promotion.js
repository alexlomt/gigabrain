...
const validateSourcePath = (path) => {
  ...
  // The SQL below uses `?` placeholders, so injection is not a concern here;
  // we only guard against traversal and pathological lengths. A character
  // whitelist would silently drop legitimate paths containing spaces,
  // parentheses, or non-ASCII (e.g. macOS "Application Support" or "My Project").
