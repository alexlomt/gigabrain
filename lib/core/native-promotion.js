...
const promoteNativeChunks = ({
  ...
  if (!dryRun) assertConfiguredCandidateOperation({ operation: 'native.promote', config, db });
  const paths = normalizeSourcePaths(sourcePaths);
  const summary = {
    scanned_chunks: 0,
    promoted_inserted: 0,
  ...
  const apply = (batchTx) => {
  const nowIso = batchTx?.now || (now ? new Date(Date.parse(now)).toISOString() : new Date().toISOString());
  const mutationOptions = (action, memoryId, payload = {}, extra = {}) => ({
    tx: batchTx,
    faultInjector,
    event: {
      action,
      component: 'native_promotion',
      memory_id: String(memoryId),
      reason_codes: [action],
      run_id: operationId || 'native-promotion',
      timestamp: nowIso,
      payload,
      ...extra,
    },
  ...
  for (const chunk of chunks) {
    ...
    if (!eligible || !row) {
      ...
      if (row && !otherValid && String(row.source_layer || '') === 'promoted_native') {
        if (!dryRun) updateCurrentStatus(db, memoryId, 'rejected', { timestamp: nowIso }, mutationOptions(
          'native_promotion_rejected',
          memoryId,
          { chunk_id: String(chunk.chunk_id), source_path: String(chunk.source_path || '') },
        ));
      }
    ...
    if (rowNeedsRepair(row, desired)) {
      if (!dryRun) upsertCurrentMemory(db, { ...row, ...desired, clear_valid_until: true }, mutationOptions(
        'native_promotion_repaired',
        memoryId,
        { chunk_id: String(chunk.chunk_id), source_path: String(chunk.source_path || '') },
      ));
  ...
  for (const chunk of chunks) {
    ...
    if (rejectedMatch && String(rejectedMatch.status || '') !== 'active') {
      ...
      if (!dryRun) {
        upsertCurrentMemory(db, { ...rejectedMatch, ...desired }, mutationOptions(
          'native_promotion_relinked',
          String(rejectedMatch.memory_id),
          { chunk_id: String(chunk.chunk_id), source_path: String(chunk.source_path || '') },
        ));
    ...
    if (exact) {
      if (!dryRun) {
        ...
        upsertCurrentMemory(db, repaired, mutationOptions(
          'native_promotion_linked_exact',
          String(exact.memory_id || exact.id || ''),
          { chunk_id: String(chunk.chunk_id), source_path: String(chunk.source_path || '') },
        ));
    ...
    if (semantic && semantic.similarity >= Number(semantic.thresholds.auto)) {
      if (!dryRun) {
        upsertCurrentMemory(db, {
          ...
          'native_promotion_linked_semantic',
          String(semantic.row.memory_id || semantic.row.id || ''),
          { chunk_id: String(chunk.chunk_id), source_path: String(chunk.source_path || '') },
    ...
    const row = dryRun ? payload : upsertCurrentMemory(db, payload, mutationOptions(
      'native_promotion_inserted',
      memoryId,
      { chunk_id: String(chunk.chunk_id), source_path: String(chunk.source_path || '') },
    ));
  ...
  return withProjectionMutationBatch({
    db,
    now,
    operationId: operationId || `native-promotion-${paths.join('|')}`,
  }, apply).result;
};
