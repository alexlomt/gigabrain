...
const computeNativeFreshness = ({ nativeLastMs = 0, dailyLastMs = 0, nowMs = Date.now() } = {}) => {
  const nativeStale = nativeLastMs > 0 ? (nowMs - nativeLastMs) > daysToMs(STALE_WINDOWS_DAYS.native) : true;
  const rawDailyStale = dailyLastMs > 0 ? (nowMs - dailyLastMs) > daysToMs(STALE_WINDOWS_DAYS.daily) : true;
  // Daily notes are a canonical native write lane, not a heartbeat file. A stale
  // daily note is actionable only when the whole native layer is stale; otherwise
  // recent curated/native sources prove the layer is alive and there may simply
  // have been no daily-note-worthy durable write.
  const dailyNoteStale = nativeStale && rawDailyStale;
  return {
    stale: nativeStale,
    daily_note_stale: dailyNoteStale,
    daily_note_idle: rawDailyStale && !dailyNoteStale,
    daily_note_freshness_mode: dailyNoteStale ? 'stale' : (rawDailyStale ? 'optional_idle' : 'fresh'),
  };
};
...
const collectSurfaceSummary = ({ db, config, runId, outputPaths = {} }) => {
  ...
  const nativeFreshness = computeNativeFreshness({ nativeLastMs, dailyLastMs, nowMs: buildMs });
...
const toTimestampMs = (value = '') => Date.parse(String(value || '').trim()) || 0;

const toIsoFromMs = (value = 0) => (Number(value || 0) > 0 ? new Date(Number(value)).toISOString() : null);
...
const statusCountsEqual = (left = {}, right = {}) => {
  const leftCounts = normalizeStatusCounts(left);
  const rightCounts = normalizeStatusCounts(right);
  const keys = new Set([...Object.keys(leftCounts), ...Object.keys(rightCounts)]);
  for (const key of keys) {
    if (Number(leftCounts[key] || 0) !== Number(rightCounts[key] || 0)) return false;
  }
  return true;
};

const collectSurfaceDrift = ({ db, config, summary = {} } = {}) => {
  const generatedMs = toTimestampMs(summary?.freshness?.vault?.last_built_at || summary?.generated_at || '');
  const reasons = [];
  const liveStatusCounts = tableStats(db).status || {};
  const summaryStatusCounts = normalizeStatusCounts(summary?.counts?.by_status || {});
  if (!statusCountsEqual(liveStatusCounts, summaryStatusCounts)) reasons.push('status_counts_changed');

  let latestMemoryUpdatedAt = '';
  try {
    latestMemoryUpdatedAt = String(db.prepare(`
      SELECT MAX(COALESCE(updated_at, created_at, archived_at, '')) AS value
      FROM memory_current
    `).get()?.value || '');
  } catch {
    latestMemoryUpdatedAt = '';
  }
  const latestMemoryUpdatedMs = toTimestampMs(latestMemoryUpdatedAt);
  if (generatedMs > 0 && latestMemoryUpdatedMs > generatedMs + 1000) reasons.push('memory_registry_changed');

  const workspaceRoot = String(config?.runtime?.paths?.workspaceRoot || process.cwd());
  let liveActiveNodes = null;
  try {
    liveActiveNodes = collectActiveNodes({ db, config, workspaceRoot }).length;
    if (Number(summary?.active_nodes || 0) !== Number(liveActiveNodes || 0)) reasons.push('active_surface_nodes_changed');
  } catch {
    liveActiveNodes = null;
    reasons.push('active_surface_nodes_check_failed');
  }

  const queuePath = String(config?.runtime?.paths?.reviewQueuePath || '').trim();
  let reviewQueueModifiedMs = 0;
  let liveReviewQueue = null;
  if (queuePath) {
    try {
      if (fs.existsSync(queuePath)) reviewQueueModifiedMs = fs.statSync(queuePath).mtimeMs;
      liveReviewQueue = collectReviewQueue({ queuePath });
      const summaryQueue = summary?.review_queue || {};
      if (Number(liveReviewQueue.pending || 0) !== Number(summaryQueue.pending || 0)
        || Number(liveReviewQueue.total || 0) !== Number(summaryQueue.total || 0)) {
        reasons.push('review_queue_counts_changed');
      }
      if (generatedMs > 0 && reviewQueueModifiedMs > generatedMs + 1000) reasons.push('review_queue_file_changed');
    } catch {
      reasons.push('review_queue_check_failed');
    }
  }
  ...
  return {
    ...
    live_status_counts: normalizeStatusCounts(liveStatusCounts),
    summary_status_counts: summaryStatusCounts,
    live_active_nodes: liveActiveNodes,
    summary_active_nodes: Number(summary?.active_nodes || 0),
    latest_memory_updated_at: latestMemoryUpdatedAt || null,
    review_queue_path: queuePath || null,
    review_queue_modified_at: toIsoFromMs(reviewQueueModifiedMs),
    live_review_queue: liveReviewQueue ? {
      total: Number(liveReviewQueue.total || 0),
      pending: Number(liveReviewQueue.pending || 0),
      by_reason: liveReviewQueue.by_reason || {},
    } : null,
    summary_review_queue: summary?.review_queue ? {
      total: Number(summary.review_queue.total || 0),
      pending: Number(summary.review_queue.pending || 0),
      by_reason: summary.review_queue.by_reason || {},
    } : null,
  };
};
...
const inspectVaultHealth = ({
  ...
  if (existing) {
    ...
    const nativeFreshness = computeNativeFreshness({ nativeLastMs, dailyLastMs });
    const ageStale = buildLastMs > 0 ? (Date.now() - buildLastMs) > daysToMs(STALE_WINDOWS_DAYS.vault) : true;
    let drift = { stale: false, reasons: [] };
    const context = openContextDb({ db, dbPath, config });
    try {
      drift = collectSurfaceDrift({ db: context.db, config, summary: existing });
    } catch (error) {
      drift = {
        stale: true,
        reasons: ['surface_drift_check_failed'],
    ...
    return {
      ...
      vault: {
        ...
        stale: ageStale || drift.stale === true,
        age_stale: ageStale,
        drift,
      },
      manual_protection: {
  ...
  try {
    ...
    return {
      ...
      vault: {
        ...
        age_stale: buildLastMs > 0 ? (Date.now() - buildLastMs) > daysToMs(STALE_WINDOWS_DAYS.vault) : true,
        drift: { stale: false, reasons: [] },
      },
      manual_protection: {
        ok: manualIssues.length === 0,
        issues: manualIssues,
