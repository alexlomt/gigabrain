...
const applyReplacement = ({
  ...
  // Integrity hardening: new-row upsert, old-row supersession, and the
  // ledger event must be atomic — a crash between the autocommitted steps
  // left TWO active rows for one fact, or a projection change without its
  // ledger trail.
  db.exec('SAVEPOINT apply_replacement');
  try {
  ...
  db.exec('RELEASE SAVEPOINT apply_replacement');
  } catch (error) {
    try {
      db.exec('ROLLBACK TO SAVEPOINT apply_replacement');
      db.exec('RELEASE SAVEPOINT apply_replacement');
...
const applyReinstate = ({
  ...
  updateCurrentStatus(db, targetRow.memory_id, 'active', {
