...
import { isNativeMemoryLockHeld, withNativeMemoryLockSync, writeNativeMemoryEntry } from './native-memory.js';
...
const VALID_ACTIONS = new Set(['remember', 'update', 'replace', 'forget', 'protect', 'do_not_store', 'reinstate']);
...
const inferTypeFromContent = (content) => {
  ...
  if (/\b(?:user|owner|quinn)\s+(?:likes?|loves?|prefers?|dislikes?|hates?)\b/i.test(text)) return 'PREFERENCE';
...
const isTrustedActionScope = (rowScope, eventScope) => (
  normalizeAgentScope(rowScope) === normalizeAgentScope(eventScope)
);
...
const VALID_ACTION_RE = /^(remember|update|replace|forget|protect|do_not_store|reinstate)$/;
...
const resolveActionTarget = ({ existingRows = [], action = {}, scope = '', sessionKey = '' } = {}) => {
  const scopedRows = existingRows.filter((row) => isTrustedActionScope(row.scope, scope));
  const targetMemoryId = String(action.target_memory_id || '').trim();
  if (targetMemoryId) {
    const direct = scopedRows.find((row) => String(row.memory_id || row.id || '') === targetMemoryId);
    if (direct) {
      return {
        matched: true,
        ambiguous: false,
...
const withMemoryActionMutation = ({ db, tx, operationId, now }, fn) => {
  if (tx) {
    if (tx.db !== db) throw new Error('PROJECTION_TRANSACTION_DB_MISMATCH');
    return fn(tx);
  }
  return withProjectionMutationBatch({ db, operationId, now }, fn).result;
};
...
const applyReplacement = ({
  ...
  const nowIso = tx?.now || new Date().toISOString();
  ...
  withMemoryActionMutation({
    db,
    tx,
    operationId: runId || `memory-action-${action.action}-${targetRow.memory_id}`,
    now: nowIso,
  }, (batchTx) => {
    mutateCurrentMemoryWithLegacyProjection({
      tx: batchTx,
    ...
    mutateCurrentMemoryWithLegacyProjection({
      ...
      event: {
        timestamp: nowIso,
        component: 'capture',
        action: 'memory_action_supersede',
        reason_codes: ['memory_action'],
        memory_id: String(targetRow.memory_id),
        cleanup_version: String(config?.runtime?.cleanupVersion || 'v3.0.0'),
        run_id: runId || '',
        review_version: reviewVersion || '',
        matched_memory_id: newMemoryId,
        payload: { requested_action: action.action, replacement_memory_id: newMemoryId },
      },
    });
  });
...
const applyForget = ({
  ...
  withMemoryActionMutation({
    db,
    tx,
    operationId: runId || `memory-action-forget-${targetRow.memory_id}`,
    now: nowIso,
  }, (batchTx) => mutateCurrentMemoryWithLegacyProjection({
    tx: batchTx,
    faultInjector,
    mutation: {
      kind: 'status',
      memoryId: targetRow.memory_id,
      status: 'rejected',
      extra: { timestamp: nowIso, last_reviewed_at: nowIso },
    },
    event: {
      timestamp: nowIso,
      component: 'capture',
      ...
      run_id: runId || '',
      review_version: reviewVersion || '',
      payload: { requested_action: action.action },
    },
  }));
  return {
    applied: 1,
...
const applyProtect = ({
  ...
  withMemoryActionMutation({
    db,
    tx,
    operationId: runId || `memory-action-protect-${targetRow.memory_id}`,
    now: nowIso,
  }, (batchTx) => mutateCurrentMemoryWithLegacyProjection({
    tx: batchTx,
    faultInjector,
    mutation: { kind: 'upsert', memory: { ...current, tags, updated_at: nowIso } },
    event: {
      timestamp: nowIso,
      component: 'capture',
...
// Operator repair path for an arbiter over-merge: a
// falsely-superseded row is reinstated by appending an `operator:reinstate`
// event (never rewriting history) and clearing the supersession on the
// projection so recall stops suppressing it. Bi-temporal guards are respected —
// updateCurrentStatus clears superseded_by AND reopens valid_until so the row
// is live for recall again. Idempotent: an already-active target is a no-op.
const applyReinstate = ({
  db,
  config,
  action,
  targetRow,
  ...
  const nowIso = tx?.now || new Date().toISOString();
  const reason = String(action.reason || action.content || '').trim() || null;
  if (String(targetRow.status || '').toLowerCase() === 'active') {
    appendEvent(db, {
      timestamp: nowIso,
      component: 'operator_repair',
      action: 'memory_action_noop',
      reason_codes: ['action_noop', 'reinstate_already_active'],
      memory_id: String(targetRow.memory_id),
      cleanup_version: String(config?.runtime?.cleanupVersion || 'v3.0.0'),
      run_id: runId || '',
      review_version: reviewVersion || '',
      payload: {
        requested_action: 'reinstate',
        target_memory_id: String(targetRow.memory_id),
        reason,
      },
    });
    logger?.info?.(`[gigabrain] reinstate no-op target=${targetRow.memory_id} already active`);
    return {
      noop: 1,
      already_active: 1,
  ...
  const previouslySupersededBy = targetRow.superseded_by ? String(targetRow.superseded_by) : null;
  const priorSupersede = db.prepare(`
    SELECT event_id FROM memory_events
    WHERE action = 'arbiter:supersede' AND memory_id = ?
    ORDER BY timestamp DESC, rowid DESC
    LIMIT 1
  `).get(String(targetRow.memory_id));

  // Reopen the row: active status, superseded_by cleared, valid_until reopened
  // (a superseded loser gets valid_until=verdict-instant; without clearing it
  // the reinstated row would come back active but permanently recall-dead).
  withMemoryActionMutation({
    db,
    tx,
    operationId: runId || `memory-action-reinstate-${targetRow.memory_id}`,
    now: nowIso,
  }, (batchTx) => mutateCurrentMemoryWithLegacyProjection({
    tx: batchTx,
    faultInjector,
    ...
    event: {
      timestamp: nowIso,
      component: 'operator_repair',
      action: 'operator:reinstate',
      reason_codes: ['memory_action', 'operator_reinstate'],
      memory_id: String(targetRow.memory_id),
  ...
  logger?.info?.(`[gigabrain] operator reinstate target=${targetRow.memory_id} was_superseded_by=${previouslySupersededBy}`);
  return {
    applied: 1,
    reinstated: 1,
  };
...
const applyMemoryActionsInNativeLock = ({
  db,
  config,
  event = {},
  actions = [],
  ...
  const listScopedActiveRows = () => listCurrentMemories(db, { statuses: ['active'], limit: 10000 })
    .filter((row) => isTrustedActionScope(row.scope, scope));
  let existingRows = listScopedActiveRows();
  ...
  for (const action of relevantActions) {
    ...
    // Reinstate resolves its target BY ID against the full projection — the
    // falsely-superseded row is not `active`, so resolveActionTarget (active-only)
    // would never find it. A missing id/row falls through to the review queue,
    // mirroring the generic unmatched-target path.
    if (action.action === 'reinstate') {
      const reinstateTargetId = String(action.target_memory_id || '').trim();
      const reinstateRow = reinstateTargetId ? getCurrentMemory(db, reinstateTargetId) : null;
      if (!reinstateRow || !isTrustedActionScope(reinstateRow.scope, scope)) {
        ...
        appendEvent(db, {
          ...
          payload: {
            requested_action: action.action,
            reason: reinstateTargetId ? 'reinstate_target_missing' : 'reinstate_missing_target_id',
            target_memory_id: action.target_memory_id || null,
          },
        });
        continue;
      }
      const reinstateResult = applyReinstate({
        db,
        config,
        action,
        targetRow: reinstateRow,
        runId,
        reviewVersion,
        logger,
        tx,
      ...
      summary.applied += Number(reinstateResult.applied || 0);
      summary.reinstated += Number(reinstateResult.reinstated || 0);
      summary.noop += Number(reinstateResult.noop || 0);
      if (reinstateResult.reinstated) {
        existingRows = listScopedActiveRows();
      }
      continue;
    }
...
const applyMemoryActions = (options = {}) => {
  const needsNativeLock = Array.isArray(options.actions)
    && options.actions.some((action) => ['replace', 'update'].includes(String(action?.action || '').toLowerCase()));
  if (!needsNativeLock) return applyMemoryActionsInNativeLock(options);
  if ((options.tx || options.db?.isTransaction) && !isNativeMemoryLockHeld(options.config)) {
    throw new Error('GIGABRAIN_NATIVE_LOCK_ORDER: memory action transaction requires the outer native-memory lock');
  }
  return withNativeMemoryLockSync(options.config, () => applyMemoryActionsInNativeLock(options));
};
