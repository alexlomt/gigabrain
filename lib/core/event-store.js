...
const ensureEventStore = (db) => {
  ...
  `);
  // Provenance/trust key for arbitration ledger events. Null-safe default so
  // existing rows and non-arbiter events stay valid (R6: agent_id now).
  ensureColumn(db, 'memory_events', 'agent_id', 'TEXT');
};
...
const buildEvent = (event = {}, defaults = {}) => {
  ...
  return {
    ...
    agent_id: (event.agent_id ?? defaults.agent_id) ? String(event.agent_id ?? defaults.agent_id) : null,
    payload,
  };
};
