...
import { parseNativeMetadata, stripNativeMetadata } from '../compat/native-metadata.js';
...
const stripScopeComment = (value = '') => {
  ...
  return {
    scope: parsed.scope || normalizeText(match?.[1]),
    text: normalizeText(stripNativeMetadata(raw)),
  };
};
...
const stripLegacyPrefix = (kind, value = '') => {
  ...
  if (kind === 'summary') {
    return normalizeText(text.replace(/^(?:Codex App|Claude|OpenClaw|Agent) session(?: \([^)]*\))?:\s*/i, ''));
  }
...
const parseLegacyCheckpointMarkdown = (text = '', options = {}) => {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!bullet) continue;
    const metadata = parseNativeMetadata(bullet[1]);
    if (metadata.originKind === 'structured_checkpoint' || metadata.originKind === 'human_native') continue;
    if (/\btype=/.test(bullet[1]) && !metadata.type) continue;
