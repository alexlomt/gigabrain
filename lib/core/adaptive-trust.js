...
const computeAdaptiveTrustTargets = ({ db, config, now = new Date().toISOString() } = {}) => {
...
const runAdaptiveTrust = ({ db, config, now = new Date().toISOString(), dryRun = false } = {}) => {
  if (!db) throw new Error('runAdaptiveTrust requires db');
  ensureAdaptiveTrustStore(db);
  const { hosts, fingerprint, settings } = computeAdaptiveTrustTargets({ db, config, now });
