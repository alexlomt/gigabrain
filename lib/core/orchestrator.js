...
const PR_LOOKUP_RE = /\b(?:pr|pull\s+request)\s*#?\d{3,}\b|#\d{4,}\b/i;
...
const classifyQueryIntent = (query = '') => {
  ...
  if (PR_LOOKUP_RE.test(text)) {
    return { strategy: 'verification_lookup', requiresDeepLookup: true, reason: 'pr_lookup' };
  }
...
const promoteQuickContextEntityBrief = ({
  ...
  if (ENTITY_PREFERENCE_QUERY_RE.test(text)) {
    return { strategy: 'entity_brief', reason: 'entity_preference' };
  }
  if (['project', 'organization', 'place'].includes(kind)) {
    return { strategy: 'entity_brief', reason: 'entity_definition' };
  }
  return { strategy: baseStrategy, reason: '' };
};
...
const orchestrateRecall = ({
  ...
  const effectiveScope = requestedScope
    || (config?.orchestrator?.profileFirst !== false && String(config?.runtime?.paths?.workspaceRoot || '').trim()
      ? 'profile:main'
      : 'shared');
  ...
  if (isMultiEntityStrategy) {
    ...
    for (const match of multiMatches) {
      const detail = getEntityDetail(db, match.entity_id, { scope: effectiveScope });
    ...
    lockedEntityMatch = primaryEntityMatch && Number(primaryEntityMatch._orchestrator_score || primaryEntityMatch.score || 0) >= entityLockMinScore
      ? primaryEntityMatch
      : null;
    topEntity = lockedEntityMatch ? getEntityDetail(db, lockedEntityMatch.entity_id, { scope: effectiveScope }) : null;
  }
