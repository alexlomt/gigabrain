...
const releaseProvenanceSchema = z.object({
  ...
  code_sha: z.string(),
  immutable_tag: z.string(),
  schema_id: z.string(),
  schema_checksum: z.string(),
  payload_root: z.string(),
  manifest_sha256: z.string(),
  dependency_root: z.string(),
  local_integrity_root: z.string(),
});
...
const doctorOutputSchema = {
  ...
  release: releaseProvenanceSchema.nullable(),
...
const createMcpServer = (defaults = {}) => {
  ...
  const assertMcpWrite = async (suffix, target = 'project') => {
    const { loadCodexContext } = await loadServices();
    const context = loadCodexContext(defaults);
    const config = target === 'user' ? context.userConfig : context.projectConfig;
    assertWriteAllowed({
      mode: resolveWriteMode(config),
      operation: `mcp.${remoteProfile ? 'remote' : 'local'}.${suffix}`,
    });
  };
  ...
  server.registerTool('gigabrain_remember', {
    ...
    await assertMcpWrite('remember', args.target === 'user' ? 'user' : 'project');
  ...
  server.registerTool('gigabrain_checkpoint', {
    ...
    await assertMcpWrite('checkpoint');
  ...
  server.registerTool('gigabrain_claim_propose', {
    ...
    await assertMcpWrite('claim_propose');
  ...
  server.registerTool('gigabrain_claim_decide', {
    ...
    await assertMcpWrite('claim_decide');
  ...
  server.registerTool('gigabrain_receipt_write', {
    ...
    await assertMcpWrite('receipt_write');
  ...
  server.registerTool('gigabrain_arbitrate', {
    ...
    annotations: additiveWriteAnnotations('Arbitrate conflict'),
    inputSchema: {
    ...
    await assertMcpWrite('arbitrate');
