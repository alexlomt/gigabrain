...
import { promoteNativeChunks, reconcilePromotedNativeRows } from './native-promotion.js';
...
import { rejectTestArtifacts } from './hygiene-migration.js';
import {
...
const activeNativeLinkIds = (db) => {
  try {
    return new Set(db.prepare(`
      SELECT DISTINCT linked_memory_id AS memory_id
      FROM memory_native_chunks
      WHERE status = 'active'
        AND COALESCE(linked_memory_id, '') <> ''
    `).all().map((row) => String(row.memory_id || '')).filter(Boolean));
  } catch {
    return new Set();
  }
};

const sourceLayerRank = (row = {}) => {
  const layer = String(row.source_layer || 'registry').trim();
  if (layer === 'native') return 3;
  if (layer === 'promoted_native') return 2;
  if (layer === 'registry') return 1;
  return 0;
};

const duplicatePriorityVector = (row = {}, nativeLinkIds = new Set()) => {
  const memoryId = String(row.memory_id || '');
  const confidence = Number.isFinite(Number(row.confidence)) ? Number(row.confidence) : 0;
  const valueScore = Number.isFinite(Number(row.value_score)) ? Number(row.value_score) : 0;
  const updated = Date.parse(String(row.updated_at || row.created_at || '')) || 0;
  return [
    isProtectedMemory(row) ? 1 : 0,
    nativeLinkIds.has(memoryId) ? 1 : 0,
    sourceLayerRank(row),
    confidence,
    valueScore,
    updated,
...
const compareDuplicatePriority = (a = {}, b = {}, nativeLinkIds = new Set()) => {
  const av = duplicatePriorityVector(a, nativeLinkIds);
  const bv = duplicatePriorityVector(b, nativeLinkIds);
...
const writeArchiveCompression = ({
  ...
  const summaryDir = archiveSummaryDir || path.join(workspaceRoot, 'memory');
  const filePath = path.join(summaryDir, `${prefix}-${new Date().toISOString().slice(0, 10)}.md`);
  ensureFileDir(filePath);
  const lines = [];
...
const pickSemanticQueuePair = (a, b, nativeLinkIds = new Set()) => {
  const sorted = [a, b].sort((left, right) => compareDuplicatePriority(left, right, nativeLinkIds));
...
const buildSemanticReviewQueueRow = ({ winner, loser, similarity, nowIso }) => ({
  ...
  payload: {
    memory_id: String(winner.memory_id || ''),
    matched_memory_id: String(loser.memory_id || ''),
    winner_memory_id: String(winner.memory_id || ''),
    loser_memory_id: String(loser.memory_id || ''),
    content: summarizeExcerpt(winner.content || winner.normalized || ''),
    matched_content: summarizeExcerpt(loser.content || loser.normalized || ''),
...
const runMaintenance = ({
  ...
  const eventCounts = {
    ...
    native_promoted_relinked_rows: 0,
    native_promoted_relinked_chunks: 0,
    native_promoted_rejected_orphaned: 0,
    test_artifacts_rejected: 0,
  ...
  try {
    ...
    const nativeReconcileSummary = reconcilePromotedNativeRows({
      db,
      config,
      dryRun,
      now: nowIso,
    });
    eventCounts.native_promoted_relinked_rows = Number(nativeReconcileSummary.relinked_rows || 0);
    eventCounts.native_promoted_relinked_chunks = Number(nativeReconcileSummary.relinked_chunks || 0);
    eventCounts.native_promoted_rejected_orphaned = Number(nativeReconcileSummary.rejected_orphaned || 0);
    eventCounts.native_promoted_rejected_policy = Number(nativeReconcileSummary.rejected_policy || 0);
    emit({
      timestamp: nowIso,
      component: 'maintenance',
      action: 'native_promotion_reconcile',
      reason_codes: [
        eventCounts.native_promoted_relinked_rows > 0
          || eventCounts.native_promoted_rejected_orphaned > 0
          || eventCounts.native_promoted_rejected_policy > 0
          ? 'complete'
          : 'noop',
      ],
      memory_id: `run:${resolvedRunId}`,
      payload: nativeReconcileSummary,
    });

    const testArtifactSummary = rejectTestArtifacts({
      db,
      apply: !dryRun,
      nowIso,
      runId: resolvedRunId,
    ...
    eventCounts.test_artifacts_rejected = Number(testArtifactSummary.changed || 0);
    if (eventCounts.test_artifacts_rejected > 0) {
      for (const memoryId of testArtifactSummary.memoryIds || []) {
    ...
    emit({
      timestamp: nowIso,
      component: 'maintenance',
      action: 'test_artifact_hygiene',
      reason_codes: [testArtifactSummary.candidates > 0 ? 'complete' : 'noop'],
      memory_id: `run:${resolvedRunId}`,
      payload: testArtifactSummary,
    });
    ...
    withTransaction(db, () => {
      ...
      const nativeLinkIds = activeNativeLinkIds(db);
      ...
      for (const list of groups.values()) {
        ...
        const sorted = [...list].sort((a, b) => compareDuplicatePriority(a, b, nativeLinkIds));
    ...
    withTransaction(db, () => {
      ...
      for (let i = 0; i < rows.length; i += 1) {
        ...
        for (let j = i + 1; j < rows.length; j += 1) {
          const b = rows[j];
          if (!scopesCanDedupe(a.scope, b.scope)) continue;
          ...
          if (similarity >= Number(semanticThresholds.auto)) {
            const { winner, loser } = pickSemanticQueuePair(a, b, nativeLinkIds);
            if (isProtectedMemory(loser)) continue;
          ...
          try {
            ...
            if (isPendingSemantic && Number.isFinite(rowTs) && rowTs < cutoffMs) {
              ...
              const winnerRow = winnerId
                ? db.prepare('SELECT memory_id, type, scope, status, source_layer, content, normalized, confidence, value_score, created_at, updated_at, tags FROM memory_current WHERE memory_id = ?').get(winnerId)
                : null;
              const loserRow = loserId
                ? db.prepare('SELECT memory_id, type, scope, status, source_layer, content, normalized, confidence, value_score, created_at, updated_at, tags FROM memory_current WHERE memory_id = ?').get(loserId)
                : null;
              ...
              if (pairStillActive) {
                const currentPair = pickSemanticQueuePair(winnerRow, loserRow, nativeLinkIds);
    ...
    const archiveSummaryPath = writeArchiveCompression({
      ...
      runId: resolvedRunId,
      dryRun,
      archiveSummaryDir: path.dirname(String(config?.maintenance?.usageLogPath || path.join(workspaceRoot, 'memory', 'usage-log.md'))),
    });
    ...
    let executionArtifactPath = writeExecutionArtifact({
      ...
      payload: {
        ...
        complete: false,
        artifact_phase: 'pre_vault_build',
        sequence: DAILY_SEQUENCE,
        cleanup_version: cleanupVersion,
        dry_run: dryRun,
        counts: eventCounts,
    ...
    executionArtifactPath = writeExecutionArtifact({
      ...
      payload: {
        ...
        trigger_source: 'gigabrainctl-nightly',
        complete: false,
        artifact_phase: 'post_vault_build',
        sequence: DAILY_SEQUENCE,
        cleanup_version: cleanupVersion,
        dry_run: dryRun,
        counts: eventCounts,
    ...
    if (!dryRun && vaultBuildSummary.mirror_root && config?.vault?.reports?.enabled !== false) {
      copyIfExists(
        executionArtifactPath,
        path.join(vaultBuildSummary.mirror_root, '40 Reports', 'nightly-execution-latest.json'),
      );
    }
