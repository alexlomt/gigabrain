...
const runMaintenance = ({
  ...
  try {
    ...
    const runMutationPhase = (phase, fn) => {
      ...
      `).all(resolvedPhaseOperationId).map((row) => ({
        ...row,
          ...
          if (!dryRun) {
            updateCurrentStatus(db, row.memory_id, 'archived', {
              ...
              event: {
                ...
                run_id: resolvedRunId,
                review_version: String(reviewVersion || ''),
                payload: { score: result.value_score, label: result.value_label },
              },
            });
            recordRowEvent();
          }
          ...
          if (!dryRun) {
            updateCurrentStatus(db, loser.memory_id, 'archived', {
              ...
              event: {
                ...
                run_id: resolvedRunId,
                review_version: String(reviewVersion || ''),
                payload: { winner_id: String(winner.memory_id) },
              },
            });
            recordRowEvent();
          }
    ...
    if (autoResolveDays > 0 && queuePath) {
      const autoResolveOperationId = phaseOperationId('auto_resolve_dedupe');
      try {
        ...
        for (const line of queueLines) {
          try {
            ...
            if (isPendingSemantic && Number.isFinite(rowTs) && rowTs < cutoffMs) {
              ...
              `).get(loserId, winnerId, autoResolveOperationId) : null;
              if (applied) {
                let appliedPayload = {};
