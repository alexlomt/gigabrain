  ...
  try {
    ...
    const runMutationPhase = (phase, fn) => {
      ...
      const result = withProjectionMutationBatch({
        db,
        now: nowIso,
        operationId: `${operationId || resolvedRunId}:${phase}`,
      }, (tx) => fn(tx, () => {
        if (tx.lastMutationEvent) committedEvents.push(tx.lastMutationEvent);
      })).result;
      for (const row of committedEvents) appendJsonl(eventsPath, row);
      return result;
    };
