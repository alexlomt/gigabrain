      ...
      properties: {
        ...
        autoCapture: {
          ...
          properties: {
            ...
            provider: { type: 'string', enum: ['openclaw', 'openai_compatible', 'none'], default: 'none' },
            baseUrl: { type: 'string', default: '' },
            model: { type: 'string', default: '' },
            apiKey: { type: 'string', default: '' },
            profile: { type: 'string', default: 'auto_capture' },
            timeoutMs: { type: 'integer', default: 45000 },
            ...
            minContentChars: { type: 'integer', default: 20 },
            maxCandidates: { type: 'integer', default: 5 },
            maxTurns: { type: 'integer', default: 16 },
            maxCharsPerTurn: { type: 'integer', default: 4000 },
            targetTokens: { type: 'integer', default: 10000 },
            softMaxTokens: { type: 'integer', default: 15000 },
            hardMaxTokens: { type: 'integer', default: 25000 },
            includeExistingMemories: { type: 'boolean', default: true },
            existingMemoryLimit: { type: 'integer', default: 80 },
            minTriggerChars: { type: 'integer', default: 80 },
            processingStaleMs: { type: 'integer', default: 300000 },
          },
        },
      },
    },
...
const normalizeOperatorRules = (value = {}) => {
  ...
  const entity = isObject(raw.entity) ? raw.entity : {};
  const memoryTier = isObject(raw.memoryTier) ? raw.memoryTier : {};
  const surface = isObject(raw.surface) ? raw.surface : {};
  const sessionBrief = isObject(raw.sessionBrief) ? raw.sessionBrief : {};
  return {
    entity: {
      rejectTerms: normalizeUniqueStringArray(entity.rejectTerms),
      nonPersonTerms: normalizeUniqueStringArray(entity.nonPersonTerms),
    ...
    memoryTier: {
      tierValues: normalizeUniqueStringArray(memoryTier.tierValues),
      durableTiers: normalizeUniqueStringArray(memoryTier.durableTiers),
    ...
    surface: {
      ...
      personCueTerms: normalizeUniqueStringArray(surface.personCueTerms),
      projectCueTerms: normalizeUniqueStringArray(surface.projectCueTerms),
    },
    sessionBrief: {
  ...
  const out = {
    ...
    capture: {
      ...
      autoCapture: {
        ...
        minContentChars: clampInt(merged?.capture?.autoCapture?.minContentChars, 1, 500, 20),
        ...
        processingStaleMs: clampInt(merged?.capture?.autoCapture?.processingStaleMs, 1000, 86400000, 300000),
      },
    },
    dedupe: {
      ...merged.dedupe,
    ...
    recall: {
      ...
      ...(Object.hasOwn(rawConfig?.recall || {}, 'embeddingDimensions')
        ? { embeddingDimensions: Number(rawConfig.recall.embeddingDimensions) }
        : {}),
    ...
    memoryLlm: {
      ...
      provider: ['openai_compatible', 'ollama', 'none'].includes(String(merged?.memoryLlm?.provider || '').trim().toLowerCase())
      ...
      apiKeyEnv: String(merged?.memoryLlm?.apiKeyEnv || '').trim(),
