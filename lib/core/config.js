...
const DEFAULT_CONFIG = Object.freeze({
  ...
  memoryLlm: {
    ...
    model: '',
    apiKey: '',
    apiKeyEnv: 'GIGABRAIN_MEMORY_LLM_API_KEY',
    timeoutMs: 15000,
    maxRetries: 1,
  },
  orchestrator: {
  ...
  llm: {
    ...
    model: '',
    apiKey: '',
    apiKeyEnv: 'GIGABRAIN_LLM_API_KEY',
    timeoutMs: 12000,
    taskProfiles: {
  ...
  properties: {
      ...
      properties: {
        ...
        autoCapture: {
          ...
          properties: {
            ...
            provider: { type: 'string', enum: ['openclaw', 'openai_compatible', 'none', ''], default: '' },
            baseUrl: { type: 'string', default: '' },
            model: { type: 'string', default: '' },
            apiKey: { type: 'string', default: '' },
            profile: { type: 'string', default: 'auto_capture' },
            timeoutMs: { type: 'number', default: 45000 },
            processingStaleMs: { type: 'number', default: 900000 },
            minConfidence: { type: 'number', default: 0.88 },
            minImportance: { type: 'number', default: 0.75 },
            queueMinConfidence: { type: 'number', default: 0.65 },
            queueMinImportance: { type: 'number', default: 0.5 },
            minContentChars: { type: 'number', default: 20 },
            maxCandidates: { type: 'number', default: 5 },
            maxTurns: { type: 'number', default: 16 },
            maxCharsPerTurn: { type: 'number', default: 4000 },
            targetTokens: { type: 'number', default: 10000 },
            softMaxTokens: { type: 'number', default: 15000 },
            hardMaxTokens: { type: 'number', default: 25000 },
            includeExistingMemories: { type: 'boolean', default: true },
            existingMemoryLimit: { type: 'number', default: 80 },
            minTriggerChars: { type: 'number', default: 80 },
          },
        },
        rememberIntent: {
          type: 'object',
    ...
    recall: {
      ...
      properties: {
        ...
        embeddingProvider: { type: 'string', enum: ['ollama', 'openai_compatible'], default: 'ollama' },
    ...
    memoryLlm: {
      ...
      properties: {
        enabled: { type: 'boolean', default: false },
        provider: { type: 'string', enum: ['openai_compatible', 'ollama', 'none'], default: 'none' },
        baseUrl: { type: 'string', default: '' },
        model: { type: 'string', default: '' },
        apiKey: { type: 'string', default: '' },
        apiKeyEnv: { type: 'string', default: 'GIGABRAIN_MEMORY_LLM_API_KEY' },
    ...
    llm: {
      ...
      properties: {
        ...
        model: { type: 'string' },
        apiKey: { type: 'string' },
        apiKeyEnv: { type: 'string', default: 'GIGABRAIN_LLM_API_KEY' },
    ...
    native: {
      ...
      properties: {
        ...
        includeFiles: {
          type: 'array',
          items: { type: 'string' },
          default: ['memory/latest.md', 'memory/recent-changes.md', 'memory/whois.md', 'memory/pinned/core-people.md', 'memory/pinned/peer-agents.md', 'memory/gigabrain-harmonized.md'],
        },
        excludeGlobs: {
          type: 'array',
          items: { type: 'string' },
...
const normalizeConfig = (rawConfig = {}, options = {}) => {
  ...
  const llmApiKeyEnv = String(merged?.llm?.apiKeyEnv || 'GIGABRAIN_LLM_API_KEY').trim() || 'GIGABRAIN_LLM_API_KEY';
  const llmApiKey = String(
    merged?.llm?.apiKey
    || (llmApiKeyEnv ? process.env[llmApiKeyEnv] : '')
    || '',
  ).trim();
  ...
  const out = {
    ...
    capture: {
      ...
      autoCapture: {
        ...
        provider: ['openclaw', 'openai_compatible', 'none', ''].includes(String(merged?.capture?.autoCapture?.provider ?? '').trim().toLowerCase())
        ...
        timeoutMs: clampInt(merged?.capture?.autoCapture?.timeoutMs, 1000, 120000, 45000),
        minConfidence: clamp01(merged?.capture?.autoCapture?.minConfidence ?? 0.88),
        minImportance: clamp01(merged?.capture?.autoCapture?.minImportance ?? 0.75),
        queueMinConfidence: clamp01(merged?.capture?.autoCapture?.queueMinConfidence ?? 0.65),
        queueMinImportance: clamp01(merged?.capture?.autoCapture?.queueMinImportance ?? 0.5),
        minContentChars: Math.max(1, Math.min(500, Number(merged?.capture?.autoCapture?.minContentChars ?? 20) || 20)),
        maxCandidates: clampInt(merged?.capture?.autoCapture?.maxCandidates, 0, 20, 5),
        maxTurns: clampInt(merged?.capture?.autoCapture?.maxTurns, 2, 32, 16),
        maxCharsPerTurn: clampInt(merged?.capture?.autoCapture?.maxCharsPerTurn, 500, 12000, 4000),
        targetTokens: clampInt(merged?.capture?.autoCapture?.targetTokens, 1000, 50000, 10000),
        softMaxTokens: clampInt(merged?.capture?.autoCapture?.softMaxTokens, 1000, 75000, 15000),
        hardMaxTokens: clampInt(merged?.capture?.autoCapture?.hardMaxTokens, 1000, 100000, 25000),
        includeExistingMemories: merged?.capture?.autoCapture?.includeExistingMemories !== false,
        existingMemoryLimit: clampInt(merged?.capture?.autoCapture?.existingMemoryLimit, 0, 200, 80),
    ...
    recall: {
      ...
      embeddingProvider: ['ollama', 'openai_compatible'].includes(String(merged?.recall?.embeddingProvider || '').trim().toLowerCase())
        ? String(merged?.recall?.embeddingProvider).trim().toLowerCase()
        : (String(merged?.recall?.embeddingBaseUrl || '').trim() ? 'openai_compatible' : 'ollama'),
      embeddingBaseUrl: String(merged?.recall?.embeddingBaseUrl || merged?.recall?.ollamaUrl || '').trim(),
    ...
    memoryLlm: {
      ...
      provider: ['openai_compatible', 'ollama', 'none'].includes(String(merged?.memoryLlm?.provider || 'none').trim().toLowerCase())
        ? String(merged?.memoryLlm?.provider || 'none').trim().toLowerCase()
        : 'none',
      baseUrl: String(merged?.memoryLlm?.baseUrl || '').trim(),
      model: String(merged?.memoryLlm?.model || '').trim(),
      apiKey: String(merged?.memoryLlm?.apiKey || '').trim(),
      apiKeyEnv: String(merged?.memoryLlm?.apiKeyEnv || 'GIGABRAIN_MEMORY_LLM_API_KEY').trim() || 'GIGABRAIN_MEMORY_LLM_API_KEY',
    ...
    llm: {
      ...
      provider: ['openclaw', 'openai_compatible', 'ollama', 'none'].includes(String(merged?.llm?.provider || 'none'))
        ...
        : 'none',
      baseUrl: String(merged?.llm?.baseUrl || '').trim(),
      model: String(merged?.llm?.model || '').trim(),
      apiKey: llmApiKey,
      ...
      queueReview: {
        ...
        limit: Math.max(0, Math.min(5000, Number(merged?.llm?.queueReview?.limit ?? 200) || 200)),
        ...
        allowedReasons: (() => {
          const list = normalizeStringArray(merged?.llm?.queueReview?.allowedReasons)
            .map((item) => item.toLowerCase());
          return list.length > 0 ? list : [...DEFAULT_CONFIG.llm.queueReview.allowedReasons];
        })(),
      },
    },
    maintenance: {
