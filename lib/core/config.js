    ...
    recall: {
      ...
      properties: {
        ...
        relevanceFloor: {
          ...
          properties: {
            enabled: { type: 'boolean', default: true },
            minMatchedTokens: { type: 'number', default: 1 },
            denseCosine: { type: 'number', default: 0.6 },
          },
        },
  ...
  const out = {
    ...
    recall: {
      ...
      relevanceFloor: {
        ...
        minMatchedTokens: Math.max(0, Number(merged?.recall?.relevanceFloor?.minMatchedTokens ?? 1) || 1),
        denseCosine: Math.max(0, Math.min(1, Number(merged?.recall?.relevanceFloor?.denseCosine ?? 0.6))),
      },
