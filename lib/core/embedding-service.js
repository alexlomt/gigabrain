/**
 * Semantic reranking service via OpenAI-compatible embeddings.
 * Defaults to Ollama on loopback, but can also target another
 * OpenAI-compatible embeddings endpoint when explicitly configured.
 * Designed as a reranker over BM25 lexical candidates -- NOT a
 ...
 * Graceful fallback: if the embedding endpoint is unreachable, candidates
...
const normalizeEmbeddingProvider = (value = '') => {
  const key = String(value || DEFAULT_EMBEDDING_PROVIDER).trim().toLowerCase();
  if (key === 'openai_compatible') return 'openai_compatible';
  return DEFAULT_EMBEDDING_PROVIDER;
};

const applyQueryInstructionTemplate = (model = '', queryText = '') => {
  const normalizedModel = String(model || '').trim().toLowerCase();
  const match = QUERY_INSTRUCTION_TEMPLATES.find(({ prefix }) => normalizedModel.startsWith(prefix));
  return match ? match.template.replace('{query}', String(queryText || '')) : queryText;
};

const buildEmbeddingEndpoint = (value = '', provider = DEFAULT_EMBEDDING_PROVIDER) => {
  ...
  try {
    ...
    parsed.pathname = normalizedPath.endsWith('/embeddings')
...
const isSafeEmbeddingBaseUrl = (value = '', provider = DEFAULT_EMBEDDING_PROVIDER) => {
  return buildEmbeddingEndpoint(value, provider) !== null;
};
...
/**
 * Fetch an embedding vector from a configured OpenAI-compatible endpoint
 * (async, uses global fetch).
 * @param {string} text
 * @param {{ provider?: string, baseUrl?: string, apiKey?: string, model?: string, timeoutMs?: number }} opts
 * @returns {Promise<number[] | null>}
 */
const getEmbedding = async (text, opts = {}) => {
  const provider = normalizeEmbeddingProvider(opts.provider);
  const baseUrl = opts.baseUrl || DEFAULT_OLLAMA_URL;
  const apiKey = String(opts.apiKey || '').trim();
  const model = opts.model || DEFAULT_MODEL;
  const timeoutMs = opts.timeoutMs || DEFAULT_TIMEOUT_MS;
  if (!isSafeEmbeddingBaseUrl(baseUrl, provider)) return null;
  const endpoint = buildEmbeddingEndpoint(baseUrl, provider);
...
const getEmbeddingSync = (text, opts = {}) => {
  ...
  try {
    ...
    if (apiKey) args.splice(5, 0, '-H', `Authorization: Bearer ${apiKey}`);
    const result = execFileSync('curl', args, { encoding: 'utf8', timeout: timeoutMs + 2000 });
 ...
 *
 * If the feature is disabled, the embedding backend is unreachable, or the query embedding
...
const semanticRerank = (candidates, query, config = {}, db = null) => {
  ...
  const alpha = typeof recall.semanticRerankAlpha === 'number'
    ...
    : DEFAULT_ALPHA;
  const provider = normalizeEmbeddingProvider(recall.embeddingProvider);
  const baseUrl = recall.embeddingBaseUrl || recall.ollamaUrl || DEFAULT_OLLAMA_URL;
  const apiKey = recall.embeddingApiKey || '';
  ...
  const queryText = applyQueryInstructionTemplate(model, query);
  const queryVec = getEmbeddingSync(queryText, { provider, baseUrl, apiKey, model, timeoutMs });
  ...
  const results = candidates.map((candidate) => {
    ...
    // Try cached embedding from DB first. Native rows use synthetic
    // `native:<chunk>` ids but often carry a linked registry memory id; use
    // that linked id so curated/native views can still participate in semantic
    // rerank without duplicating embeddings.
    if (db && (candidate.memory_id || candidate.linked_memory_id)) {
      try {
        const candidateIds = [candidate.memory_id, candidate.linked_memory_id]
          .map((id) => String(id || '').trim())
          .filter(Boolean);
        for (const candidateId of candidateIds) {
          const stored = getStoredEmbedding(db, candidateId);
          if (stored && stored.model === model && stored.dims === queryVec.length && stored.embedding.length === queryVec.length) {
            candidateVec = stored.embedding;
            break;
          }
        }
      } catch {
...
const buildMissingEmbeddings = (db, config = {}) => {
  ...
  const probe = getEmbeddingSync('gigabrain semantic probe', { provider, baseUrl, apiKey, model, timeoutMs });
  if (!probe) {
  ...
  for (const row of rows) {
    ...
    const vec = getEmbeddingSync(text, { provider, baseUrl, apiKey, model, timeoutMs });
    if (!vec) {
      failed++;
      continue;
    }
