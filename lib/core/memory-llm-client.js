...
const completeOllamaJson = async ({ prompt, profile, memoryConfig }) => {
...
const completeMemoryJson = async ({ config = {}, prompt, profile = {} } = {}) => {
  ...
  for (let attempt = 0; attempt <= memoryConfig.maxRetries; attempt += 1) {
    try {
      ...
      if (memoryConfig.provider === 'ollama') {
        return await completeOllamaJson({ prompt, profile, memoryConfig });
      }
