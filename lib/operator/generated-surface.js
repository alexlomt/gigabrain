...
const ensurePrivateDir = (root, dirPath) => {
  ...
  for (const component of ['', ...components]) {
    ...
    try {
      ...
      if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error('GENERATED_SURFACE_DIRECTORY_INVALID');
    } catch (error) {
      if (error?.code !== 'ENOENT') throw error;
      fs.mkdirSync(current, { mode: DIRECTORY_MODE });
      stat = fs.lstatSync(current);
    }
    if ((stat.mode & 0o777) !== DIRECTORY_MODE) fs.chmodSync(current, DIRECTORY_MODE);
  }
};
