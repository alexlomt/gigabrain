...
const readStableReleaseFile = ({ root, relativePath, maxBytes, codes, requireCanonicalMode = false }) => {
  ...
  let pathBefore;
  try { pathBefore = fs.lstatSync(absolutePath, { bigint: true }); } catch {
    throw provenanceError(codes.missing, relativePath);
  }
  if (pathBefore.isSymbolicLink()) throw provenanceError(codes.symlink, relativePath);
  if (!pathBefore.isFile()) throw provenanceError(codes.type, relativePath);
  if (pathBefore.nlink !== 1n) throw provenanceError(codes.hardlink, relativePath);
  if (requireCanonicalMode) requireCanonicalFileMode(pathBefore, codes.mode, relativePath);
  if (Number(maxBytes) > 0 && pathBefore.size > BigInt(maxBytes)) {
  ...
  let data;
  try {
    handle = fs.openSync(absolutePath, fs.constants.O_RDONLY | NO_FOLLOW);
    ...
    if (!sameStableStat(pathBefore, opened)) throw provenanceError(codes.unstable, relativePath);
    ...
    if (error?.code === "ENOENT" && pathBefore) throw provenanceError(codes.unstable, relativePath);
    throw error;
  } finally {
  ...
  if (!sameStableStat(pathBefore, opened)
    || !sameStableStat(opened, afterRead)
