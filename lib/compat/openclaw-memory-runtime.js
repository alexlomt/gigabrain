...
const readEntityDocument = (db, id, visibleScopes) => {
  ...
  const entity = db.prepare(`
    SELECT entity_id, kind, display_name, normalized_name, confidence, aliases, created_at, updated_at
  ...
  let mentions = [];
  if (tableExists(db, "memory_entity_mentions")) {
    const mentionScope = visibleScopePredicate(visibleScopes, "mention.scope");
    const backingScope = visibleScopePredicate(visibleScopes, "current.scope");
    mentions = db.prepare(`
      SELECT DISTINCT mention.entity_display
      FROM memory_entity_mentions AS mention
      JOIN memory_current AS current ON current.memory_id = mention.memory_id
      WHERE current.status = 'active'
        AND ${mentionScope.sql}
        AND ${backingScope.sql}
        AND (
          lower(mention.entity_key) = lower(?)
          OR lower(mention.entity_key) = lower(?)
        )
      ORDER BY lower(mention.entity_display), mention.entity_display
      LIMIT 100
    `).all(...mentionScope.params, ...backingScope.params, String(entity.normalized_name || ""), id);
  }
  const authorizedNames = [...new Set(mentions.map((row) => String(row.entity_display || "").trim()).filter(Boolean))];
  const scopedEntity = {
    entity_id: String(entity.entity_id || id),
    kind: String(entity.kind || ""),
    confidence: Number(entity.confidence || 0),
    ...(authorizedNames.length > 0 ? {
      display_name: authorizedNames[0],
      aliases: authorizedNames,
    } : {}),
  };
  return `# ${authorizedNames[0] || scopedEntity.entity_id}\n\n${JSON.stringify({ entity: scopedEntity, beliefs }, null, 2)}\n`;
};
