...
const operationHasRowEvents = (db, operationId) => Boolean(db?.prepare(`
  SELECT 1 FROM memory_events
  WHERE json_extract(payload, '$.operation_id') = ?
...
const reviewQueuedCandidates = async ({
  ...
  if (reviewed.length > 0 && applyDecision === defaultApplyDecision && db) {
    const batchOperationId = stableOperationId(operationId || `queue-review:${runId || reviewed.map((item) => item.identity).join(':')}`);
    if (operationHasRowEvents(db, batchOperationId)) {
      for (const item of reviewed) recordApplied({ ...item, applied: appliedCountsForDecision(item.decision) });
    } else {
