...
import { updateCurrentStatus, withProjectionMutationBatch } from '../core/projection-store.js';
...
const stableOperationId = (value) => String(value || 'queue-review')
  .replace(/[^A-Za-z0-9._-]/g, '-')
  .slice(0, 128) || 'queue-review';
...
const defaultApplyDecision = ({
  ...
  if (decision.decision === 'archive_loser') {
    ...
    const timestamp = tx?.now || new Date().toISOString();
    updateCurrentStatus(db, loser, 'archived', { superseded_by: winner, timestamp }, {
      tx,
      faultInjector,
      operationId,
      event: {
        action: 'queue_review_archive_loser',
        component: 'queue_review',
        matched_memory_id: winner,
        memory_id: loser,
        reason_codes: ['queue_review', 'duplicate_semantic'],
        run_id: String(runId || ''),
        timestamp,
        payload: { queue_row_id: String(row.id || ''), winner_id: winner },
      },
    });
...
const appliedCountsForDecision = (decision) => {
  if (decision.decision === 'dismiss') return { dismissed: 1 };
  if (decision.decision === 'keep_both') return { keptBoth: 1 };
  if (decision.decision === 'archive_loser') return { duplicateArchived: 1 };
  return { stored: 1 };
};

const decisionReceiptPayload = ({ decision, identity, row }) => {
  const payload = row?.payload && typeof row.payload === 'object' ? row.payload : {};
  const loser = String(row.loser_memory_id || row.matched_memory_id || payload.matched_memory_id || '').trim() || null;
  const winner = String(row.winner_memory_id || row.memory_id || payload.memory_id || '').trim() || null;
  return {
    loser_id: loser,
    queue_row_id: String(row.id || ''),
    queue_row_identity: identity,
    review_confidence: Number(decision.confidence),
    review_decision: String(decision.decision),
    review_reason: String(decision.reason || ''),
    review_scope: String(decision.scope || payload.scope || row.scope || ''),
    review_type: String(decision.type || payload.type || ''),
    result_status: decision.decision === 'archive_loser'
      ? 'archived'
      : decision.decision === 'store' ? 'stored' : decision.decision === 'dismiss' ? 'dismissed' : 'kept_both',
    winner_id: winner,
  };
};

const appendDecisionReceipt = ({ db, decision, identity, operationId, row, runId }) => appendEvent(db, {
  action: 'queue_review_decision_receipt',
  component: 'queue_review',
  memory_id: `queue-receipt:${String(row.id || identity).slice(0, 96)}`,
  reason_codes: ['queue_review_decision_receipt'],
  run_id: String(runId || ''),
  payload: {
    ...decisionReceiptPayload({ decision, identity, row }),
...
const reviewQueuedCandidates = async ({
  ...
  const reviewed = [];
  const selected = eligible.slice(0, maxRows);
  const receipts = new Map();
  if (applyDecision === defaultApplyDecision && db && selected.length > 0) {
    ...
    for (const receipt of rows) {
      let payload = {};
      try { payload = JSON.parse(String(receipt.payload || '{}')); } catch { continue; }
      if (payload.queue_row_identity) receipts.set(String(payload.queue_row_identity), payload);
    }
  }
  for (const row of selected) {
    ...
    if (receipt) {
      reviewed.push({
        ...
        row,
        decision: {
          confidence: Number(receipt.review_confidence || 0),
          content: '',
          decision: String(receipt.review_decision || ''),
          reason: String(receipt.review_reason || ''),
          scope: String(receipt.review_scope || ''),
          type: String(receipt.review_type || ''),
        },
        fromReceipt: true,
      });
      continue;
    ...
    try {
      ...
      reviewed.push({ at, decision, identity, row });
    } catch (error) {
  ...
  const recordApplied = ({ applied, at, decision, identity, row }) => {
  ...
  if (reviewed.length > 0 && applyDecision === defaultApplyDecision && db) {
    const completed = reviewed.filter((item) => item.fromReceipt === true);
    const pending = reviewed.filter((item) => item.fromReceipt !== true);
    for (const item of completed) recordApplied({ ...item, applied: appliedCountsForDecision(item.decision) });
    if (pending.length > 0) {
      const batchOperationId = stableOperationId(operationId || `queue-review-${sha256(
        pending.map((item) => item.identity).sort().join('|'),
      )}`);
      await withNativeMemoryLock(config, () => withProjectionMutationBatch({
        db,
        now: new Date(atMs).toISOString(),
        operationId: batchOperationId,
      }, (tx) => {
        for (const item of pending) {
          const applied = defaultApplyDecision({
            config,
            db,
    ...
    for (const item of reviewed) {
      try {
        const invokeDecision = () => applyDecision({ config, db, decision: item.decision, row: item.row, runId });
        // A custom decision callback without a DB is observational with respect
        // to the registry/native pair. Any callback given a DB is treated as a
        // writer and must enter through the same native→DB order as the default.
        const applied = db
          ? await withNativeMemoryLock(config, invokeDecision)
          : await invokeDecision();
        recordApplied({ ...item, applied });
      } catch (error) {
        const replacement = failureRow({ at: item.at, atMs, error, row: item.row });
        replacements.set(item.identity, replacement);
        if (replacement.status === 'failed_retryable') result.retryable += 1;
        else result.terminal += 1;
      }
    }
  }
  if (typeof completionFaultInjector === 'function') completionFaultInjector('before_queue_completion');
