...
const registerHttpCompatibility = (api, config, logger) => {
  if (typeof api.registerHttpRoute !== "function" && typeof api.registerHttpHandler !== "function") return;
  const token = String(config?.runtime?.apiToken || "").trim();
  const allowNoAuth = config?.runtime?.allowNoAuth === true;
  if (!token && !allowNoAuth) return;
