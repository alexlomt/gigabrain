...
const discoverNestedCliBranches = ({ source, commandFunctions, writerSymbols = new Set() }) => {
  const rows = [];
  const seen = new Set();
...
const discoverCliEntrypoints = (repoRoot, writerSymbols = new Set()) => {
  ...
  rows.push(...discoverNestedCliBranches({ source, commandFunctions, writerSymbols }));
  return rows;
};
