...
const discoverNestedCliBranches = ({ source, commandFunctions }) => {
  const rows = [];
  const seen = new Set();
  for (const [commandName, functionName] of commandFunctions) {
    ...
    const pushBranch = (rawSubcommand, branch) => {
      ...
      seen.add(operation);
      rows.push({
        access: containsMutation(branch) ? "write" : "read",
        operation,
...
const discoverCliEntrypoints = (repoRoot) => {
  ...
  rows.push(...discoverNestedCliBranches({ source, commandFunctions }));
  return rows;
};
