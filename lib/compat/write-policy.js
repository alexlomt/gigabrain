...
const discoverNestedCliBranches = ({ source, commandFunctions, declaredWriteOperations = new Set(), writerSymbols = new Set() }) => {
  const rows = [];
  const seen = new Set();
  for (const [commandName, functionName] of commandFunctions) {
    const body = extractBalancedFunctionBody(source, functionName);
    const pushBranch = (rawSubcommand, branch) => {
      ...
      seen.add(operation);
      const commandToken = normalizeOperationToken(commandName).replaceAll("-", "_");
      const subcommandToken = subcommand.replaceAll(".", "_");
      const canonicalOperation = [
        `cli.${commandToken}_${subcommandToken}`,
        ...(commandToken === "migrate" ? [] : [`cli.${commandToken}`]),
      ].find((candidate) => declaredWriteOperations.has(candidate)) || operation;
      rows.push({
        access: containsMutation(branch, writerSymbols) ? "write" : "read",
        canonicalOperation,
        operation,
...
const discoverCliEntrypoints = (repoRoot, writerSymbols = new Set()) => {
  ...
  rows.push(...discoverNestedCliBranches({ source, commandFunctions, declaredWriteOperations, writerSymbols }));
  return rows;
};
...
const discoverWriterEntrypoints = ({ repoRoot = path.resolve(import.meta.dirname, "..", "..") } = {}) => {
  const exportedWriters = discoverExportedWriterEntrypoints(repoRoot);
  const writerSymbols = new Set(exportedWriters.map((entry) => entry.symbol).filter(Boolean));
  const rows = [
