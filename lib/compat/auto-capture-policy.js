...
const messageExclusionReason = (message = {}) => {
  ...
  if (kinds.some((kind) => /^(?:tool|function)(?:_|$)/.test(kind)) || PLAINTEXT_TOOL_CHATTER_RE.test(text)) {
    return 'tool_event';
  }
  if (kinds.some((kind) => /(?:^|_)(?:test|fixture|harness)(?:_|$)/.test(kind)) || PLAINTEXT_TEST_CHATTER_RE.test(text)) {
    return 'test_event';
  }
  return '';
};
...
const classifyAutoCaptureCandidate = (candidate = {}) => {
  ...
  if (
    ...
    || candidate?.test === true
    || PLAINTEXT_TOOL_CHATTER_RE.test(content)
    || PLAINTEXT_TEST_CHATTER_RE.test(content)
  ) {
