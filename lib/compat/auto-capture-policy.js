...
const EXPLICIT_DURABLE_RE = /(?:\b(?:remember(?:\s+that)?|save this|note this down|keep this in memory)\b|(?:^|\n)\s*decision\s*:)/i;
const EXPLICIT_ASSISTANT_DECISION_RE = /^\s*decision\s*:\s*\S/i;
const SECRET_RE = /(?:\bBearer\s+[A-Za-z0-9._~+/=-]{12,}|\b(?:sk|rk|pk|sess|ghp|github_pat|xox[baprs])[-_][A-Za-z0-9._-]{8,}|\b[A-Za-z0-9_.-]*(?:api[_ -]?key|access[_ -]?token|auth[_ -]?token|secret|password|passwd|pwd|client[_ -]?secret)[A-Za-z0-9_.-]*\s*[:=]\s*["']?[^"'\s,;]+|\b(?:token|authorization|cookie|session)\s*[:=]\s*["']?[^"'\s,;]+|\bAKIA[0-9A-Z]{16}\b|-----BEGIN [A-Z ]*PRIVATE KEY-----|\beyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{5,}\b)/i;
const CREDENTIAL_URL_RE = /\b[a-z][a-z0-9+.-]*:\/\/[^\s/:@]+:[^\s/@]+@/i;
const WHITESPACE_SECRET_RE = /\b(?:[A-Za-z0-9_]*(?:api_key|access_token|auth_token|client_secret|github_token|secret|password|passwd|pwd)|token|authorization|cookie|session)\b\s+["']?[^"'\s,;]+/i;
...
const NEGATED_OR_QUESTION_RE = /^\s*(?:i\s+(?:do\s+not|don't|cannot|can't)\s+remember\b|(?:do|can|could|would)\s+you\s+remember\b)/i;
const REASONING_TAG_RE = /<(?:thinking|reasoning|analysis)\b[\s\S]*?<\/(?:thinking|reasoning|analysis)>/i;
const PLAINTEXT_TOOL_CHATTER_RE = /^\s*(?:\[tool(?:[_ -](?:call|result|output))?\]|<tool(?:[_-](?:call|result|output))?\b|tool\s+(?:call|result|output)\s*:)/i;
const PLAINTEXT_TEST_CHATTER_RE = /^\s*(?:\[test(?:[_ -](?:fixture|harness|result|output))?\]|test\s+(?:fixture|harness|result|output)\s*:)/i;
const EXECUTION_TOOL_CHATTER_RE = /(?:\bfunctions\.exec\b|\braw_params\b|\[tools?\]|\bdiagnostics?\b)/i;
const UNIT_TEST_CHATTER_RE = /\bunit tests?\b/i;
const PAPERCLIP_WAKE_RE = /\bpaperclip\b[^\n]{0,80}\bwake\b|\bwake\b[^\n]{0,80}\bpaperclip\b/i;
const PAPERCLIP_SAFE_LINE_RE = /^(issue|reason|objective)\s*:\s*(.+)$/i;
const PAPERCLIP_LABELS = Object.freeze({ issue: 'Issue', reason: 'Reason', objective: 'Objective' });

const isRecord = (value) => Boolean(value && typeof value === 'object' && !Array.isArray(value));
const cleanText = (value) => String(value ?? '').replace(/\r\n?/g, '\n').trim();
const compactText = (value) => cleanText(value).replace(/[ \t]+/g, ' ').replace(/\n{3,}/g, '\n\n').trim();
const containsSensitiveAutoCaptureContent = (value) => {
  const text = compactText(value);
  return SECRET_RE.test(text)
    || CREDENTIAL_URL_RE.test(text)
    || WHITESPACE_SECRET_RE.test(text)
    || HOST_POSTURE_RE.test(text);
};
...
const sanitizePaperclipWake = (value) => {
  ...
  const lines = text
    ...
    .filter(Boolean)
    .map((line) => PAPERCLIP_SAFE_LINE_RE.exec(line))
    .filter(Boolean)
    .map((match) => `${PAPERCLIP_LABELS[String(match[1]).toLowerCase()]}: ${String(match[2]).trim()}`);
...
const secretScanEventText = (event = {}) => {
  const parts = [];
  for (const key of ['prompt', 'text']) {
    const value = contentText(event?.[key]);
    if (value) parts.push(PAPERCLIP_WAKE_RE.test(value) ? sanitizePaperclipWake(value) : value);
  }
  for (const key of ['output', 'response', 'result', 'final']) {
    const value = contentText(event?.[key]);
    if (value) parts.push(value);
  }
  for (const message of Array.isArray(event?.messages) ? event.messages : []) {
    ...
    parts.push(
      ['user', 'human'].includes(role) && PAPERCLIP_WAKE_RE.test(value)
        ? sanitizePaperclipWake(value)
        : value,
    );
  }
  return parts.join('\n');
...
const structuralMessageKind = (message = {}) => [message?.type, message?.kind]
  .map((value) => String(value || '').trim().toLowerCase().replace(/[ -]+/g, '_'))
  .filter(Boolean);

const textExclusionReason = (text = '') => {
  const value = contentText(text);
  if (REASONING_TAG_RE.test(value)) return 'reasoning_event';
  if (UNIT_TEST_CHATTER_RE.test(value) || PLAINTEXT_TEST_CHATTER_RE.test(value)) return 'test_event';
  if (EXECUTION_TOOL_CHATTER_RE.test(value) || PLAINTEXT_TOOL_CHATTER_RE.test(value)) return 'tool_event';
  return '';
};

const messageExclusionReason = (message = {}) => {
  const role = String(message?.role || message?.author || '').trim().toLowerCase();
  const kinds = structuralMessageKind(message);
  const text = contentText(message?.content ?? message?.text ?? message?.output);
  if (
    ['reasoning', 'thinking', 'analysis'].includes(role)
    || kinds.some((kind) => ['reasoning', 'thinking', 'analysis'].includes(kind))
    || message?.reasoning != null
    || message?.thinking != null
    || message?.analysis != null
    || REASONING_TAG_RE.test(text)
  ) return 'reasoning_event';
  if (kinds.some((kind) => /^(?:tool|function)(?:_|$)/.test(kind)) || textExclusionReason(text) === 'tool_event') {
    return 'tool_event';
  }
  if (kinds.some((kind) => /(?:^|_)(?:test|fixture|harness)(?:_|$)/.test(kind)) || textExclusionReason(text) === 'test_event') {
    return 'test_event';
  }
  return '';
};
...
const classifyAutoCaptureCandidate = (candidate = {}) => {
  ...
  if (
    ...
    || candidate?.test === true
    || textExclusionReason(content)
  ) {
    return { action: 'reject', reason: 'excluded_content' };
  }
  if (
    candidate?.containsSecret === true
    || containsSensitiveAutoCaptureContent(content)
  ) {
    return { action: 'reject', reason: 'sensitive' };
  }
  if (NEGATED_OR_QUESTION_RE.test(content)) {
    return { action: 'reject', reason: 'negated_or_question' };
  }
...
const collectTurns = (event = {}) => {
  ...
  const append = (role, value) => {
    ...
    if (normalizedRole === 'user' || normalizedRole === 'human') content = sanitizePaperclipWake(content);
      ...
      if (!EXPLICIT_ASSISTANT_DECISION_RE.test(content)) return;
    }
    if (!content) return;
  ...
  for (const message of Array.isArray(event?.messages) ? event.messages : []) {
    if (messageExclusionReason(message)) continue;
...
const prepareAutoCaptureEvent = ({ config = {}, context = {}, event = {} } = {}) => {
  ...
  const excludedText = textExclusionReason(rawText);
  if (excludedText) return { eligible: false, reason: excludedText };
  const excludedMessage = (Array.isArray(event?.messages) ? event.messages : [])
    .map((message) => messageExclusionReason(message))
    .find(Boolean);
  if (excludedMessage) return { eligible: false, reason: excludedMessage };
  const secretText = secretScanEventText(event);
  if (containsSensitiveAutoCaptureContent(secretText)) return { eligible: false, reason: 'sensitive' };
  ...
  let decision = classifyAutoCaptureCandidate({
    ...
    mode,
    role: 'user',
    scope: scope === 'shared' ? 'profile:main' : scope,
  });
  if (decision.action === 'reject') return { eligible: false, reason: decision.reason };
  const assistantDecision = messages.some((turn) => turn.role === 'assistant');
  const explicit = decision.reason === 'explicit_durable_request' || assistantDecision;
  ...
  if (assistantDecision) decision = { action: 'save', reason: 'explicit_durable_request' };
  if (scope === 'shared') decision = { action: 'review', reason: 'shared_review_only' };
...
const createAutoCaptureHook = ({
  ...
  queueMicrotask(() => {
    ...
    Promise.resolve(pending).then((result) => {
      if (result?.enqueued !== false) return;
      const reason = String(result?.reason || '').trim().toLowerCase();
      if (['duplicate', 'disabled', 'capture_disabled', 'auto_capture_disabled'].includes(reason)) return;
      const safeReason = ['queue_full', 'invalid_event', 'sensitive'].includes(reason) ? reason : 'rejected';
      logger.warn?.(`[gigabrain] auto-capture enqueue not accepted reason=${safeReason}`);
    }).catch(() => logger.warn?.('[gigabrain] auto-capture enqueue failed'));
  });
  return undefined;
};
...
export {
  ...
  containsSensitiveAutoCaptureContent,
