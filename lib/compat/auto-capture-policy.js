...
const EXECUTION_TOOL_CHATTER_RE = /(?:\bfunctions\.exec\b|\braw_params\b|\[tools?\]|\bdiagnostics?\s*:)/i;
const UNIT_TEST_CHATTER_RE = /\bunit tests?\b(?:\s*:\s*|\s+(?:passed|failed|completed|running)\b)/i;
...
const prepareAutoCaptureEvent = ({ config = {}, context = {}, event = {} } = {}) => {
  ...
  if (SECRET_RE.test(secretText) || HOST_POSTURE_RE.test(secretText)) return { eligible: false, reason: 'sensitive' };
