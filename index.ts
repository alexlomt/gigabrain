...
import { ensureWorldModelReady, ensureWorldModelStore, getSynthesis, projectArbitrationBeliefRows, rebuildWorldModel } from './lib/core/world-model.js';
  ...
  register(api: PluginApi) {
    ...
    withDb(dbPath, config, (db) => {
      ...
      if (config.worldModel?.enabled !== false) {
        ...
        // U17: world model OFF still arbitrates — verdicts are independent of
        // the world-model surfaces.
        const arbitration = runBeliefArbitration({ db, config, projectBeliefRows: projectArbitrationBeliefRows });
        logger.info?.(`[gigabrain] belief arbitration (world model OFF) beliefs=${arbitration.counts.beliefs} verdicts=${arbitration.counts.verdicts}`);
      }
    ...
    if (api.registerHttpHandler || api.registerHttpRoute) {
      ...
      if (!token && !allowNoAuth) {
        ...
        if (allowNoAuth && !token) {
          logger.warn?.('[gigabrain] DANGER: GB_ALLOW_NO_AUTH is enabled; all /gb data routes accept unauthenticated requests');
        }
