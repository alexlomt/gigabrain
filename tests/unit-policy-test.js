...
const run = async () => {
  ...
  const relationshipContinuity = classifyValue({
    type: 'PREFERENCE',
    scope: 'example-agent',
    content: 'The fictional user treats the Atlas assistant as someone, not something, and expresses synthetic care and appreciation.',
    confidence: 0.9,
    updated_at: new Date().toISOString(),
  }, policy);
  ...
  const durableUserGoal = classifyValue({
    type: 'USER_FACT',
    scope: 'example-agent',
    content: 'A fictional user wants to improve fitness and reach a synthetic goal weight of 80kg',
    confidence: 0.58,
    updated_at: new Date().toISOString(),
  }, policy);
  ...
  const technicalOpsFact = classifyValue({
    type: 'USER_FACT',
    scope: 'example-agent',
    content: 'Example vacuum found at 192.0.2.52:54321 (test protocol, synthetic device reference demo-031). Control needs a test token.',
    confidence: 0.8,
    updated_at: new Date().toISOString(),
  }, policy);
  ...
  for (const [left, right] of [
    ['Project alpha launched today', 'Project alpha launched today'],
    ['User prefers tea at 7', 'User prefers coffee at 8'],
    ['', 'non-empty'],
    ['A short project note', 'Completely unrelated wording'],
  ]) {
    assert.equal(
      jaccardSimilarityFromFingerprints(
        createSemanticFingerprint(left),
        createSemanticFingerprint(right),
      ),
      jaccardSimilarity(left, right),
      'cached semantic fingerprints must preserve similarity behavior',
    );
  }
};
