...
const run = async () => {
  ...
  assert.equal(longSlugConfig.vault.inbox.enabled, true);
  assert.equal(longSlugConfig.vault.inbox.maxFindings, 100, 'vault inbox digest size should be clamped');
  ...
  const malformedConfigRoot = fs.mkdtempSync(path.join(
    process.env.GIGABRAIN_TEST_TMPDIR || os.tmpdir(),
    'gb-malformed-config-',
  ));
  const malformedStandalonePath = path.join(malformedConfigRoot, 'standalone.json');
  ...
  const malformedOpenclawPath = path.join(malformedConfigRoot, 'openclaw.json');
  ...
  fs.rmSync(malformedConfigRoot, { recursive: true, force: true });
};
