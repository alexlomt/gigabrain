...
OpenClaw already ships a strong native memory engine. Gigabrain is an alternative memory-slot provider for operators who specifically need cross-host provenance, arbitration, and the shared Gigabrain protocol. If OpenClaw's built-in memory covers your workflow, keep it.
...
- Enables the current hybrid memory defaults for explicit remember intent, native promotion, and world-model-aware surfaces
...
1. Install Obsidian if you want the optional memory surface.
