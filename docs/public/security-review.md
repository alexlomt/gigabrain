...
- Every Node/OpenClaw `/gb` route is registered with `auth: "gateway"`; OpenClaw blocks unauthenticated dispatch before the Gigabrain handler runs. The deprecated generic-handler fallback registers no routes. Gigabrain then applies bounded bodies, input validation, bounded per-peer rate-limit state, and explicit budgets for expensive benchmark, recall, control, and suggestion routes.
