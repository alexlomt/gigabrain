...
Codex native memories stay a useful local recall layer. Required project rules should remain in `AGENTS.md` or checked-in documentation; Gigabrain adds a separate, auditable project/user store over MCP.
...
Bootstrap Codex wiring for the current repo. Fresh installs use the host-neutral standalone store under `~/.gigabrain/`, keep the personal user store under `~/.gigabrain/profile/`, and derive a stable repo-specific scope for the current workspace. A supported legacy install under `~/.codex/gigabrain/` is reused in place:
...
- Codex desktop, CLI, and IDE clients on the same Codex host can share MCP configuration; that does not synchronize Gigabrain data with another computer.
...
Run setup and doctor independently on each host. Matching versions are not proof of matching state. For a deliberate one-time transfer, export an integrity-hashed bundle on the source and import it on the destination:
...
Review and transport the bundle as sensitive data. For a persistent connection, configure the authenticated remote bridge explicitly; it is off by default.
...
Re-run `npx gigabrain-codex-setup --project-root /path/to/repo` to refresh the standalone defaults, verify helper scripts, and doctor path. Existing `~/.codex/gigabrain` installs remain supported in place, but fresh installs use `~/.gigabrain`.
