...
if ! command -v node >/dev/null 2>&1; then
  echo "Gigabrain Claude Desktop launcher could not find Node.js 22.18+." >&2
  echo "Install Node.js 22.18+ and ensure PATH includes one of: Homebrew, Volta, ~/.nvm, ~/.fnm, ~/.asdf, or ~/.local/bin." >&2
  exit 127
fi
